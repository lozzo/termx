...
- 这份文档描述“产品和交付主线”
- 内部代码如何重构，另见 [`refactor-roadmap.md`](refactor-roadmap.md)

---

## 1. 当前总体判断

当前 TUI 不需要推倒重写，但也不应继续只靠补丁前进。
...
- 全重写会丢失已经修好的复杂边界
- 继续补丁式开发会让 shared terminal、input、render cache 更难维护
...
- 已有基线
- `shared + floating + owner/follower + fit/fixed + acquire + alt-screen` 组合场景已补进回归
- 仍需继续补复杂边界测试
...
- 以新整理的 4 份主文档为准
- 增补架构重构文档与分阶段重构 roadmap
- 清理旧补丁式文档
- 确认术语、心智模型、当前状态描述一致
- 明确后续代码改动以 staged refactor 为主，不再继续散点补丁
...
- resize / acquire / owner migration / stop / exit 混合场景

### Phase 4. 架构重构主线

目标：

- 按 [`architecture-refactor.md`](architecture-refactor.md) 与 [`refactor-roadmap.md`](refactor-roadmap.md) 拆分状态与输入系统
- 先抽连接状态，再统一输入，再清理 reducer/runtime/render 边界

完成标准：

- `owner / follower` 成为显式模型
- intent / reducer / runtime 职责清楚
- render cache 失效主要回到 render layer
- 全量测试持续稳定

### Phase 5. UI 视觉统一与 modernize
...
6. 结构性变更先跟 `architecture-refactor.md` 对齐
