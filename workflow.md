...
当前下一步：切片 41 待开始。自动执行必须从切片 41 开始，建立 layout measurement 与 viewport 驱动 renderer；不得跳到 active content resize、copy mode rebind、Unicode 线框或默认入口验收。
...
- 切片 41 待开始：下一次 `/goal` 或自动推进必须先处理 layout measurement 与 viewport 驱动 renderer，不能先改线框或直接修默认入口 smoke。
