...
- 当前最早未完成切片是 `AR002`。在 `AR001-AR005` 全部完成前，暂停 `C3B` 以及后续连接功能开发。
...
| AR002 | 进行中 | client contract 与依赖图 | 先定义 `client/runtime` application contract、session/attempt/lifecycle DTO 和 `client/port` host capability；小 harness 验证 owner、取消、释放和错误边界；不实现网络 |
| AR003 | 待开始 | TUI application boundary 清理 | 按 history/terminal/path/storage/clipboard/endpoint event 拆分 `tui/port`；移除 TUI state/port 对 Cloud/protocol concrete type 的依赖；建立 `tui/adapter/clientruntime` 接口壳和守卫 |
