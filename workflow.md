...
| 85. pane chrome 目标截图级槽位重绘 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在切片 81 的连续线框基础上继续对齐目标截图：pane top chrome 使用更接近 `tuiv2` 的 title/status/owner/follower/action 槽位，title 与 action 之间保持连续线段，active pane accent 更明显，inactive muted 更弱，card/split/floating 都不出现断线、虚线感或样式丢失；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 85 待开始。切片 84 已完成 shell bar 高密度重绘第一轮：默认 smoke 顶部不再输出 `ws:/tab:/active:`，底部不再输出 `mode:/keys:`，而是输出 workspace/tab/action/owner token 和 `MODE • [KEY] ACTION` footer。切片 85 继续做 pane chrome 目标截图级槽位重绘。terminal live streaming/rich attributes、copy-history 深层语义、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 84-87 内，必须另行新增切片后再做。
...
- 当前任务队列已重新打开截图级视觉返工：切片 85-87 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
