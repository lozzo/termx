...
当前下一步：继续切片 34，建立 tui-v3 shell/panel/toast reducer-owned 状态模型；不得扩展到切片 35 的 RenderVMBuilder 分层重建。
...
- 实现前检查已更新：当前 `state.Root` 仍缺少 shell/panel/toast/overlay 状态；这不是架构阻塞，但必须按切片 34-39 分阶段落地。
