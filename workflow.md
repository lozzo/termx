...
当前下一步：215PR 已完成，真实 CLI attach 已改成阻塞唤醒事件循环，拖窗 resize 的 latest-wins 与 owner 回弹链路已收口。history MVP 主线中的 `215H1 live/history 边界与 contract 收口` 与 `215H2 core-v2 authoritative history MVP` 已完成；现有 `history.window` wire contract 继续维持 terminal-scoped 设计，core-v2 已补齐 older stale guard 对 cursor/boundary/cols/token/generation 的 authoritative 校验。下一切片切到 215H3 tui-v3 history binding MVP；215D1 floating group commands 与 215F shortcut integration/tmux harness 继续顺延到 history MVP 后；215E clipboard paste/history overlay 继续保持阻塞，直到 215H1-215H3 达到可验收基线后再重启。继续保持 copy/history 不从 live surface、snapshot 或 local VTerm scrollback 推断历史，也不得把 styled payload parser 扩张成补丁式完整 VT emulator。
