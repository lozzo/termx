...
- `termx-app/` 与 `remote-ui/`，在 RP006 统一客户端 contract、LIC001 notice 分发、RP007 public snapshot 边界、GA001A Android 质量观测和 GA002 跨平台 SmartRoute contract/ICE 计划切片内触及；不得引入私有源码构建依赖或 legacy fallback。
...
以上目录只供代码考古和迁移决策参考，不加入 `go.work`、构建脚本、依赖图或 runtime fallback。活动公开客户端位于 `remote/`、`termx-app/` 与 `remote-ui/`；活动闭源服务位于 `private/termx-cloud/`。
...
- CL004 已完成（历史状态）：当时撤回“旧 remote/app/web 目录无效可删”的判断并恢复资产；RP005/RP007 随后已把旧 Hub、`termx-remote/` 与 `web-control/` 收口到私有 archive，活动 `termx-app/`、`remote-ui/` 和 `remote/` 按公开 contract 保留。
