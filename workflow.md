...
| 91. 目标截图级整体 UI 构图三轮重绘 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在用户确认切片 89 后仍不通过的基础上，不再只做局部 token polish，而是按目标截图重做默认 TUI 的整体构图和视觉层级：top bar / bottom bar 的整屏填充和密度、pane card/split/floating 的比例与留白、active/inactive 边框强弱、toast/modal 实体 card、默认首屏内容占位、窄屏退化、真实 ANSI frame 截图级 smoke 对照；必须继续使用 render framework + content renderer，不复制 `tuiv2` runtime/model，不引入 Bubble Tea；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 91 待开始。用户已经明确反馈当前 TUI 样子仍与目标不一致，因此切片 90 以不通过归档完成，当前视觉对齐 goal 不能标记完成。下一轮必须进入整体 UI 构图三轮重绘，不能继续把“有 styled ANSI frame、有 Unicode 线框、有 chrome token”当作视觉完成证据。
