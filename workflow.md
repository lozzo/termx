...
- 稳定跨语言 binding contract `PA005B` 已完成。下一切片是 `PA005N1` Android native/JNI 纵向 spike；Android 与 Web 必须复用当前 runtime/managed/auth/protocol/binding 引擎，不再按两套客户端网络实现分别打补丁。
