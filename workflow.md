...
- `PG004` 公网 bootstrap staging 已部署：155 使用 systemd 运行 Controller + US West Edge，114 使用 systemd 运行 China East Edge，真实 Supabase `muxvia_staging` schema、Cloudflare DNS、Let's Encrypt、Web 登录/二维码、operator 双 fleet 和三进程重启恢复均通过；证据见 `docs/remote-platform/pg004-public-bootstrap-deployment.md`。R2 恢复、Android HTTPS production origin、正式 daemon enrollment 与完整 managed terminal/file E2E 尚未完成，因此 PG004 继续保持进行中。
...
| PG004 | 进行中 | Supabase staging、公网 bootstrap 装配与备份恢复验收 | Supabase Session pooler 已从 155 完成 IPv4/TLS/登录验证；使用专用用户和 systemd 运行一个 Controller 与两个独立 Edge，接通 HTTPS/DNS、真实 Supabase schema、Web 注册/登录、bootstrap daemon enrollment、Android 扫码与 managed P2P/Relay 基础链路；服务可用后补 R2 加密上传和独立恢复。bootstrap 单账号 enrollment、测试支付或未配置真实 provider 必须明确标记 staging，不得宣称正式商业生产 |
