...
| 96. floating 鼠标拖动移动与 resize | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-tui-v3/input/`、`termx-tui-v3/state/`、`termx-cli/` 按需 | floating window 必须能用鼠标拖动标题栏移动，并能拖动边框/角落 resize；点击 raise、close、collapse 等只有在功能真实可用时才显示可点击 token；拖动必须 clamp 在 viewport 内，保持 z-order、active 状态、content rect 裁切、toast/overlay 前景优先和 no terminal input leak；harness 覆盖 move drag、resize drag、collapse/close 可见性、viewport clamp 和 smoke 输出；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 96 待开始。用户最新复核指出的 pane 鼠标拖动 resize 已在切片 94 接入真实 SGR drag/release 和 runtime transient drag state；横纵分屏入口与可见按钮一致性已在切片 95 完成；下一步需要补齐 floating 标题栏拖动移动和边框/角落连续 resize。
...
- 当前任务队列下一步是切片 96：floating 鼠标拖动移动与 resize。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
