...
| 79. TUI 视觉差距审计与验收基线 | 待开始 | `termx-tui-v3/docs/`、`termx-tui-v3/render/`、`termx-cli/`、`workflow.md` 按需 | 重新以用户提供的 `tuiv2` 截图和目标描述为视觉基准，审计当前 `v3 smoke` / 默认 TUI 与目标的差距；补固定 viewport visual snapshot/smoke case 和文档验收项，明确 top bar、bottom bar、pane chrome、active accent、inactive muted、overlay、toast、floating、copy-history、Terminal Pool/Tree 的视觉要求；不得把现有基础 styled chrome 继续标为视觉完成；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 79 待开始。切片 75-78 已完成产品壳、基础 styled renderer、terminal live 前推、copy-history 深化和 render 输入清理，但当前 `v3 smoke` / 默认 TUI 的视觉效果尚未达到用户截图目标。下一阶段必须先做视觉差距审计与验收基线，再按顺序推进 header/footer、pane chrome、overlay/toast/floating 和真实默认 TUI 视觉验收。terminal live streaming/rich attributes、copy-history 最终 polish、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 79-83 内，必须另行新增切片后再做。
...
- 当前任务队列已重新打开视觉对齐返工：切片 79-83 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
