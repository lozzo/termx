...
当前下一步：切片 70 进行中。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期、Terminal Picker 真实交互深化和 Terminal Pool 数据源与 Picker 服务接线一期已完成；本切片只实现独立 Terminal Pool 管理页一期，并以页面可见性、action 可操作性、service/effect/result 边界和 no terminal input leak 作为验收重点。切片 71-77 已补充为后续顺序：Workbench Tree、floating、Prompt/Help、Tab/Workspace、terminal-live 深化、copy-history 深化和 render cleanup/performance。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary 仍不在当前队列内，必须另行新增切片后再做。
...
- 当前下一阶段任务队列继续：切片 70 Terminal Pool 管理页一期。切片 70 的当前文档口径已收紧为独立 terminal-pool page/content、常规 viewport 下 list/detail/preview/action 可见、鼠标与键盘共用 selected row/action 语义、attach/edit/kill 必须走 service/effect/result 且不得伪造 lifecycle。后续如继续推进 Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
