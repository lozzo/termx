...
- 已收紧 mutable live tail metadata 恢复校验：`live_records` 的 origin 必须是 live/reclaimed/resize 之一，且 dirty 必须与 origin 一致，live/resize 为 dirty、reclaimed 为 clean；未知 origin 或 dirty/origin 不一致会被视为损坏 metadata 并忽略恢复。
