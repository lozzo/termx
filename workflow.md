...
- 当前最早未完成切片是 `R186. SK remote transport session core-v2 routing`，状态为 `待开始`。
- 当前明确缺口：remote WebRTC/datachannel transport session 还需要 R186 的 core-v2 scoped transport routing smoke；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
