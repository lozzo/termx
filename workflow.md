...
| 88. 目标截图级 shell/pane 视觉重绘二轮 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 以用户最新反馈和 `tuiv2` 截图为准，继续重绘默认 workbench 视觉：top bar 必须更像产品级 tab/status chrome，bottom bar 必须更像 `tuiv2` 彩色快捷键 taxonomy，pane top chrome 必须更接近截图中的 title、状态点、owner、时间/序号/action 密度，active/inactive/floating/toast 的层级必须用 ANSI style 明显区分；不得只新增 smoke marker 伪装完成；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 88 待开始。切片 87 已确认当前真实 TUI 仍与目标截图不一致，只能作为复核未通过归档和 smoke 证据扩展，不能作为视觉完成证据。切片 88 必须直接处理 shell/pane 的真实视觉重绘，不得只新增文本 marker、不得回退到旧 `tuiv2`、不得把 terminal live streaming/rich attributes、copy-history 深层语义、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary混入本轮视觉重绘范围。
...
- 当前任务队列已重新打开截图级视觉返工：切片 86-87 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
