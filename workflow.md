...
当前下一步：继续切片 35，重建 RenderVMBuilder 分层；不得扩展到切片 36 的实际线框渲染器。
...
- 实现前检查已更新：当前 `RenderVMBuilder` 仍主要输出旧 `RenderVM{Lines, Status}` 兼容模型；下一步必须按切片 35 把 state shell/body/layout/panel/content/overlay/toast 投影成分层 VM。
