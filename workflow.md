...
| 99. pane split 图标有效性复核 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-tui-v3/state/`、`termx-cli/` 按需 | 针对用户反馈的部分窗口点击分屏图标无效问题，复核 pane chrome 可见 action token、hit region、目标 pane、active pane 和 NewPane 默认填充；只绘制真实可执行的 split-down、split-right、close 入口，窄 pane 或不可执行状态不得显示假按钮；harness 覆盖 active/non-active pane、横向/纵向分屏图标、不同 pane kind、窄宽退化和无 invalid toast；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 98 已完成，继续切片 99，复核 pane split 图标在不同 pane/窗口状态下的可见性与点击有效性；切片 100-101 不得在切片 99 未完成前扩展实现。
