...
- 已收紧 core persisted logical line record metadata 写出：只有完整 sealed persisted logical line records 才写入 `grid.lines.json`，尾部未封口 wrapped prefix 不再伪装成 persisted sidecar truth。
