...
| R350. SK TUI live render loop 重写 | 待开始 | `termx-tui-v3/services/`、`termx-tui-v3/state/`、`termx-tui-v3/app/` | TUI 改为订阅 live invalidation、维护 `wanted_revision + in_flight + dirty` 渲染请求状态、渲染完再拉最新；screen、lifecycle、resize-control 消息分离，不再用 LiveSurface union event 和 runtime 旧队列补丁追帧 |
| R351. SK 旧 live projection 链路删除 | 待开始 | `termx-core-v2/`、`internal/protocol/`、`termx-tui-v3/` | 在新 live contract 和 TUI render loop 站稳后，删除 v3 live 对 `Snapshot/CompactSnapshot`、`TypeScreenUpdate` stream、`grid.viewport`、snapshot overlap trimming、`LiveSurfaceSnapshot.Lines` fallback 和旧 coalescer 的依赖 |
...
- `R349` 已完成：core-v2 新增 `LiveRevision`、`NativeScreenSnapshot`、`LiveScreenInvalidated` domain contract；protocol 新增 `live.screen.get` 和 `terminal.live.invalidated` event，live revision 写入独立 live 字段，不再借 `Snapshot/CompactSnapshot.HistoryGeneration`。普通 PTY 输出、resize、restart 和 exit marker 都发布明确 live invalidation；旧 attach `ScreenUpdate` stream 暂时改为监听 live invalidation，等待 R351 删除。`snapshot`/`SnapshotCompact` 仍保留过渡兼容，但不再承载 live revision。新增 protocol/native screen payload roundtrip、live invalidation event roundtrip、core live screen revision 单调、terminal live invalidation 和 scoped transport 覆盖。准入已通过 `go test ./internal/protocol/... -count=1`、`cd termx-core-v2 && go test ./... -count=1`、`cd termx-tui-v3 && go test ./... -count=1`、`git diff --check`。
