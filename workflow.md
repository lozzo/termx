...
- 当前最早未完成切片是 `PA002`。在 `PA001-PA007` 全部完成前，暂停 `C3B` 以及后续连接功能开发。
...
| PA002 | 进行中 | Proto API inventory 与缺口表 | 列出 terminal/attachment/path/history/live/file/storage/workbench/endpoint/access/cloud 的现有 proto、Go-only DTO、consumer、兼容字段和缺失 command/event/error；不写实现 |
| PA003 | 待开始 | 公共 API schema 与 envelope | 在 `proto/` 定义 versioned command/result/event/error/capability/resource handle；区分公共 application API 与内部 wire framing；生成代码和 compatibility harness 通过 |
