...
- `PG004` 公网 bootstrap staging 已部署：155 使用 systemd 运行 Controller + US West Edge，114 使用 systemd 运行 China East Edge，真实 Supabase `muxvia_staging` schema、Cloudflare DNS、Let's Encrypt、Web 登录/二维码、operator 双 fleet 和三进程重启恢复均通过。Android ARM64/API 35 公网 HTTPS profile 已完成 Web 扫码批准、Go-owned managed P2P、Relay 双端租约、Relay terminal attach/input 和远端文件浏览；daemon enrollment session 已补 refresh token 轮换。当前仍未通过 R2 独立恢复、文件上传/下载内容校验、网络切换后 workspace 自动重建、Android account session 自动续期和 Edge 重启后的 daemon Presence 自动恢复，因此 PG004 继续保持进行中；证据见 `docs/remote-platform/pg004-public-bootstrap-deployment.md`。
...
| PG004 | 进行中 | Supabase staging、公网 bootstrap 装配与备份恢复验收 | Supabase Session pooler、systemd Controller + 双 Edge、HTTPS/DNS、真实 schema、Web/Android 扫码、daemon enrollment refresh、managed P2P、Relay 租约/terminal 与远端文件浏览已通过；继续完成 R2 加密上传/独立恢复、Android account refresh、Edge 重启 Presence 恢复、网络切换 workspace 重建及文件上传/下载内容校验。bootstrap enrollment 和测试支付必须明确标记 staging，不得宣称正式商业生产 |
