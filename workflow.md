...
| 215E1-R157. SK floating overlay repaint isolation | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/render/`、`workflow.md` | 已修复产品回归：copy/history 增量 patch 在内容区被 visible floating 覆盖时退回整帧，避免底层 pane 历史滚动把浮窗擦掉；floating 渲染会先铺满自身 overlay 背景，且内容行空白继承 overlay fill，不再透出背后 pane 的 styled ANSI 背景。未改 copy/history truth 或多 view 会话语义；准入 `cd termx-tui-v3 && go test ./... -count=1`、`git diff --check` 通过 |
