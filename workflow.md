...
- 当前最早未完成切片是 `R184. SK remote status/local/pair core-v2 smoke`，状态为 `待开始`。
- 当前明确缺口：remote status/local/pair 还需要 R184 的 core-v2 daemon smoke 证明完整 CLI flow 经过 `termx-remote.Service`；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
