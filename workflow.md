...
| 75. terminal-live content renderer 深化 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-cli/`、协议范围按需 | 在页面与 overlay 基础产品壳稳定后，深化 terminal-live 内容 renderer：richer styled cell、truecolor/link/reverse/underline、terminal mode token、content-local hit region、selection/search、clipped marker 和状态 metadata；仍不得把 live surface 当 history truth，也不得绕过 content rect 或 FrameSink contract；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
| 76. copy-history content renderer 深化 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-cli/`、`termx-core-v2/` 按需 | 在 terminal-live 深化后，深化 copy-history 内容 renderer：scrollbar、搜索、content-local mouse selection、selection 颜色层级、logical-line 拼接提示、position token 精细化和滚动交互；继续只消费 core-v2 authoritative `HistoryWindow`，resize 后不得显示旧 cols rows 或从 live surface fallback；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`cd termx-core-v2 && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
| 77. render 兼容投影清理与性能基线 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-tui-v3/terminalhost/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在主要内容和 overlay 页面稳定后，删除或重命名剩余旧 `RenderVM{Lines, Status}` 兼容语义，建立 content-level cache / layer dirty region / large terminal output 性能基线；不得改变 `RenderResult` 单一路径，不得重新引入裸文本 frame 主路径；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 71 待开始。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期、Terminal Picker 真实交互深化、Terminal Pool 数据源与 Picker 服务接线一期和 Terminal Pool 管理页一期已完成；下一刀进入 Workbench Tree overlay 一期。切片 72-77 已补充为后续顺序：floating、Prompt/Help、Tab/Workspace、terminal-live 深化、copy-history 深化和 render cleanup/performance。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary 仍不在当前队列内，必须另行新增切片后再做。
...
- 当前下一阶段任务队列继续：切片 71 Workbench Tree overlay 一期。后续如继续推进 floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
