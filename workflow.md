...
| 94. pane 鼠标拖动边框 resize | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-tui-v3/input/`、`termx-tui-v3/state/`、`termx-tui-v3/terminalhost/`、`termx-cli/` 按需 | 鼠标按下 pane 分隔线或边框 resize handle 后拖动必须改变 pane 大小，而不是只点击触发一次 resize；drag begin/move/end 必须进入 reducer-owned 或 runtime-owned transient interaction，再回投统一 `PaneCommandResize` / set size contract；拖动过程中 UI chrome 优先于 terminal mouse forwarding，active content rect resize 去重和 copy rebind 必须跟随；harness 覆盖横向和纵向 divider 拖动、边界 clamp、release 后状态清理、terminal resize 参数和 no terminal input leak；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 94 待开始。用户最新复核指出的 toast 自动消失已经在切片 93 接入真实 runtime timer；接下来需要处理 pane 鼠标拖动边框 resize，让 divider/resize handle 的拖动改变 pane 大小并保持 no terminal input leak。
...
- 当前任务队列下一步是切片 94：pane 鼠标拖动边框 resize。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
