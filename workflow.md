...
当前下一步：任务队列 0-91 均已完成。切片 91 的自动验收已通过，但默认 TUI 是否真正达到用户给出的目标截图风格，仍需用户在真实终端中人工复核；如果复核仍不通过，必须先在本文件新增下一轮视觉返工切片，再继续实现。
...
- 当前任务队列下一步是切片 91：目标截图级整体 UI 构图三轮重绘。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
