...
| 215H1. SK live/history 边界与 contract 收口 | 进行中 | `workflow.md`、`termx-core-v2/docs/architecture.md`、`termx-tui-v3/docs/architecture.md`、`internal/protocol/` 按需 | 明确 live surface 只服务实时 pane 投影，authoritative history 只服务 `HistoryWindow`；收口 latest/older、token/generation、cols、stale guard、resize/attach/reattach/full replace/alt-screen/exit 的边界；若 wire contract 仍缺少区分字段，先补 protocol/document/fake harness；禁止 live -> history 回填与 TUI local scrollback/reflow fallback |
| 215H2. SK core-v2 authoritative history MVP | 待开始 | `termx-core-v2/`、`internal/protocol/`、`termx-proto/`、`workflow.md` 按需 | 让 `HistoryTrack` / `HistoryWindow` 覆盖 latest replace、older prepend、exhausted、logical boundary、cols/token/generation stale guard；live surface 与 history 从同源事件分流但不互相回填；resize/attach/reattach/full replace 不创造 committed history，alt-screen 不写 primary history，process exit force commit；补齐 core-v2 harness 与协议适配证据 |
| 215H3. SK tui-v3 history binding MVP | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-tui-v3/render/`、`termx-cli/`、`workflow.md` 按需 | `HistoryStore + CopyModeStore` 按 active view 绑定 terminal id、request id、bound cols/rows 与 pending/error；`RenderVMBuilder` live/copy 严格分流，宽度变化只触发 invalidate/rebind authoritative window，不本地 reflow、不从 live surface fallback；补 integration/tmux/CLI harness，达到 history MVP 可验收基线 |
...
当前下一步：215PR 已完成，真实 CLI attach 已改成阻塞唤醒事件循环，拖窗 resize 的 latest-wins 与 owner 回弹链路已收口。任务队列现在切到 history MVP 主线，`215H1 live/history 边界与 contract 收口` 已进入进行中；完成后依次执行 215H2 core-v2 authoritative history MVP、215H3 tui-v3 history binding MVP。215D1 floating group commands 与 215F shortcut integration/tmux harness 顺延到 history MVP 后；215E clipboard paste/history overlay 继续保持阻塞，直到 215H1-215H3 达到可验收基线后再重启。继续保持 copy/history 不从 live surface、snapshot 或 local VTerm scrollback 推断历史，也不得把 styled payload parser 扩张成补丁式完整 VT emulator。
