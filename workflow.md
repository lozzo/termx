...
| 215A. SK copy/display 快捷键语义核验 | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/docs/`、`workflow.md` | 已核验 display/copy 的 `Home/End`、`g/G`、`u/d` 均走 authoritative `HistoryWindow` 上的 copy reducer；`Enter` 复制 selection 后退出 copy mode；补充 integration harness；`p/P PASTE`、`H HISTORY` 保持未实现并排入 215E；准入已通过 |
| 215B. SK view-local terminal layout state | 进行中 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/input/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 建立 view-local terminal layout domain，用于 size lock、content pan、content align、center/reset 和 resize mode layout toggle；状态必须挂在 pane/floating view binding 或明确 view-local store，不得写成全局 terminal truth；`s LOCK`、`Space LAYOUT`、`Shift+WASD/Shift+Arrow PAN`、`0/$/^/B ALIGN`、`m/|/_ CENTER`、`r RESET` 走统一 semantic command、render projector 和 harness |
...
- 当前正在进行切片 215B：215A 已核验 copy/display `Home/End`、`g/G`、`u/d` 并补齐 `Enter` 复制后退出 copy mode；下一步建立 view-local terminal layout state，再按 215C-215E 逐项补齐未实现能力。
