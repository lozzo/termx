...
- `R305` 已完成：真实 shared-vterm synchronized primary screen transaction 已接入 `HistoryTrackProjector`，只在 primary-screen decision + publish frame + sync end 的最小安全子集接管；repaint 替换 current frame，旧 primary frame 进入 archived frame journal，ordinary committed depth 不增长。新增 `TestTerminalSemanticProjectorPrimaryScreenSessionArchivesRepaintViaProjector` 覆盖 current projection、archived older 和无 raw fallback。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
