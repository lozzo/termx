...
- `215E1-R140 repeated stress RSS 高水位回收` 待开始：按用户实测的同一 terminal 多次执行压力脚本后 RSS 高水位不回落问题，补连续运行 harness，并定位落盘 history 之外的临时 decode/reflow/snapshot/transport/allocator 常驻来源；不得靠定时 scrub、丢 history truth、清 live fallback 或固定 FPS 掩盖。
