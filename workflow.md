...
| 215E1-R128. SK daemon compact clean/reencode 与 live/vterm alloc 热点 | 待开始 | `termx-core-v2/`、`termx-vterm/`、`workflow.md` | 基于 R127 profile，下一步不再处理已退下的 LoadLine clone churn；优先评估 dirty->clean commit 是否必须重写 compact payload，减少 `markLineClean -> saveCompactLine/encodeCompactLogicalLine` 的 inuse/alloc，同时审计 live/vterm alloc 热点如 `compactASCIIStyleRuns`、`scrollbackAppendOpsFromCharmVTDamages`。必须保持 logical-line truth、dirty/clean generation 与 observer COW 语义 |
