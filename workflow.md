...
- `R341` 已完成：新增 backend-backed HistoryStore 构造与 `LinePayloadBackend` 按 id 读取契约；store 应用 mutation 后可把 sealed logical-line payload spill 到 backend，并从热 `lines` map 移除 sealed payload。latest/older/copy/freeze 在 backend-backed 路径下使用 projection row refs + cursor window，只按窗口读取 payload，不再为了分页先 materialize 全量 rows；frozen token 保存 index refs，不复制整份历史 rows。新增 counting backend harness 覆盖 2 万行 sealed timeline，验证 latest/older/frozen latest 只读取 limit 级别 payload；新增最小 JSONL file payload backend 和窗口读取回归。Terminal 默认接入文件 backend、目录生命周期、恢复/compaction 和 current frame spill 留后续切片。准入已通过 `cd termx-core-v2 && go test ./... -count=1`、`git diff --check`。
