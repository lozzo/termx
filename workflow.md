...
| 101. 用户复核事项归档 | 待开始 | `workflow.md`、`termx-tui-v3/docs/`、`termx-tui-v3/`、`termx-cli/` 按需 | 归档用户本轮反馈处理结果：toast 去重/降噪、分屏图标有效性、无必要消息收敛、中文输入法与默认 cursor 隐藏的手工复核步骤和剩余边界；自动 smoke/e2e 只作为证据，不替代用户对真实 TTY 视觉和输入法行为的人工确认；准入按是否有代码改动运行对应测试，文档-only 至少运行 `git diff --check` |

当前下一步：切片 100 已完成，继续切片 101，归档本轮用户复核事项和手工验收边界。
