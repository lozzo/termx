...
| KS007 | 进行中 | 删除快捷键硬编码 fallback 并补齐提示 | 先补显式空 catalog 无 fallback harness；sticky/copy Esc、空 footer `Ctrl-G`、Help close、Prompt suggestion 回到 catalog；每个 action 在 domain spec 明确 footer/help/click 策略 |
...
- KS007 进行中：下一步删除 sticky/copy Esc、空 footer `Ctrl-G`、Help close、Prompt suggestion 等残余硬编码 fallback，并让每个 action 的 footer/help/click 策略完全由 domain spec 声明。
