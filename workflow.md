...
| R372. SK single SemanticTap 合同与 harness | 待开始 | `termx-core-v2/`、`termx-vterm/`、按需 `termx-core-v2/docs/*` | 定义唯一 vterm owner：顺序消费 PTY bytes 与 resize，维护 latest native screen，产出 immutable semantic transaction，并拥有 OSC/DA/DSR response 回写；用 harness 锁定当前双 vterm 链路问题、resize 顺序边界、response 只回写一次和 live 不丢 emulator 状态 |
