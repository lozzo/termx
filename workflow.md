...
| 21. CLI v3 attach/TUI 装配 | 进行中 | `termx-cli/`、`termx-tui-v3/` 按需 | `termx v3 attach <id>` 使用 tui-v3 `TerminalHost`、protocol adapters 和 `NewInteractiveRuntime` 装配真实交互路径；非交互环境给出明确错误或运行专用 harness；不得调用 `tuiv2/app.RunWithClient` |
...
当前下一步：继续切片 21“CLI v3 attach/TUI 装配”。
...
- 当前正在执行切片 21：`CLI v3 attach/TUI 装配`。
