...
当前下一步：切片 83 待开始。切片 82 已完成 overlay/toast/floating 第一轮视觉对齐：toast 使用右上角实体 card、severity accent、title/body 裁切、close action 和上下安全区；overlay 与 floating 使用 title/state/action 槽位、content padding、active/focus token、ANSI reset 和宽字符安全。切片 81 已完成 `tuiv2` 风格 pane chrome 与 split 视觉重绘。切片 80 已完成 `tuiv2` 风格 shell header/footer 重绘。切片 79 已完成视觉差距审计与验收基线。下一阶段推进真实默认 TUI 视觉验收。terminal live streaming/rich attributes、copy-history 最终 polish、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 79-83 内，必须另行新增切片后再做。
