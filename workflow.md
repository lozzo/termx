...
| 103. 中文输入法候选区不顶起页面复核 | 待开始 | `workflow.md`、`termx-tui-v3/terminalhost/`、`termx-tui-v3/render/`、`termx-tui-v3/app/`、只读参考 `tuiv2/` | 处理用户补充复现：`Ctrl-n` 后切中文输入法输入 `o`，候选或预编辑文字出现在整个窗口最下方并导致页面上抬；必须先复核真实 host/frame sink 的 cursor anchor、raw mode、alt screen、光标定位和输入路径，必要时只读参考 `tuiv2`；完成后 host cursor 应隐藏并稳定锚定当前真实输入 pane/floating/prompt 的全局位置，中文 IME 候选不得落到底部输出行；补充可自动验证的 ANSI/cursor harness，并把需要真实终端手工复核的步骤写入文档；准入至少运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 102 已完成。后续按用户要求先处理切片 103 的中文输入法候选区上抬问题，再处理切片 104 的 floating 获焦后 tiled pane 失焦视觉问题。
