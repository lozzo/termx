...
| 215E1-R126. SK daemon history clone churn 与 payload backend 量级优化 | 待开始 | `termx-core-v2/`、`scripts/`、`workflow.md` | 不再继续死磕每行 header；基于 R125 pprof，优先处理 daemon `MemoryStorageBackend.LoadLine -> LogicalLine.Clone -> cloneCells` 在 history ingest/commit 热路径的 GB 级 alloc churn，或设计分段/文件 backend 把 100k history payload 移出 daemon heap。必须用 100000 行真实 TUI stress 对比 RSS/CPU/copy-oldest，并证明不破坏 authoritative logical-line truth |
