...
- `R302` 已完成：`termx-vterm/vterm/semantic_source.go` 新增 `TerminalSemanticSource`、`TerminalSemanticTransaction`、`TerminalSemanticSize`、frame/scroll-out contract 和 `SemanticSource` adapter；`ApplyPTYWrite` / `Resize` 产出 ordered ops、primary scroll-out proof、primary/alt frame、alt enter/exit、synchronized output、full replace/resize boundary。`termx-core-v2/terminal_semantic_contract.go` 以 alias 消费 vterm contract，并把现有 semantic batch 投影成 transaction；`live.SurfaceTrack` 改为通过 `SemanticSource.ApplyPTYWrite` 生成当前真实 PTY damage。准入已通过 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-vterm && go test ./... -count=1`。
- `R303` 已完成：新增 `HistoryProjector`、`ScreenAppClassifier`、`ScreenAppDecision`、`HistoryMutation` 和 `HistoryTrackProjector` domain harness；fake `TerminalSemanticTransaction` 已覆盖 ordinary scroll-out commit、primary screen-frame latest projection、alt transient current frame、process exit force close。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R304` 已完成：ordinary stdout 的完整 logical line 经 shared-vterm ordered transaction 进入 `HistoryTrackProjector` 后直接 `force-commit-frontier`，真实 ingest 只在 plain ASCII + 硬行结束的安全子集接管，复杂 cursor/erase/screen app 仍留给后续切片；process exit 继续 force commit primary mutable frontier。相关协议/终端 harness 已按完整行 committed、open tail live-tail 的新边界更新。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R305` 已完成：真实 shared-vterm synchronized primary screen transaction 已接入 `HistoryTrackProjector`，只在 primary-screen decision + publish frame + sync end 的最小安全子集接管；普通 repaint 只替换 current frame，不把旧 primary frame 或 `/re`、`/res` 这类中间输入态写入 archived frame journal，ordinary committed depth 不增长。`TestTerminalSemanticProjectorPrimaryScreenSessionReplacesRepaintViaProjector` 覆盖 current projection、older 回 committed history 和无 raw fallback。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R306` 已完成：alt enter/running/exit frame 已接入 `HistoryTrackProjector`，projector 按 ordered alt mode 保持 PTY 顺序，进入 alt 前归档/隐藏 primary current，alt frame 仅作为 transient latest；退出 alt 后的新 primary frame 重新 publish，不复活 pre-alt current，也不把 alt 内容写入 primary history。新增 `TestTerminalSemanticProjectorAltScreenArchivesPrimaryBeforeTransient` 覆盖真实 server/live segmentation、archived older、latest-only alt frame 和无 raw fallback。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R307` 已完成：新增 `TestHistoryTrackResizeKeepsFinalScreenFrameFixedGrid` 锁定 resize 后 committed IDs 不变、ordinary logical line 仅按新 cols 重投影、final committed `screen-frame` 仍保持一条 fixed-grid row；扩展 `TestTerminalResizeDoesNotLetSynchronizedFrameDeleteCommittedHistory` 覆盖真实 server resize 后 running primary current frame 仍是 mutable `screen-frame` 且不增长 committed depth。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R308` 已完成：`history.ResolveCellStyle` / `ResolveHistoryColor` 定义 view-time 主题解析边界，default fg/bg 只在查看历史时按 `HistoryTheme` 解析，`ansi:N` / `idx:N` 在提供查看调色板时解析、否则保持 terminal SGR token，明确 RGB 作为内容属性保留；新增默认色 ordinary/frame semantic harness，确保 OSC 10/11 默认色不烘焙进历史 payload，显式 truecolor SGR 继续保存为内容 RGB。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
- `R309` 已完成：新增 `HistoryStorageBackend`、`HistoryStorageTransaction`、`HistoryStorageSnapshot`、内存/文件事务日志 backend 和 `HistoryTrack` storage snapshot 导出/恢复入口；文件 backend 只重放 domain transaction，恢复仍重建 `LogicalLineStore`、`CommittedHistoryIndex`、`MutableFrontier`、screen frame journal、screen owner 和 generation。新增 harness 覆盖 file reopen 后 append/update transaction 重放、final screen-frame 后 older 跳过被替换 repaint frame 并回到 committed shell history、以及 `ResidencyFile` 不等于 immutable。准入已通过 `cd termx-core-v2 && go test ./... -count=1`。
