...
| 36. render framework 最小渲染器 | 进行中 | `termx-tui-v3/render/`、`termx-tui-v3/terminalhost/` 按需 | 实现最小 render framework：viewport layout、header/footer 占位与隐藏、card panel、split line、最小双 pane 横向/纵向分割、panel chrome、content renderer dispatch、toast 层、Terminal Picker overlay/placeholder、hit region 合成、cursor 归属、最终 `RenderResult -> FrameSink` 适配；harness 覆盖宽窄屏、裁切、层级优先级、toast 不改变 body layout、opaque overlay cursor，以及 panel 标题/content/toast 中的 emoji、CJK、combining mark、ANSI styled text 不破坏边框、split line 或 row width |
...
当前下一步：继续切片 36，实现 render framework 最小渲染器；不得扩展到切片 37 的 app/input 快捷键接线。
...
- 实现前检查已更新：当前 renderer 仍把 active content 兼容投影成裸 frame；下一步必须按切片 36 实现最小 render framework，产出 workbench shell、header/footer、card/split panel、toast 和 Terminal Picker overlay/placeholder。
