...
- 当前最早未完成切片是 `R182. SK core-v2 daemon remote lifecycle`，状态为 `待开始`。
- 当前明确缺口：默认 `termx daemon` 仍未装配真实 remote config/service/start/close/local auto-enable 生命周期；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
