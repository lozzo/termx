...
- RM002 已完成：公开 Go 代码收口为根 module `github.com/lozzow/termx`，CLI、core、TUI、remote、shared、proto、vterm、testkit 与 internal 分别归位到 `cmd/termx`、`core`、`tui`、`remote`、`shared`、`proto`、`vterm`、`testkit` 与 `internal`；九个公开子 module、本地 replace、旧 `termx-*-vN` import/path 和 `go.work.sum` 已删除，六个私有部署 module 通过根 module replace 消费公开 contract。protobuf Go/TypeScript 生成物、Go workspace、Make 入口、public snapshot/dependency guard、license notice 与活动路径均已同步。清空 `TERMX*` 后的根公开全量测试（排除三个既有视觉基线）、六个私有 module 全量、相关公开/私有 race、非 TUI 全量 vet、CLI build、remote-ui 433 项测试/typecheck/build、移动 Web/Capacitor build、Android unit、public snapshot guard、notice 幂等和 `git diff --check` 通过。额外全公开 race 仍复现切片前 `vterm` response drain goroutine 与 emulator close/reset 的生命周期竞争；全量 vet 仍只有已记录的 `tui/state/history.go` self-assignment 与 `tui/app/runtime.go` unreachable code，本切片未把这些运行时问题混入目录迁移。
