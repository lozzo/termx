...
| 86. overlay/page/copy 视觉产品化 polish | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 把 Terminal Pool、Workbench Tree、Prompt/Help、copy-history 和 toast/floating 的内容层视觉从功能可见推进到产品级：实体 card padding、搜索行、selected row、detail/preview、action row、copy selection/search/status、toast severity 和宽字符内容层级统一；不要求背景灰度遮罩，不复制旧 `tuiv2` 实现；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 86 待开始。切片 85 已完成 pane chrome 目标截图级槽位重绘：pane 顶边不再只显示单一 `[x]`，而是按宽度显示状态点、`↔0`、`◆ owner`、full/compact action cluster，并保持 title 到 action 之间的连续横线。terminal live streaming/rich attributes、copy-history 深层语义、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 84-87 内，必须另行新增切片后再做。
