...
| 97. 用户复核回归与视觉验收归档 | 待开始 | `termx-tui-v3/`、`termx-cli/`、`termx-tui-v3/docs/`、`workflow.md` 按需 | 完成切片 92-96 后重新归档真实默认入口复核：按钮只在功能可用时出现，pane 横纵分屏、拖动 resize、floating 拖动移动、直角边框、toast 自动消失和截图式 toast 样式都必须在固定 smoke、e2e smoke 和手工步骤中可验证；仍不得把自动 smoke 当作用户人工拍板；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 97 待开始。用户最新复核指出的 pane 鼠标拖动 resize 已在切片 94 接入真实 SGR drag/release 和 runtime transient drag state；横纵分屏入口与可见按钮一致性已在切片 95 完成；floating 标题栏拖动移动和边框/角落连续 resize 已在切片 96 完成；下一步需要重新归档切片 92-96 后的真实复核和视觉验收状态。
...
- 当前任务队列下一步是切片 97：用户复核回归与视觉验收归档。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
