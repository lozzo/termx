...
- 当前快捷键迁移已完成到 215R2：floating overview/summon 已复用 reducer-owned floating state，右上角 toast 卡片已按临时试验屏蔽，退出重进会先从 core-v2 opaque storage 恢复 workbench snapshot；terminal size 权限、size lock 与 TUI control 投影已实现；随后按 215D1、215F 连续收敛非 history 快捷键。215E clipboard paste/history overlay 已阻塞后置，history MVP 今天不重启。
