...
当前下一步：切片 43 待开始。自动执行必须从切片 43 开始，建立 copy mode resize rebind 与 history window cols；不得跳到 Unicode 线框或默认入口验收。
...
- 切片 43 待开始：下一次自动推进必须处理 copy mode resize rebind 与 history window cols，不能先改 Unicode 线框或默认入口 smoke。
