...
| 215E1-R127. SK daemon history LoadLine clone churn 优化 | 待开始 | `termx-core-v2/`、`workflow.md` | 基于 R126 clean profile，不再压 compact header；处理 `MemoryStorageBackend.LoadLine -> LogicalLine.Clone -> cloneCells` 在 ingest/commit/window/frozen generation 判断里的 GB 级分配。优先设计只读 snapshot/可写 owned mutation 的 store 边界，必须保护 observer/frozen snapshot 语义，不能用返回共享 slice 给外部随便改的症状补丁 |
