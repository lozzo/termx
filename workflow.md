...
| 87. 默认入口截图级视觉验收 | 待开始 | `termx-tui-v3/`、`termx-cli/`、`termx-tui-v3/docs/`、`workflow.md` 按需 | 在切片 84-86 后重新验收 `go run ./termx-cli/cmd/termx`：常用 viewport 下 header/footer、pane chrome、active/inactive、toast、floating、Terminal Pool/Tree、Prompt/Help、copy mode 都必须接近用户截图目标；必须保留手工验收记录和必要 smoke/e2e visual assertions；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 87 待开始。切片 86 已完成 overlay/page/copy 内容层视觉产品化 polish：Terminal Picker、Terminal Pool、Workbench Tree、Prompt、Help 和 copy-history 不再以工程表格/占位作为主要视觉语言，而是统一 search affordance、selected row marker、detail/preview/context/input label、action row 和 copy search/match/scrollbar/status。切片 87 必须做真实默认入口截图级视觉验收，不能只凭 smoke 文本、Unicode 线框或 ANSI 颜色判定通过。terminal live streaming/rich attributes、copy-history 深层语义、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 84-87 内，必须另行新增切片后再做。
