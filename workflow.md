...
| 95. 横纵分屏入口与可见按钮一致 | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/input/`、`termx-tui-v3/render/`、`termx-cli/` 按需 | 默认 TUI 必须同时具备横向和纵向分屏入口：pane mode 快捷键、鼠标/命中区和后续 CLI mini command 必须统一调用 `PaneCommandSplit` 并明确 orientation；可见 split 按钮只有在对应命令真实可用时才绘制，点击按钮必须创建对应方向分屏；harness 覆盖 horizontal/vertical split、active pane 选择、content rect resize、copy rebind、smoke 可见入口和无假按钮；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 95 待开始。用户最新复核指出的 pane 鼠标拖动 resize 已在切片 94 接入真实 SGR drag/release 和 runtime transient drag state；接下来需要恢复横纵分屏入口与可见按钮一致性，确保可见按钮只在真实命令可用时绘制并能创建对应方向分屏。
...
- 当前任务队列下一步是切片 95：横纵分屏入口与可见按钮一致。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
