...
当前下一步：继续切片 37，完成 app/input 接线与交互入口；不得扩展到切片 38 的默认入口 UI smoke 验收。
...
- 实现前检查已更新：当前 shell semantic message 已存在，但 `Ctrl-f`、`Ctrl-v`、header/footer hide、card/split、toast close/clear 等默认交互入口尚未接到 input/runtime 主路径；下一步必须按切片 37 接线。
