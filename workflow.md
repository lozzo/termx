...
| 80. tuiv2 风格 shell header/footer 重绘 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在视觉基线明确后，把默认 header/footer 从当前稀疏信息条推进到 `tuiv2` 风格的整屏 top/bottom bar：workspace/tab strip、mode、快捷键 token、状态摘要、右侧 summary、背景填满、窄屏退化和 ANSI style 均可见；header/footer hide 仍真实回收空间；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 80 待开始。切片 79 已完成视觉差距审计与验收基线：新增 `termx-tui-v3/docs/visual-alignment-audit.md`，`termx v3 smoke` 新增固定 `120x40` 的 `visual-audit-current` case，并把 architecture、render-architecture、ui-interaction-spec 中“基础 styled chrome 等于视觉完成”的口径降级为“工程基线已落地，截图级视觉仍需返工”。切片 75-78 已完成产品壳、基础 styled renderer、terminal live 前推、copy-history 深化和 render 输入清理，但当前 `v3 smoke` / 默认 TUI 的视觉效果尚未达到用户截图目标。下一阶段按顺序推进 header/footer、pane chrome、overlay/toast/floating 和真实默认 TUI 视觉验收。terminal live streaming/rich attributes、copy-history 最终 polish、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 79-83 内，必须另行新增切片后再做。
...
- 当前任务队列已重新打开视觉对齐返工：切片 80-83 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
