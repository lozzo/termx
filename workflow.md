...
- 当前最早未完成切片是 `R188. SK remote backend docs checkpoint`，状态为 `待开始`。
- 当前明确缺口：remote 后端 core-v2 contract 已完成代码 smoke，仍需 R188 更新文档 checkpoint；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
