...
当前下一步：切片 95 进行中。用户最新复核指出的 pane 鼠标拖动 resize 已在切片 94 接入真实 SGR drag/release 和 runtime transient drag state；当前需要恢复横纵分屏入口与可见按钮一致性，确保可见按钮只在真实命令可用时绘制并能创建对应方向分屏。
