...
当前下一步：切片 74 待开始。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期、Terminal Picker 真实交互深化、Terminal Pool 数据源与 Picker 服务接线一期、Terminal Pool 管理页一期、Workbench Tree overlay 一期、Floating pane 一期和 Prompt / Help overlay 一期已完成。切片 74-78 继续按顺序推进：Tab/Workspace、TUI 产品壳总验收、terminal live 连接展示与交互前推、copy-history 深化和 render cleanup/performance。当前 goal 的完成口径是：除 terminal/copy 深层内容渲染仍可继续深化外，UI、界面和基础交互必须完整可操作；若 UI 产品壳总验收后仍有余力，再前推 terminal live 的连接展示和输入交互。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary 仍不在当前队列内，必须另行新增切片后再做。
...
- 当前下一阶段任务队列继续：切片 74 Tab / Workspace 产品入口一期。后续 UI 产品壳总验收之前必须继续按顺序完成 Tab/Workspace；若仍有余力，再前推 terminal live 连接展示与交互。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
