...
当前下一步：切片 42 待开始。自动执行必须从切片 42 开始，建立 active content rect 驱动 terminal resize；不得跳到 copy mode rebind、Unicode 线框或默认入口验收。
...
- 切片 42 待开始：下一次自动推进必须先处理 active content rect 驱动 terminal resize，不能先改 copy mode rebind、Unicode 线框或默认入口 smoke。
