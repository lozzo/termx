...
| 81. tuiv2 风格 pane chrome 与 split 视觉重绘 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 对齐 `tuiv2` pane chrome：square 细线连续边框、顶边 title/state/action 槽位、active accent、inactive muted、card/split 两种呈现、分割线连接、内容区裁切、宽字符安全和鼠标 hit region 一致；不得出现当前 smoke 中“看起来有线但视觉密度/槽位/颜色不像目标”的状态；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 81 待开始。切片 80 已完成 `tuiv2` 风格 shell header/footer 重绘：header/footer 从稀疏拼接文本条改为分段产品栏，status background 填满整行，workspace/tab/mode/action/active/summary 使用稳定 token，accent/muted/warning 语义可见，Unicode `│` 分隔，error/exited 关键状态优先保留，80 列常规 smoke 可见 mode-specific keys。切片 79 已完成视觉差距审计与验收基线。下一阶段按顺序推进 pane chrome、overlay/toast/floating 和真实默认 TUI 视觉验收。terminal live streaming/rich attributes、copy-history 最终 polish、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 79-83 内，必须另行新增切片后再做。
