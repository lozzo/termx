...
| 68. Terminal Picker 真实交互深化 | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/services/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在 Terminal Picker 一期 VM 成型后，接入真实过滤、键盘移动选择、确认 attach、new terminal/create action、preview/detail 的最小状态和服务边界；允许仍只基于当前 workspace panes，不得伪装完整 Terminal Pool；harness 覆盖 query 更新、selected row 移动、Enter attach/focus、Esc close、new terminal action feedback、no terminal input leak、overlay cursor/row action 宽字符安全；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 68 待开始。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期已完成，下一刀深化 Terminal Picker 的真实过滤、选择和 attach/create 交互。Terminal Pool、Workbench Tree、floating 完整交互、remote 迁移和旧依赖拆分不作为下一刀，必须继续按任务队列显式新增。
...
- 当前下一阶段任务队列继续：切片 68 Terminal Picker 真实交互深化。后续如继续推进 Terminal Pool、Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
