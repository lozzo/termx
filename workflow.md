...
- `R339` 已完成：把用户要求的 iTerm2 式 `/clear` 行为固化成 core-v2 回归。语义是：Codex primary screen app 内部 ED2 clear 后，live/current frame 显示 clear 后内容；history/copy 向上仍能看到 clear 前的 shell timeline 和 primary frame 内容，且不能因为 clear-time proof 把已 sealed shell tail 重复写入 authoritative history。实现上 vterm `TerminalSemanticScrollOut` 增加 clear-time proof 原 viewport row metadata；core frame reducer 按 current primary frame row ownership 过滤 ED2 proof，只把 Codex frame 拥有的 rows 写入 scrollable history。准入已通过 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-vterm && go test ./... -count=1`、`git diff --check`。
