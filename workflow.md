...
| 70. Terminal Pool 管理页一期 | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/services/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在 Terminal Pool 数据源与 picker 服务接线完成后，实现独立 Terminal Pool page/content 的第一版产品页：列表、搜索、selected row、detail/preview、attach/kill/edit action 的 VM 与 service/effect/result 边界；不得混入 Workbench Tree、floating 或 remote 管理；harness 覆盖页面打开/关闭、list loading/empty/error、键盘选择、鼠标 action、attach/kill/edit 反馈、宽字符安全、no terminal input leak 和不伪造 terminal lifecycle；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
...
当前下一步：切片 70 待开始。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期、Terminal Picker 真实交互深化和 Terminal Pool 数据源与 Picker 服务接线一期已完成；下一刀进入独立 Terminal Pool 管理页一期。切片 71-77 已补充为后续顺序：Workbench Tree、floating、Prompt/Help、Tab/Workspace、terminal-live 深化、copy-history 深化和 render cleanup/performance。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary 仍不在当前队列内，必须另行新增切片后再做。
...
- 当前下一阶段任务队列继续：切片 70 Terminal Pool 管理页一期。后续如继续推进 Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
