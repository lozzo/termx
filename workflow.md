...
- `termx-remote-v2/`，仅在 RP002、RP003、RP006 与 GA001 对应公开 contract、端到端授权、客户端接入和 WebRTC 被动质量观测切片触及；不承载私有云服务实现。
- `termx-app/` 与 `remote-ui/`，在 RP006 统一客户端 contract、LIC001 notice 分发、RP007 public snapshot 边界和 GA001A Android 质量观测对齐切片内触及；不得引入私有源码构建依赖或 legacy fallback。
...
| GA001A | 待开始 | Android managed WebRTC 质量观测对齐 | Android stats 使用同一窗口、隐私和异步成本语义；Community/official 构建边界不变，不自动改路 |
...
- RP002 已完成：`termx-proto/cloudpb` 建立带 Hello 版本协商的跨平台 companion/signaling DTO、CloudError taxonomy 和禁止 grant/private key/terminal/DataChannel/session token 的描述符守卫；`termx-shared/cloudcompanion` 提供 public domain interface、稳定错误、请求记录 fake 和有界 stream harness。`termx-remote-v2` client/daemon/WebRTC 已删除对 `termx-hub` 的 import、旧 HTTP session token、grant-in-signaling、terminal inventory/heartbeat 和 agent bearer token，CLI 删除旧环境变量 Hub agent，managed endpoint 在 RP006 接真实 companion IPC 前返回 `COMPANION_MISSING` 且不 fallback。`termx-proto`、`termx-shared`、`termx-remote-v2` 全量通过；CLI 新 companion/依赖守卫通过，清理当前 `TERMX_*` 环境并排除三个已记录的视觉断言后 `go test ./cmd/termx -count=1` 全量通过；未排除时仍只有旧 rename marker 与 footer 顺序/`PgUp` 对应的 `TestV3SmokeCommandIncludesVisualReviewCases`、`TestV3VisualSnapshotCommandPrintsFixedVisualFrame`、`TestV3TmuxVisualCompareCapturesTargetAndDiffArtifacts` 失败。真实 DataChannel DeviceIdentity/capability handshake 已由 RP003 接入。
