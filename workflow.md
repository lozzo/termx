...
| 2. core-v2 domain 骨架 | 进行中 | `termx-core-v2/` | 建立 `LogicalLine`、`LogicalLineStore`、`CommittedHistoryIndex`、`MutableFrontier`、`StorageBackend` 内存实现和基础 harness |
...
- 正在执行切片 2：core-v2 domain 骨架。
- 旧 `termx-core/` 与 `tuiv2/` 的历史修补进度不再作为当前主线状态；如需查阅只能通过 git 历史或只读参考。
- 下一步继续完成切片 2：建立 core-v2 logical line domain 内存骨架和基础 harness。
