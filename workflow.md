...
| R373. SK SemanticTap fan-out 接入 | 待开始 | `termx-core-v2/`、按需 `termx-vterm/` | 真实 terminal PTY 输出改为进入 single tap；live publisher 只发布 latest invalidation/snapshot，可合并 wakeup；history queue 接完整 semantic transaction，不再把 raw PTY text 交给第二个 `vterm.SemanticSource` replay；保留 history disabled live-only 模式边界 |
