...
- 当前最早未完成切片是 `R187. SK remote backend contract smoke`，状态为 `待开始`。
- 当前明确缺口：remote 后端 status/local/pair、terminal/storage/events、transport session 已分切片验证，仍需 R187 做 backend contract 总烟测和旧 fallback 不恢复守卫；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
