...
- 当前最早未完成切片是 `R183. SK remote config path 独立策略`，状态为 `待开始`。
- 当前明确缺口：remote config path 仍需要在 R183 明确默认 daemon 与 CLI remote 命令的统一策略；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
