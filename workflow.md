...
- 当前最早未完成切片是 `R185. SK remote terminal management/storage/events routing`，状态为 `待开始`。
- 当前明确缺口：remote runtime API 的 terminal management、storage 和 events 还需要 R185 的 core-v2 truth routing smoke；`remote-ui` terminal 路径仍包含 snapshot/scrollback copy/history 风险，需在 App 阶段迁到 logical-line history source。
