...
| 215C2. SK pane lock 快捷键对齐 | 待开始 | `termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 对齐 tuiv2 pane mode `s LOCK`：pane footer、keyboard binding、action catalog 和 reducer path 必须复用 215B 的 view-local layout command，不新增第二套 lock state；无 active terminal view 时给 reducer-owned toast，不透传 terminal |
| 215D. SK floating overview 与 summon | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 建立 floating overview overlay state 和 selection/open/close/summon domain，覆盖 floating `o OVERVIEW`、`1-9 SUMMON` 以及 overview `Up/Down`、`Enter OPEN`、`1-9 SUMMON`、`Esc BACK`；overview 必须是 reducer-owned state，不得只渲染 placeholder |
...
- 当前快捷键迁移已完成到 215C：overlay keyboard command router 已收敛到 `ActionSpec`、`ShellContentActionMsg` 和 reducer/effect/message；下一步按 215C1、215C2、215D、215D1、215F 连续收敛非 history 快捷键。215E clipboard paste/history overlay 已阻塞后置，history MVP 今天不重启。
