...
| 215P. SK 渲染效率快速优化 | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/terminalhost/`、`workflow.md` | 不改变 UI 设计架构、render framework、layout 或 styled chrome 语义；`AppRuntime` 保持事件驱动，不引入定时器轮询，且不按固定消息数强制产出中间帧，而是以真实 `WriteFrame` 完成作为可见帧边界：写帧期间积压的 reducer message 合并到下一帧，避免持续 live/input 流因只等队列清空而出现可见帧饥饿；terminal live resize/layout 尺寸短暂不一致时只裁切和空白填充，不显示 extent dots 或 chrome overflow 箭头；真实 `FrameSink` 缓存上一帧，首帧、尺寸或行数变化仍全清屏，同尺寸普通帧只写变化行，完全相同 frame 直接零写入，光标变化保留最小 cursor sequence；按性能采集将 renderer 热点 `contentViewportLineWindow` 从整行 segment 生成后裁切改为直接按 cell window 裁切，让 action spec 按 ID 查询复用 catalog 缓存但保持动态 chrome glyph，`FrameFromRenderResult` 不再重复 deep clone styled lines，`canvas.writeLine` 改为 streaming grapheme 写入避免整行 segment 临时切片，并让 canvas 零值 cell 表达空白；已补 sink diff/perf harness、runtime frame-boundary 合帧 harness、live resize 降噪 harness 与动态 glyph 缓存 harness，并记录 benchmark 对比 |
