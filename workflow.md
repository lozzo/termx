...
- `215E1-R51 terminal size lock chrome action` 已完成
- 左上 terminal 名称后现在固定展示 terminal Size lock 图标；点击 pane 或 floating 的图标都会通过 terminal metadata tags 切换 `termx.size_lock`，不会再误用 view-local layout lock
