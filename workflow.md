...
- `R188` 已完成：`termx-cli/docs/v2-v3-switch-audit.md` 已更新为 remote 后端迁移完成 checkpoint，记录 `termx remote ...` 只连 core-v2 daemon、status/local/pair 经 typed hook、runtime API terminal/storage/events 与 WebRTC/datachannel transport 经 `termx-remote.Service` 路由到 core-v2 truth、旧 fallback 不得恢复，并把后续 App/remote-ui history/copy 边界写为下一阶段。准入 `git diff --check` 已通过。
- 当前最早未完成切片是 `R189. SK App/remote-ui contract 准入设计`，状态为 `待开始`。
- 当前明确缺口：remote 后端 core-v2 contract 和文档 checkpoint 已完成；下一阶段要设计真实 `termx-app/` + `remote-ui/` 连接 CLI remote runtime、terminal protocol、history/copy 的 contract，尤其要移除 snapshot/scrollback copy/history truth 风险。
