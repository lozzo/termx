...
| 215E1-R138. SK history range/newer backend fetch | 待开始 | `termx-core-v2/`、`internal/protocol/`、`termx-proto/`、`termx-tui-v3/services/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`workflow.md` | 补 R137 未扩协议的完整虚拟滚动后半段：新增 authoritative frozen snapshot 按 logical index/range 或 around/newer window 拉取 contract，让 TUI 已回收窗口后仍能向下回滚到 newer 内容；copy selection 只保存 logical boundary/index，大范围复制按 index 从 core/file backend 拉 authoritative logical line，不要求 TUI 常驻所有滚过内容；必须更新协议 stale guard，不用 local scrollback 或固定帧率掩盖 |
| 215E1-R139. SK repeated stress RSS 高水位回收 | 待开始 | `scripts/`、`termx-core-v2/`、`termx-tui-v3/`、`termx-cli/`、`workflow.md` | 处理用户实测“同一 terminal 连续执行 100k stress 脚本约 10 次后 RSS 升到 120MB+ 且不回落”：harness 需连续执行压力脚本并逐轮记录 daemon/TUI RSS、CPU、memstats、heap、history file 大小和 live surface footprint；定位 file-backed history 落盘后仍常驻的临时 decode/reflow/snapshot/transport/allocator 高水位，在 terminal/request 边界释放或复用；不得靠定时 scrub、丢 history truth、清 live fallback 或固定 FPS 掩盖 |

当前下一步：

- `215E1-R138 history range/newer backend fetch` 待开始：补完整双向虚拟滚动和按 index/range 复制 contract，让 R137 已回收的本地窗口能从 core/backend 拉回 newer/around 内容；这需要 protocol/schema 支持，不在 R137 里用旧字段硬塞。
- `215E1-R137 copy/history 虚拟滚动窗口` 已完成：older 接收后裁剪 TUI 本地 history window，只释放已经滚过的新尾部并保留更老方向连续性；cursor/selection/query matches 重定位，frozen tail boundary stale guard 不变。新增连续 older harness 证明 rows/source lines 有界，不靠 fixed FPS 或 local scrollback。
- `215E1-R139 repeated stress RSS 高水位回收` 待开始：按用户实测的同一 terminal 多次执行压力脚本后 RSS 高水位不回落问题，补连续运行 harness，并定位落盘 history 之外的临时 decode/reflow/snapshot/transport/allocator 常驻来源；该切片排在 range/newer backend fetch 之后执行，不和 copy/history 虚拟滚动混在一起。
