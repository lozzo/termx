...
| 42. active content rect 驱动 terminal resize | 进行中 | `termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/services/`、`termx-cli/` 按需 | app 根据 layout plan 计算 active terminal content rect，并只用 content rect cols/rows 向 core-v2 terminal 发送 resize；外部 viewport、header/footer hide、card/split 切换和 split 变化导致 content rect 变化时触发 resize effect，重复尺寸不重复发送；attach 初始尺寸也以 content rect 为准；harness 覆盖 card 扣边框、split 分割、header/footer 隐藏回收空间、resize effect 去重和真实 protocol adapter 参数 |
...
当前下一步：切片 42 进行中。自动执行必须继续建立 active content rect 驱动 terminal resize；不得跳到 copy mode rebind、Unicode 线框或默认入口验收。
...
- 切片 42 进行中：本切片只处理 active content rect 驱动 terminal resize，不能先改 copy mode rebind、Unicode 线框或默认入口 smoke。
