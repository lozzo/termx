...
- `215E1-R132 TUI live 调度去固定帧率审计` 已完成：ordinary live refresh 继续由事件队列 drain、runtime coalescing、in-flight dirty 背压驱动，不使用固定 FPS / fixed interval；本轮新增 runtime render-pressure guard，当前返回的 ordinary surface 如果已经被同 terminal 的 dirty/queued successor 覆盖，就只接纳状态、不写中间帧，避免在压力期间按 batch 上限渲染即将过期的 live 画面；lifecycle/resize/attach/exit 等边界消息不进入 skip-render。100k artifact `/tmp/termx-r132-live-pressure-nofps-100k`：daemon copy-oldest 约 44.7MB、TUI stress 峰值约 39.4MB、TUI copy-oldest 约 51.8MB、stress `real 2.58 user 2.22 sys 0.23`、outside baseline `real 1.33 user 1.30 sys 0.02`、oldest 可见 `000000`；TUI `alloc_space` 约 1.357GB，与 R131 约 1.362GB 基本持平，说明 R129 已经收掉固定节拍/过量 live 拉取大头，下一步继续压应看 render/canvas、protocol row decode 和 copy-oldest history clone 高水位。
