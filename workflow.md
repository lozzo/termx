...
| 90. 用户真实截图拍板与后续返工入口 | 阻塞 | `workflow.md`、`termx-tui-v3/docs/`、按用户反馈决定的后续范围 | 等待用户在真实终端运行 `go run ./termx-cli/cmd/termx` 并提供截图或明确拍板：如果视觉已接近目标 `tuiv2` 截图，本切片可改为完成并结束当前视觉对齐 goal；如果仍不一致，必须把具体差距写入本文档并新增后续重绘切片。自动 smoke、e2e、PTY ANSI 捕获或 Codex 文本输出都不能单独解除本阻塞 |

当前下一步：切片 90 阻塞。切片 89 已证明默认入口真实 PTY 路径可输出二轮 styled chrome，但这仍不能替代用户对目标截图风格的人工拍板。后续自动执行不得越过切片 90；必须等待用户提供真实终端截图或明确视觉通过/不通过结论，再决定完成当前 goal 或新增后续重绘切片。
...
- 当前任务队列下一步是切片 90 阻塞：必须等待用户真实终端截图或明确拍板。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
