...
| 51. shell chrome / top bar / bottom bar 视觉对齐 | 进行中 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-cli/` 按需 | header/footer 从占位文本升级为 styled top/bottom bar：workspace/tab/mode/hint/status token、active tab/创建 token、右侧摘要、隐藏 header/footer 后状态仍可达；harness 覆盖 ANSI 背景填满整行、token 宽度安全、hide 后 body 回收、默认入口 smoke 包含 styled top/bottom bar |
...
- 当前下一步是切片 51：shell chrome / top bar / bottom bar 视觉对齐，把 header/footer 升级为 styled top/bottom bar，并覆盖 workspace/tab/mode/hint/status token、背景填满整行和 hide 后 body 回收。
