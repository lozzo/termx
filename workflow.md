...
当前下一步：切片 94 进行中。用户最新复核指出的 toast 自动消失已经在切片 93 接入真实 runtime timer；当前需要处理 pane 鼠标拖动边框 resize，让 divider/resize handle 的拖动改变 pane 大小并保持 no terminal input leak。
