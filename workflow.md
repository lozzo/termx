...
| 84. tuiv2 风格 shell bar 高密度重绘 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 把 top/bottom bar 从当前工程标签式 `ws:/tab:/active:` 信息条推进到目标截图风格：高密度 tab/workspace strip、active tab block、右侧 owner/status/action token、底部彩色快捷键 taxonomy、右侧 summary、背景填满、窄屏优先级退化和 ANSI style 均可见；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 84 待开始。切片 83 已完成真实默认 TUI 视觉复核与失败归档：当前 TUI 已具备 styled chrome renderer 和基本操作闭环，但用户确认它仍不像目标 `tuiv2` 截图，不能当作视觉验收通过。切片 84-87 继续按 shell bar、pane chrome、overlay/page/copy polish、最终截图级验收推进。terminal live streaming/rich attributes、copy-history 深层语义、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 84-87 内，必须另行新增切片后再做。
...
- 当前任务队列已重新打开截图级视觉返工：切片 84-87 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
