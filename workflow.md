...
| 93. toast 自动消失与真实生命周期 | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-cli/` 按需 | toast 必须能在真实 runtime 中按时间自动消失，而不仅是 reducer harness 支持 tick；新增 toast 时默认设置合理 auto-dismiss TTL，pending/progress 或错误 toast 可保留更久但必须有明确生命周期；runtime timer/effect 必须定期派发 tick 并触发重绘；鼠标 close current 和 clear all 仍可用；harness 覆盖真实 runtime 自动消失、pending/error 退化、重绘和 no terminal input leak；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 93 待开始。用户最新复核指出的可见假按钮、直角边框和截图式 toast 样式已在切片 92 收敛；仍需继续按切片 93-97 顺序处理 toast 真实自动消失、pane 边框拖动 resize、横纵分屏入口、floating 鼠标拖动移动/resize 和最终用户复核归档，不能跳过最早未完成切片。
...
- 当前任务队列下一步是切片 93：toast 自动消失与真实生命周期。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
