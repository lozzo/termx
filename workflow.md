...
| 78. render 兼容投影清理与性能基线 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-tui-v3/terminalhost/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 在主要内容和 overlay 页面稳定后，删除或重命名剩余旧 `RenderVM{Lines, Status}` 兼容语义，建立 content-level cache / layer dirty region / large terminal output 性能基线；不得改变 `RenderResult` 单一路径，不得重新引入裸文本 frame 主路径；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 78 待开始。terminal-live、copy-history、empty/exited/Terminal Picker 内容 renderer 一期、Terminal Picker 真实交互深化、Terminal Pool 数据源与 Picker 服务接线一期、Terminal Pool 管理页一期、Workbench Tree overlay 一期、Floating pane 一期、Prompt / Help overlay 一期、Tab / Workspace 产品入口一期、TUI 产品壳总验收、terminal live 连接展示与交互前推和 copy-history content renderer 深化已完成。下一切片只处理 render 兼容投影清理与性能基线，不混入 terminal live streaming/rich attributes、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary；这些事项必须另行新增切片后再做。
...
- 当前下一阶段任务队列继续：切片 78 render 兼容投影清理与性能基线。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
