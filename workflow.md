...
| 82. overlay/toast/floating 视觉对齐 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/app/`、`termx-cli/`、`termx-tui-v3/docs/` 按需 | 把 Terminal Picker、Terminal Pool、Workbench Tree、Prompt/Help、toast 和 floating 从“可见可操作”推进到目标视觉：实体 modal/card、现代 toast、合理 padding、title/action 槽位、层级遮挡、active/focus token、ANSI reset 和宽字符安全；不引入灰度遮罩作为必要项；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |
| 83. 真实默认 TUI 视觉验收 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/terminalhost/`、`termx-cli/`、`termx-tui-v3/docs/`、`workflow.md` 按需 | 在 header/footer、pane chrome、overlay/toast/floating 重绘后做真实默认入口视觉验收：`go run ./termx-cli/cmd/termx` 在常用 viewport 下必须接近用户截图目标，支持分屏、focus、resize、floating、Terminal Pool/Tree、Prompt/Help、copy mode 的视觉检查；补手工验收说明和必要 smoke/e2e visual assertions；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 82 待开始。切片 81 已完成 `tuiv2` 风格 pane chrome 与 split 视觉重绘：card panel 与 split line 均使用 square 细线、title/state/action 槽位、active accent、inactive muted、共享外框、split divider 连接点、content rect 裁切和宽字符安全；split line 的 terminal resize 与 copy rebind 已按新的 content rect cols/rows 验收。切片 80 已完成 `tuiv2` 风格 shell header/footer 重绘。切片 79 已完成视觉差距审计与验收基线。下一阶段按顺序推进 overlay/toast/floating 和真实默认 TUI 视觉验收。terminal live streaming/rich attributes、copy-history 最终 polish、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary不在切片 79-83 内，必须另行新增切片后再做。
...
- 当前任务队列已重新打开视觉对齐返工：切片 81-83 待开始。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
