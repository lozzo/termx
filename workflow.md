...
| 215E1-R142. SK floating/tiled history view isolation | 完成 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`workflow.md` | 已修复 floating 与 tiled history/copy 交互串扰：TUI state 现在按 TerminalView 保存独立 history/copy 会话，reducer 处理输入/async history response 前会装载对应 view 会话并写回；render 按 pane/floating view 投影自己的 copy history，不再把 active floating 的 copy 内容复用到最后 selected tiled pane；切 tab/workspace/floating deactivate 不会自动退出其它 view 的 history，只有该 view ESC、关闭/解绑/terminal remove 或 storage restore 会清理对应会话；新增 floating scroll 不影响 tiled、第二个 view 进入 copy 不退出第一个、ESC 只退出当前 view 的回归测试；准入 `cd termx-tui-v3 && go test ./... -count=1` 通过 |
| 215E1-R143. SK real-state stress memory profile | 完成 | `termx-tui-v3/render/`、`workflow.md` | 已先修掉 R142 后真实 CLI pane view id 与派生 `pane:<id>` 不一致导致的 copy/history 渲染错会话：pane copy 投影、entering live 和 inactive pane 判断现在优先使用 TerminalView binding 的真实 ViewID；100k clean artifact `/tmp/termx-r143-clean-fixed-100k` 通过 oldest marker，daemon copy-oldest 约 40.4MB、TUI copy-oldest 约 49.0MB、stress `real 2.61 user 2.26 sys 0.21`；100k real-state artifact `/tmp/termx-r143-realstate-100k` 显示用户默认 history 目录已有 19 个文件约 236.9MB，但新 terminal daemon copy-oldest 约 38.9MB、TUI copy-oldest 约 49.8MB，未复现 daemon 200MB+ / TUI 110MB；准入 `cd termx-tui-v3 && go test ./... -count=1` 通过 |
| 215E1-R144. SK floating/tiled history active view isolation | 完成 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`workflow.md` | 已补齐 R142/R143 后剩余真实场景：floating 的 history/copy 会话、entering live、鼠标选择、terminal pool target、attach/result target 和关闭清理都优先使用 TerminalViewStore 里的真实 ViewID，而不是派生 `floating:<id>`；floating 与 tiled 都可同时保持 history，互相滚动不串扰，第二个 view 进入 history 不会退出第一个，ESC 只退出当前 view；新增真实 floating ViewID 不等于派生 ID 的 render/app 回归测试；准入 `cd termx-tui-v3 && go test ./... -count=1` 与 `git diff --check` 通过 |
| 215E1-R145. SK stress residual heap and CPU pressure | 完成 | `termx-tui-v3/services/`、`termx-tui-v3/state/`、`scripts/`、`workflow.md` | 已按 100k stress + history oldest pprof 继续压 TUI history/copy 接收热点：HistoryStore 接纳带 SourceLines 的 protocol window 时直接接管 source/rows，避免 reducer 重复 clone/reflow；protocol history adapter 直接从 CompactRow 构造 HistoryWindow，plain history row 保持 text-only，不再先展开 protocol.Cell 再二次转换。最终 artifact `/tmp/termx-r145-compact-history-adapter-100k`：daemon copy-oldest 约 39.5MB、TUI copy-latest 约 37.3MB、TUI copy-oldest 约 42.4MB、stress `real 2.62 user 2.26 sys 0.21`、oldest 可见 `000000`；TUI historyWindowFromProtocol alloc 从 R143/R144 后约 32MB 降到约 9.2MB，总 alloc 约 358MB。剩余最大热点转向 live snapshot protocol decode 与 render ANSI/string builder，不继续死磕 history window 小 clone |
| 215E1-R146. SK compact live snapshot decode | 完成 | `internal/protocol/`、`termx-tui-v3/services/`、`workflow.md` | 已继续按 100k stress pprof 压 live snapshot decode 残余热点：协议层增加 compact snapshot decode，TUI live surface 直接消费 CompactRow，避免普通 live refresh 先展开成 `[][]protocol.Cell` 再合并成 LiveCell；100k artifact `/tmp/termx-r146-compact-live-snapshot-100k`：daemon copy-oldest 约 39.5MB、TUI copy-oldest 约 43.0MB、stress `real 2.61 user 2.27 sys 0.21`、oldest 可见 `000000`；TUI alloc_space 从 R145 约 357.8MB 降到约 211.1MB，live refresh effect alloc 从约 143.7MB 降到约 10.0MB，`CompactRowsToCells` 退出 TUI live refresh 热点。RSS 基本持平，后续继续看 render ANSI/string builder |
| 215E1-R147. SK render ANSI string builder pressure | 完成 | `termx-tui-v3/render/`、`termx-tui-v3/terminalhost/`、`workflow.md` | 已继续按 R146 100k pprof 压 TUI 剩余最大 alloc hotspot：safe ASCII terminal/chrome run 在 canvas 里保留整段，避免 ANSI 输出逐列重锚；ANSI 行和 FrameSink builder 加容量预估，变短行补空格直接写 builder。100k artifact `/tmp/termx-r147-render-ansi-builder-100k`：daemon copy-oldest 约 41.7MB、TUI copy-oldest 约 42.9MB、stress `real 2.58 user 2.29 sys 0.19`、oldest 可见 `000000`；TUI alloc_space 从 R146 约 211.1MB 降到约 119.2MB，`RenderANSI` 从约 84.2MB 降到约 43.6MB，`canvasRowANSIString` 从约 58.1MB 降到约 9.0MB，TUI stress CPU 时间从约 0.96s 降到约 0.60s。RSS 基本持平，后续继续看 reducer clone/state churn 与 history source 常驻 |
| 215E1-R148. SK shell defaults readonly clone pressure | 完成 | `termx-tui-v3/state/`、`termx-tui-v3/render/`、`termx-tui-v3/app/`、`workflow.md` | 已按 R147 100k pprof 压 TUI state clone churn：新增 `ShellStore.ReadonlyDefaults()`，render/product projection/app 只读目标查询复用已规范化 shell，不再每帧反复 clone workspace/tabs/panes；reducer mutation 路径仍保留 `EnsureDefaults()` 的 COW 边界。最终 artifact `/tmp/termx-r148-shell-readonly-app-100k`：daemon copy-oldest 约 40.2MB、TUI copy-oldest 约 42.4MB、stress `real 2.57 user 2.27 sys 0.20`、oldest 可见 `000000`；TUI alloc_space 从 R147 约 119.2MB 降到约 101.2MB，`ShellStore.EnsureDefaults` 从约 20MB 降到约 2.5MB；loaded line scroll benchmark 降到约 `2.1KB / 17 allocs` |
| 215E1-R149. SK canvas safe cell strip pressure | 完成 | `termx-tui-v3/render/`、`workflow.md` | 已继续按 R148 100k pprof 压 TUI render/canvas 剩余热点：`canvas.writeLine` 对 Safe 且无控制字符的 cell 直接使用已清理文本，不再逐 cell 调用 `ansi.Strip(SafeLine(...))`；误标 Safe 但含 ESC/newline/control 的文本仍走原 sanitize 路径。100k artifact `/tmp/termx-r149-canvas-safe-strip-100k`：daemon copy-oldest 约 40.9MB、TUI copy-oldest 约 42.7MB、stress `real 2.59 user 2.29 sys 0.20`、oldest 可见 `000000`；TUI alloc_space 从 R148 约 101.2MB 降到约 90.9MB，`RenderANSI` 从约 44.6MB 降到约 31.7MB，copy history render benchmark 从约 `337KB / 4828 allocs` 降到约 `270KB / 3492 allocs` |
| 215E1-R150. SK copy history local viewport cache | 完成 | `termx-tui-v3/app/`、`workflow.md` | 已处理真实历史滚动内存累加问题：TUI copy/history 已有 older 后本地裁剪窗口，本切片把 newer 方向也接入同一套 `trimCopyModeHistoryWindow`，连续向上/向下滚动都只保留 viewport 周边窗口；裁掉的内容仍按 frozen token、logical selection range 和 `HistoryCopyRange` 从 core/backend 取，不改 core-v2 logical-line truth、不从 live fallback，也不破坏主动 ESC 退出语义。新增回归证明连续 older/newer 后本地 rows 和 SourceLines 都保持不超过 `copyModeHistoryMaxRequestRows`。100k artifact `/tmp/termx-r150-history-window-cache-100k`：daemon copy-oldest 约 40.7MB、TUI copy-oldest 约 41.9MB、stress `real 2.57 user 2.27 sys 0.19`、oldest 可见 `000000`；TUI alloc_space 约 88.2MB，主收益是防止手工连续历史滚动累计窗口，不是一次 `g` oldest 的 RSS 大幅下降 |
| 215E1-R151. SK floating/tiled history focus projection regression | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/render/`、`workflow.md` | 已处理产品回归：floating 拿焦点时，最后 active tiled pane 不再复用 floating 的 active content，floating history 滚动不会投影到 tiled pane；`Ctrl-V` 在第二个 view 进入 history 时按进入后的真实 ViewID 写回会话，不覆盖第一个 view，多个 history/copy 会话都会保持到对应 view 主动 ESC 退出。新增 render/app 回归覆盖 floating history 不串到 tiled pane、第二个 view 进入 history 不覆盖第一会话；准入 `cd termx-tui-v3 && go test ./... -count=1` 通过 |
| 215E1-R152. SK history queue segmented ingest allocation | 完成 | `termx-core-v2/`、`workflow.md` | 已把 history queue worker 的批量输入从先 join 成单个大字符串改为分段传给 `terminalHistoryPipeline.IngestBatch`，parser pending、flush marker 和一次 generation publish 语义保持不变。100k artifact `/tmp/termx-r152-segmented-ingest-100k`：daemon copy-oldest 约 38.7MB、TUI copy-oldest 约 41.4MB、stress `real 2.56 user 2.27 sys 0.19`、oldest 可见；daemon alloc_space 从当前基线约 957.5MB 降到约 903.2MB，queue segmented benchmark 从约 836KB/op 降到约 460KB/op，`joinTerminalHistoryIngestBatch` 退出热点 |
| 215E1-R153. SK compact commit-state metadata fast path | 完成 | `termx-core-v2/`、`workflow.md` | 已给 `MemoryStorageBackend` 增加 lock 内 commit-state metadata 快路径，commit 扫描只读 seal/dirty，compact line 直接返回 clean sealed 状态，不为元数据判断解码 payload；seal/commit membership、screen ownership、frozen snapshot 和 logical-line truth 不变。microbench：queue segmented ingest 约 `879us` 降到约 `740us`，alloc 基本不变。100k artifact `/tmp/tx-r153`：daemon copy-oldest 约 39.0MB、TUI copy-oldest 约 42.5MB、stress `real 2.61 user 2.30 sys 0.20`，oldest/newest/done marker 均可追溯；这是小 CPU/alloc 收口，下一步转向 live snapshot 直出 compact rows |
| 215E1-R154. SK floating copy history view input isolation | 完成 | `termx-tui-v3/app/`、`workflow.md` | 已处理产品回归：copy/history 鼠标滚轮按命中的 TerminalView 写回会话，不再退回最后 active tiled pane；floating history 内滚动只移动 floating copy cursor，tiled session 保持不动。第二个 view 进入 history/copy 的既有多会话语义保持不变，只有对应 view 主动 ESC 或关闭/解绑才退出；准入 `cd termx-tui-v3 && go test ./... -count=1` 通过 |
| 215E1-R155. SK live snapshot 直出 compact rows | 完成 | `termx-core-v2/`、`termx-vterm/`、`internal/protocol/`、`termx-tui-v3/`、`workflow.md` | 已把 core live snapshot 编码侧从 `LiveSnapshot -> [][]vterm.Cell -> [][]protocol.Cell -> CompactRows` 改成只读访问 trimmed screen row 并直接产出 `CompactSnapshot`；同一个 binary payload 仍可被旧 `Snapshot` decoder 读取，TUI 继续消费 compact snapshot。100k artifact `/tmp/termx-r155-compact-snapshot-100k`：daemon stress 峰值约 37.3MB、copy-oldest 约 39.2MB，TUI stress 峰值约 34.3MB、copy-oldest 约 41.7MB，stress `real 2.56 user 2.26 sys 0.19`，oldest/newest/done marker 均可追溯；daemon alloc_space 从 R152/R153 约 903MB 级降到约 755.9MB，`liveCompactSnapshot` 只剩约 8.5MB。未改 history truth，未引入固定 FPS / fixed interval |
| 215E1-R156. SK history parser segment allocation pressure | 完成 | `termx-core-v2/`、`workflow.md` | 已把 history parser 的 flush/ASCII 文本段从单 cell slice 改成 inline segment，并在 pipeline 聚合时延迟构造 cells slice；混合 Unicode 仍回到 grapheme builder 路径，保持组合字符/宽字符语义。microbench：plain log 约 `1.096MB / 1075 allocs`，queue segmented 约 `430KB / 1070 allocs`；100k artifact `/tmp/termx-r156-history-parser-100k`：daemon stress 峰值约 37.4MB、copy-oldest 约 38.6MB，TUI copy-oldest 约 43.1MB，stress `real 2.72 user 2.29 sys 0.22`，oldest/newest/done marker 均可追溯；daemon alloc_space 从 R155 约 755.9MB 降到约 669.3MB，`historyANSIParser.Parse` 从约 171.0MB 降到约 76.5MB。未改 history truth，未引入固定 FPS / fixed interval |
| 215E1-R157. SK floating overlay repaint isolation | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/render/`、`workflow.md` | 已修复产品回归：copy/history 增量 patch 在内容区被 visible floating 覆盖时退回整帧，避免底层 pane 历史滚动把浮窗擦掉；floating 渲染会先覆盖自身矩形并清理背后内容，避免底层 pane 的 styled ANSI 背景透进浮窗。未改 copy/history truth 或多 view 会话语义；准入 `cd termx-tui-v3 && go test ./... -count=1`、`git diff --check` 通过 |
| 215E1-R158. SK current stress residual hotspot compression | 完成 | `scripts/`、`termx-core-v2/`、`termx-tui-v3/`、`workflow.md` | 已按用户指定真实命令重新跑当前 clean harness 并压缩残余热点：history parser 不再对 PTY CRLF chunk 先整块 `ReplaceAll`，而是在扫描时把 CRLF 当 LF 消费、保留裸 CR 行编辑语义；TUI chrome 单宽 box glyph 直接写 canvas segment，避免每个边框字符构造临时 `Line/[]Cell`。当前基线 `/tmp/termx-r158-current-100k`：daemon stress/copy-oldest 约 38.8MB/40.2MB，TUI stress/copy-oldest 约 34.5MB/42.6MB，stress `real 2.61 user 2.33 sys 0.19`。最终 `/tmp/termx-r158-final-100k`：daemon stress/copy-oldest 约 37.5MB/39.6MB，TUI stress/copy-oldest 约 34.5MB/42.7MB，stress `real 2.57 user 2.28 sys 0.19`，history file 约 30.7MB，copy-oldest 可追溯 `000000`；daemon `normalizeTerminalOutput/strings.ReplaceAll` 退出 top，TUI alloc_space 从约 98.4MB 降到约 92.2MB。未引入固定帧率、不丢 authoritative history truth、不靠 scrub/fallback |
| 215E1-R159. SK floating body default background | 完成 | `termx-tui-v3/render/`、`workflow.md` | 已修复产品回归：floating 主体不再用 `StyleOverlay` 铺色，改用默认背景空白覆盖底层 pane 的 ANSI 样式；浮窗边框/动作仍保留 active/accent 样式，terminal picker 等 overlay 不受影响。新增回归锁定浮窗空白区无 overlay 背景且不继承底层 ANSI 背景；准入 `cd termx-tui-v3 && go test ./... -count=1`、`git diff --check` 通过 |
...
| 215E1-R167. SK hidden floating whole pane | 完成 | `termx-tui-v3/state/`、`termx-tui-v3/render/`、`workflow.md` | 已收口 hide floating 语义：隐藏后整个 floating pane 不再进入布局计划，不渲染边框/chrome，不暴露 move/resize/action 命中区，也不会继续作为 active 输入目标；`z/m` 仍可从全 hidden 状态召回最高 Z floating |
| 215E1-R168. SK prefix mode command exit policy | 完成 | `termx-tui-v3/input/`、`termx-tui-v3/app/`、`workflow.md` | 已调整快捷键 prefix mode：普通命令触发后回到 normal 主菜单，不再要求手动 ESC；只有 pane resize/focus、floating move/size、tab/workspace next/previous、layout pan 这类连续调节动作保留当前 mode 并刷新 3 秒超时。准入 `cd termx-tui-v3 && go test ./... -count=1`、`git diff --check` 通过 |
| 215E1-R169. SK multi TUI owner and local floating geometry | 完成 | `termx-core-v2/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/cmd/termx/`、`workflow.md` | 已调整多 TUI 协作边界：每个 TUI runtime 使用独立 surface 身份，owner 判断按 surface+view；默认 attach 不抢 owner，显式 take-owner 才转移 resize owner；floating storage 只同步 slot/terminal 结构，不同步本地几何、大小、展示态和 z；workbench reload 保留各 TUI 的 owner/copy/history/active 本地交互态；prefix mode 连续动作超时改 1 秒。准入 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./cmd/termx -count=1`、`git diff --check` 通过 |
| 215E1-R170. SK owner identity surface panel guard | 完成 | `termx-core-v2/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`workflow.md` | 已把 core-v2 resize owner registry 提升为 daemon 全局 session+channel attachment identity，TUI owner 投影按 surface session + panel/view ID 匹配；多 TUI 同 panel 连接同 terminal 时只有全局 owner 对应客户端显示 owner，chrome `xN` 使用全局 attachment count。准入 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./cmd/termx -count=1`、`git diff --check` 通过 |
| 215E1-R171. SK attachment detach and owner projection cleanup | 完成 | `termx-core-v2/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/cmd/termx/`、`workflow.md` | 已收口真实回归：关闭 tab/pane/floating 会按 view attachment 调 core detach，client session 关闭会释放全局 attachment；owner chrome 只显示 core 确认的 surface+view owner，本地 pending owner 显示 owner?，避免多 TUI 同 terminal 双 owner；resize 失败不会被后续 attached lifecycle 洗掉错误。准入 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./cmd/termx -count=1`、`git diff --check` 通过 |
| 215E1-R172. SK startup attach duplication guard | 完成 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/cmd/termx/`、`workflow.md` | 已处理多 TUI 启动/重进时同一 view 被显式 attach 与 workbench restore 同时 attach 的回归：TUI attach 请求会先写入 view-local pending binding，storage restore 和输入路由都会把 pending attach 视为已占用；attach 成功后刷新 terminal pool 投影，避免 `xN` 长时间落到本地 fallback。准入 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./cmd/termx -count=1`、`git diff --check` 通过 |

当前下一步：

- `215E1-R168 prefix mode command exit policy` 已完成：普通快捷命令执行后退出 prefix mode 回到主菜单，连续调节类动作才保留并刷新超时。
- `215E1-R171 attachment detach and owner projection cleanup` 已完成：关闭 pane/tab/floating 和 client session 退出都会释放 core attachment；owner chrome 只展示 core 确认 owner，pending 本地 owner 显示 owner?；resize 失败错误不再被 lifecycle attached 投影清掉。
- `215E1-R170 owner identity surface panel guard` 已完成：owner 权限按 daemon 全局 session+attachment 裁决，TUI 按 surface+view 投影；多 TUI 同 panel 不再同时显示 owner，terminal chrome 使用全局 attachment count。
- `215E1-R169 multi TUI owner and local floating geometry` 已完成：多 TUI 共享 workbench 结构但保留各自 owner/交互态/floating 几何/大小/展示态；默认 attach 不抢 owner，显式 take-owner 才转移 resize owner；prefix mode 连续动作超时改 1 秒。
- `215E1-R167 hidden floating whole pane` 已完成：hide floating 会隐藏整个 pane，包括边框、chrome action、move/resize 命中区和 active 输入目标；`z/m` 仍可从全 hidden 状态召回最高 Z floating。
...
- `215E1-R158 current stress residual hotspot compression` 已完成：当前 clean 100k 真实路径 daemon/TUI RSS 已稳定在约 37-43MB 区间；本切片去掉 daemon CRLF 整块 normalize 分配，并让 TUI chrome 单宽 glyph 直写 canvas，stress 时间约 `2.61s -> 2.57s`，TUI alloc_space 约 `98.4MB -> 92.2MB`。继续压性能应看 daemon `HistoryTrack.commitFrontier/markLineClean/cloneCells` 与 TUI `historySourceLinesFromProtocol/copyHistoryRowCells`，不是固定帧率或丢 history。
- `215E1-R159 floating body default background` 已完成：floating 仍会先清掉背后 pane 的 ANSI 样式，但使用默认背景空白，不再给主体加 `StyleOverlay` 背景色；已补 render 回归。
...
- `215E1-R164 copy history exit release boundary` 已完成：补证据确认 copy/history 本地窗口 trim 会重新分配保留窗口，ESC 主动退出或关闭 pane 后 current root、view session map 和 copy-history patch cache 都断开 rows/source lines 引用。
...
- `215E1-R152 history queue segmented ingest allocation` 已完成：history queue 不再把 batch 先 join 成一个大字符串，而是分段进入同一个 pipeline ingest；100k daemon alloc_space 降到约 903.2MB，copy-oldest daemon/TUI 约 38.7MB/41.4MB。目标仍未完成，下一步继续看 daemon parser flush/string builder、live snapshot trim 与 TUI history/render 残余热点。
- `215E1-R150 copy history local viewport cache` 已完成：连续 older/newer 历史滚动都会裁剪 TUI 本地窗口，rows/source lines 不再随着经过的历史页无界累加；复制跨裁剪窗口继续按 logical range 回 core/backend 拉取。
- `215E1-R151 floating/tiled history focus projection regression` 已完成：floating 拥有焦点时 tiled pane 不再使用 floating active content；`Ctrl-V` 在第二个 view 进入 copy/history 会写回第二个 view 的会话，不覆盖第一个 view，ESC 仍只退出当前 active view。
- `215E1-R148 shell defaults readonly clone pressure` 已完成：render/product projection/app 只读查询改用 `ReadonlyDefaults()`，100k TUI alloc_space 降到约 101.2MB，`ShellStore.EnsureDefaults` 只剩约 2.5MB；RSS 仍约 42MB，后续若继续压应看 render chrome/footer/canvas 和 history source 常驻，而不是默认化 clone。
- `215E1-R147 render ANSI string builder pressure` 已完成：canvas/ANSI 输出不再把 safe ASCII run 拆成逐列重锚，TUI alloc_space 降到约 119.2MB，stress 阶段 TUI CPU 时间约 0.60s；RSS 仍约 43MB，后续要继续看 reducer clone/state churn 和 history source 常驻。
- `215E1-R146 compact live snapshot decode` 已完成：compact live snapshot decode 让 TUI live refresh 不再经过 `CompactRowsToCells` 展开，100k TUI alloc_space 降到约 211.1MB；RSS 仍在约 43MB，后续优先看 render ANSI/string builder，而不是 live protocol decode。
- `215E1-R143 real-state stress memory profile` 已完成：修复 pane copy/history 投影使用派生 view id 导致的真实 oldest 渲染回归，并复测 clean 与 `--use-real-state` 100k；real-state 默认 history 目录虽有约 236.9MB 落盘历史，但新 terminal RSS 仍保持 daemon 约 38.9MB、TUI copy-oldest 约 49.8MB，当前未复现用户报告的 200MB+/110MB。
- `215E1-R144 floating/tiled history active view isolation` 已完成：floating history/copy 路径全部按真实 TerminalView ID 查会话和清理，补真实 bound floating ViewID 的 render/app 回归；floating/tiled 历史滚动互不影响，多 view 可同时保持 history，ESC 只退出当前 view。
- `215E1-R145 stress residual heap and CPU pressure` 已完成：TUI history/copy window 接收路径去掉 reducer 重复 clone/reflow，并让 protocol history adapter 直接消费 CompactRow；100k artifact `/tmp/termx-r145-compact-history-adapter-100k` 显示 TUI copy-oldest 约 42.4MB、daemon copy-oldest 约 39.5MB、stress `real 2.62 user 2.26 sys 0.21`、oldest 可见 `000000`。后续若继续压性能，优先看 live snapshot decode/render，而不是 history window clone。
- `215E1-R142 floating/tiled history view isolation` 已完成：history/copy 会话按 TerminalView 隔离，floating 和 tiled 的进入、滚动、渲染、async response 和 ESC 退出互不串扰；多个 view 可同时保持 history，准入 `cd termx-tui-v3 && go test ./... -count=1` 通过。
