...
| 215P. SK 渲染效率快速优化 | 完成 | `termx-tui-v3/app/`、`termx-tui-v3/terminalhost/`、`workflow.md` | 不改变 UI 设计架构、render framework、layout 或 styled chrome 语义；`AppRuntime` 保持事件驱动，不引入定时器轮询，但单次 `Drain()` 内连续 reducer message 只在队列清空后渲染一次，避免高频 live/input/resize 事件被放大成多次同步 render；真实 `FrameSink` 缓存上一帧，首帧、尺寸或行数变化仍全清屏，同尺寸普通帧只写变化行，完全相同 frame 直接零写入，光标变化保留最小 cursor sequence；已补 sink diff/perf harness 与 runtime coalesced render harness |
