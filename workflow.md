...
| 104. floating 获焦时 tiled pane 失焦视觉 | 待开始 | `workflow.md`、`termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-cli/` 按需 | 处理用户补充反馈：floating window 获取焦点后，后面的 tiled pane 不应继续显示 active 高亮边框，而应显示 inactive 灰色边框；需要统一 shell active focus truth，保证 top floating active 时 pane chrome、footer/header active 摘要和 hit region 仍一致；harness 覆盖 floating raise/move/content focus 后 tiled pane active border token 降级为 muted，关闭或失焦 floating 后 pane active 视觉恢复；准入至少运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |

当前下一步：切片 103 已完成。后续按用户要求处理切片 104 的 floating 获焦后 tiled pane 失焦视觉问题。
