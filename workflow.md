...
- 已修正 core authoritative `history.window` 过滤缺 stable logical line id rows 后的 logical total：非权威 committed 片段会从 logical total / window logical total 扣除，并避免与 discontiguous stable id 过滤重复扣减。
