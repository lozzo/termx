...
- 用户要求连续完成剩余六个切片，固定顺序为：`QR002` 协议能力 -> `WEBUX001` Web 使用入口 -> `APPUX001` Android 使用入口 -> `UXE2E001` 产品体验总验收 -> `PG004` 公网可靠性与数据恢复 -> `CLOUDP007` Development 全产品业务验收。前四个体验切片现已完成，当前唯一主线恢复为 `PG004`；不得在 PG004/CLOUDP007 中重新设计已经验收的 UI 或 pairing 协议。
...
- `APPUX001` 已完成 Android 首次使用与设备信息架构：未登录首屏并列提供 Cloud 登录和本地设备添加；扫码与 `MXP1`/分享链接手工输入同屏，相机拒绝后保持手工回退；设备列表只展示友好身份，技术 ID 进入详情；terminal/file 剩余文案全部进入英文/简体中文 locale。共享 App UI 405 个 locale key 对称，Client 142 条、Mobile 29 条测试和 standard/devcloud APK 门禁通过；ARM64/API 35 模拟器已覆盖中英文、150% 字体、竖横屏、无摄像头回退、三种 Route 主入口和 crash scan。证据见 `docs/remote-platform/appux001-android-first-use.md`。
...
- `PG004` 公网 bootstrap staging 已部署：155 使用 systemd 运行 Controller + US West Edge，114 使用 systemd 运行 China East Edge，真实 Supabase `muxvia_staging` schema、Cloudflare DNS、Let's Encrypt、Web 登录/二维码、operator 双 fleet 和三进程重启恢复均通过。Android ARM64/API 35 公网 HTTPS profile 已完成 Web 扫码批准、Go-owned managed P2P、Relay 双端租约、Relay terminal attach/input 和远端文件浏览；daemon enrollment session 已补 refresh token 轮换。多用户 enrollment 子任务已完成并部署，但当前 enrollment 仍把新 daemon 固定到 Controller 配置中的第一个 Hub，无法把公网双 Hub 作为真实注册候选。当前追加 `PG004-HUBSEL` 子任务：Controller 返回有界健康候选，daemon Go CLI 对候选执行有界并发 HTTPS health 延迟探测并提交 Proto 观测，Controller 结合可达性、延迟和当前 assignment 数最终选择并原子写入唯一 assignment；Companion 必须使用 Controller 返回的动态 Hub 目录，不再要求 enrollment Hub 等于启动 manifest 的固定 Hub。该子任务只处理首次 enrollment，不建设自动迁移、无界 Hub 并发探测、Relay Mesh 或通用调度平台；已有 daemon 换 Hub 继续走 assignment migration/epoch fencing。R2 独立恢复和文件上传/下载/取消/摘要校验仍未通过，且长时间空闲 managed P2P 的半开 application session 已记录为后续弱网可靠性项，因此 PG004 继续保持进行中；证据见 `docs/remote-platform/pg004-public-bootstrap-deployment.md`。
...
| PG004 | 进行中 | Supabase staging、公网 bootstrap 装配与备份恢复验收 | 当前先完成 `PG004-HUBSEL`：Proto-first Hub 候选/观测、Controller 有界候选和最终 assignment、Go CLI HTTPS health 延迟探测、Companion 动态 Hub 目录以及双 Hub enrollment 进程测试；Controller 最多返回 100 个候选，Go 端固定最多 16 个并发 worker，最终归属不得由客户端自行决定。随后继续 R2 加密上传/独立恢复以及文件上传/下载/取消/摘要校验；长时间空闲 managed P2P 半开 session 作为弱网可靠性 finding 随文件验收继续定位。测试支付必须明确标记 staging，不得宣称正式商业生产 |
