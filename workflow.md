...
| 89. 真实默认入口截图级验收 | 待开始 | `termx-tui-v3/`、`termx-cli/`、`termx-tui-v3/docs/`、`workflow.md` 按需 | 切片 88 完成后必须重新验收真实 `go run ./termx-cli/cmd/termx`：在常用 viewport 下对照用户截图检查 header/footer、pane chrome、active/inactive、toast、floating、Terminal Pool/Tree、Prompt/Help、copy mode；自动 smoke 只能作为辅助证据，不能替代真实 TTY 视觉复核；若仍不一致，本切片必须继续归档差距并新增后续重绘切片，不能标记本 goal 完成；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 89 待开始。切片 88 已完成二轮 shell/pane 视觉重绘并通过自动准入；切片 89 必须做真实默认入口截图级验收，不能只凭 smoke 文本、Unicode 线框或 ANSI 颜色判定通过。如果真实 TTY 仍不像目标截图，切片 89 必须继续归档差距并新增后续重绘切片，不得把本 goal 标记完成。
...
- 当前任务队列下一步是切片 89：必须做真实默认入口截图级验收。remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
