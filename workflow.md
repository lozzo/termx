...
| 215E1-R137. SK copy/history 虚拟滚动窗口 | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-core-v2/`、`internal/protocol/`、`workflow.md` | 处理用户实测“进入历史模式持续向前滚动时 TUI 内存累加”：copy/history 不能无限保留滚过的行，TUI 只持有可见窗口与少量预取，cursor/selection 记录 logical index/boundary；滚动结束后的旧窗口内容应可回收，复制模式按 index 从 core/backend 拉取 authoritative logical line，不用 local scrollback 或固定帧率掩盖 |
| 215E1-R138. SK repeated stress RSS 高水位回收 | 待开始 | `scripts/`、`termx-core-v2/`、`termx-tui-v3/`、`termx-cli/`、`workflow.md` | 处理用户实测“同一 terminal 连续执行 100k stress 脚本约 10 次后 RSS 升到 120MB+ 且不回落”：harness 需连续执行压力脚本并逐轮记录 daemon/TUI RSS、CPU、memstats、heap、history file 大小和 live surface footprint；定位 file-backed history 落盘后仍常驻的临时 decode/reflow/snapshot/transport/allocator 高水位，在 terminal/request 边界释放或复用；不得靠定时 scrub、丢 history truth、清 live fallback 或固定 FPS 掩盖 |

当前下一步：

- `215E1-R137 copy/history 虚拟滚动窗口` 待开始：按用户实测的持续历史滚动内存累积问题，收口 TUI history/copy store 边界，让 TUI 只持有可见窗口与少量预取；cursor/selection 保存 logical index/boundary，复制按 index 从 core/backend 拉 authoritative logical line，不靠 local scrollback、固定帧率或清缓存症状补丁。
- `215E1-R138 repeated stress RSS 高水位回收` 待开始：按用户实测的同一 terminal 多次执行压力脚本后 RSS 高水位不回落问题，补连续运行 harness，并定位落盘 history 之外的临时 decode/reflow/snapshot/transport/allocator 常驻来源；该切片排在 R137 之后执行，不和 copy/history 虚拟滚动混在一起。
