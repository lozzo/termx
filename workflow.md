...
- `R340` 已完成：用户明确要求 `/clear` 是“新建一页”，不是“撕掉前面的页”。core-v2 不再把 ED3/clear-scrollback 当作物理清空 authoritative history；renderer 遇到 clear-scrollback 只发布 soft boundary mutation，不重置 stream/frame reducer，store 只清 frozen window token 并 bump generation，不删除 logical lines、timeline、open line 或 frame journal。新增回归覆盖单独 ED3 和 Codex primary frame 内 `ED2+ED3` clear 后仍可向上看到旧 shell/page 与 clear 前 Codex frame。准入已通过 `cd termx-core-v2 && go test ./... -count=1`、`cd termx-vterm && go test ./... -count=1`、`git diff --check`。
