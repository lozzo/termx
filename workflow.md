...
| 100. 默认隐藏 host cursor 与中文输入法处理 | 待开始 | `termx-tui-v3/terminalhost/`、`termx-tui-v3/render/`、`termx-tui-v3/app/`、只读参考 `tuiv2/` | 针对中文输入法敲击字母时预编辑文本出现在窗口底部并顶起界面的问题，只读复核 `tuiv2` cursor 处理经验；真实 TUI 进入后默认隐藏 host cursor，FrameSink 在无明确 overlay/prompt/live cursor 归属时持续输出 hide cursor，仅在需要文本输入或真实 live cursor 时定位显示；harness 覆盖 enter/exit cursor ANSI、frame 无 cursor 默认隐藏、overlay cursor 可见、中文/UTF-8 输入不因 host cursor 停在底部破坏布局；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`git diff --check` |
...
当前下一步：切片 99 已完成，继续切片 100，处理默认隐藏 host cursor 与中文输入法预编辑位置问题；切片 101 不得在切片 100 未完成前扩展实现。
