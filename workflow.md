...
| 215A. SK copy/display 快捷键语义核验 | 进行中 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 逐项核验 display/copy 的 `Home/End`、`g/G`、`u/d`、`Enter copy 后退出` 是否已走 authoritative `HistoryWindow` 与现有 copy reducer；缺口只通过 copy domain reducer、input catalog、ActionSpec 和 harness 补齐，不从 live surface 或本地 scrollback fallback；更新 keybindings 状态 |
| 215B. SK view-local terminal layout state | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/input/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 建立 view-local terminal layout domain，用于 size lock、content pan、content align、center/reset 和 resize mode layout toggle；状态必须挂在 pane/floating view binding 或明确 view-local store，不得写成全局 terminal truth；`s LOCK`、`Space LAYOUT`、`Shift+WASD/Shift+Arrow PAN`、`0/$/^/B ALIGN`、`m/|/_ CENTER`、`r RESET` 走统一 semantic command、render projector 和 harness |
| 215C. SK overlay keyboard command router | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 收敛 Terminal Picker、Terminal Pool、Workbench Tree 的键盘动作路由：picker `Tab SPLIT`、`Ctrl-E EDIT`、`Ctrl-K KILL`、`Ctrl-X DELETE`；pool `Ctrl-T TAB`、`Ctrl-O FLOAT`、`Ctrl-E EDIT`、`Ctrl-K KILL`、`Ctrl-X DELETE`；workbench tree `Ctrl-N/R/X/D/Z`；必须复用 overlay item selection、ActionSpec 和 reducer/effect/message，不新增字符串旁路 |
| 215D. SK floating group 与 overview domain | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 建立 floating overview 和 group command domain，覆盖 `o OVERVIEW`、`1-9 SUMMON`、`v ALL`、`= FIT`、`s AUTO-FIT`、overview selection/open/show-all/collapse-all/close/summon；overview 必须是 reducer-owned overlay state，floating group collapse/fit/auto-fit 不得只作为 render 标志 |
| 215E. SK clipboard paste 与 history overlay | 待开始 | `termx-tui-v3/state/`、`termx-tui-v3/input/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-tui-v3/render/`、`termx-tui-v3/docs/` | 建立 clipboard provider/history service 边界和 reducer-owned clipboard history overlay，覆盖 display `p/P PASTE`、`H HISTORY` 与 clipboard history 的移动、过滤、paste/new/edit/delete/close；paste 只能通过 effect/message 写入 active view input channel，不得让 renderer 或 service 直接改状态 |
...
当前下一步：执行切片 215A，先核验 display/copy 已有语义与 `termx-tui-v3/docs/keybindings.md` 的待复核项，再按 copy domain reducer 补齐缺口。后续未实现项按 215B-215E 顺序推进，先建 reducer-owned domain model 和 harness，再接 input/action/footer/render；历史 VT 语义路由方案暂不进入当前自动执行队列，等 TerminalView/Attachment 主线完成后再单独讨论和重新落档。继续保持 copy/history 只消费 authoritative `HistoryWindow`，不得从 live surface、snapshot 或 local VTerm scrollback 推断历史，也不得把 styled payload parser 扩张成补丁式完整 VT emulator。
...
- 当前正在进行切片 215A：已把 `termx-tui-v3/docs/keybindings.md` 的“部分实现或未实现”拆为 copy/display 核验、view-local terminal layout、overlay keyboard router、floating overview/group、clipboard paste/history 五个小阶段；下一步先处理 display/copy 待复核项，再按 215B-215E 逐项补齐未实现能力。
