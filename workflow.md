...
| 12. core-v2 server API 与 daemon 骨架 | 进行中 | `termx-core-v2/`、`internal/protocol/`、`termx-cli/` 按需 | core-v2 提供独立 server/daemon API、options、listen/shutdown、terminal registry、事件订阅 fake harness；不调用旧 `termx-core.NewServer` |
...
当前下一步：继续切片 12“core-v2 server API 与 daemon 骨架”。
...
- 当前正在执行切片 12“core-v2 server API 与 daemon 骨架”。
