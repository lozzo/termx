...
- `R354` 已完成：按用户要求把 live invalidation 从旧持续 stream/伪消息链路完整重构为 one-shot arm。protocol 新增 `live.invalidation.next` 请求参数 `RenderedRevision`，core-v2 `Server.NextLiveInvalidation` 只在当前 native screen revision 超过 TUI 已渲染 revision 时返回一次，否则等待下一次超过该 revision 的 invalidation；不会推送 screen payload，也不会暴露持续事件流。tui-v3 删除 `LiveFrameRenderedMsg` 和 `liveStreamEffect`，runtime 改成通用 `AfterRenderFunc` hook，live 模块只在完整 frame 写入 FrameSink 后、且没有 pending fetch 时 arm 一次；arm 返回后才触发一次 `live.screen.get` 拉 latest。service adapter 删除 live union stream，`ArmLiveInvalidation` 只接受 `terminal.live.invalidated`，metadata/lifecycle 不再混入 live wake path。准入已通过 `go test ./internal/protocol/... -count=1`、`cd termx-core-v2 && go test ./... -count=1 -timeout=60s`、`cd termx-tui-v3 && go test ./... -count=1 -timeout=60s`。
