...
当前下一步：切片 62 待开始。header/footer 产品化已完成；下一阶段继续补真实鼠标 hit region dispatch 与 pane focus，再做 active pane 视觉反馈，之后再接 terminal-live content renderer；Terminal Pool、Workbench Tree、floating 完整交互、remote 迁移和旧依赖拆分不作为下一刀，必须继续按任务队列显式新增。
...
- 当前下一阶段任务队列继续：切片 62-64 依次推进鼠标 hit region dispatch、active pane 视觉反馈和 terminal-live content renderer 一期。后续如继续推进 Terminal Pool、Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
