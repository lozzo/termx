...
- 下一步继续 core 显式 logical line store：把 persisted store / mutable live tail / screen projection 三层的 logical line 记录结构进一步收敛，补齐 residency、dirty、generation 的落盘 metadata；不回退修补旧 TUI snapshot/grid viewport 滚动路径。
