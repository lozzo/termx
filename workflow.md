...
| R201BU. SK TUI Codex primary update card 帧保留 | 待开始 | `termx-core-v2/`、`termx-tui-v3/`、按需 `termx-vterm/`、`workflow.md` | 根据用户在 TUI 测试中的截图复现并修复 Codex primary-screen 首屏 update available card 在后续当前帧/history 视图中消失的问题；必须先定位 core-v2 HistoryWindow mutable current frame、styled blank footprint、scroll-region/RI 或 tui-v3 render/clipping 的权威边界，补 harness 后再实现，不得用局部补丁或 snapshot fallback |
...
- `R201BU` 待开始：用户确认问题是在 TermX TUI 测试中出现；截图显示 Codex 0.141.0 primary-screen 首屏包含 `Update available! 0.141.0 -> 0.142.0` 卡片，随后同一 UI 的 current frame/history 视图只剩 main card、Tip、input 与 footer，上方 update card 区域消失或被裁掉。下一切片必须先复现并判断丢失发生在 core-v2 authoritative `HistoryWindow` mutable current frame、styled blank footprint/scroll-region 语义，还是 tui-v3 render/clipping；修复必须补对应 harness，不能从 snapshot/VTerm scrollback 或本地显示缓存拼 history truth。
