...
- `ICE001` 本机 WebRTC-over-TCP 隔离验证已完成；当前没有后续活动切片。Pion 双端仅启用 TCP4 后，真实 selected candidate pair 为 TCP，现有 auth、Hello、Proto API、取消 teardown 和 100 次连续独立建连均通过。该结论不覆盖 SSH tunnel、FRP、Android 真机或弱网性能。
...
- `UX002` 已修正 `termx pair create` 的终端二维码比例：两行模块仍通过 `▀/▄` 合并为一个终端行，但每个模块只占一个终端列，不再横向重复输出。几何 harness 验证二维码可见列数与 packed row 数保持终端单元格等比例关系。
- `ICE001` 已通过独立 PeerConnection factory 给 client/server 注入仅 TCP4 的 Pion API，daemon 使用真实 `ICETCPMux`，E2E 读取 selected candidate pair 并确认两端 protocol 都是 TCP。DataChannel 随后完成 DeviceIdentity/CapabilityGrant auth、Hello 和 `TerminalList` Proto API；信令取消会关闭 PeerConnection。定向测试重复 10 次、核心建连重复 100 次、race、相关 Pion/WebRTC 包和 repository doctor 均通过。
...
| UX002 | 已完成 | 配对二维码终端比例 | 半块字符每个模块只占一个终端列；几何测试防止二维码再次横向拉伸 |
| ICE001 | 已完成 | 本机 ICE-TCP 可行性验证 | Pion 双端仅启用 TCP4，真实 DataChannel 完成 auth/Hello/`api.execute`；selected candidate 为 TCP；100 次建连与取消 teardown 通过 |
...
- `UX002`：`go test ./cmd/termx -run 'TestPairInspectAndTerminalQRNeverPrintLongLivedGrant' -count=1`；`go test ./cmd/termx -count=1`；`make doctor`；`git diff --check`。
- `ICE001`：`go test ./client/adapter/managed/pion -run 'TestPionICETCP' -count=10`；`go test ./client/adapter/managed/pion -run 'TestPionICETCPCompletesAuthHelloAndProtoAPI' -count=100`；`go test -race ./client/adapter/managed/pion -run 'TestPionICETCP' -count=1`；`go test ./client/adapter/managed/pion ./remote/webrtc -count=1`；`make doctor`；`git diff --check`。
