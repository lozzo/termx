...
| R201K. SK core-v2 vterm damage history projector | 待开始 | `termx-core-v2/`、`termx-vterm/`、按需 `termx-tui-v3/`、`workflow.md` | history projector 改为优先消费 shared vterm batch 的 primary scroll-out、screen ops、mode/cursor/alt-screen 语义并转换成 `HistoryEvent`；收缩 `historyANSIParser` 到文本/style/OSC8 辅助，覆盖 rows=1/2 普通输出、Codex raw repaint、scroll region + reverse index + absolute cursor、styled blank、alt-screen final frame |
...
- `R201K` 待开始：下一步必须把 history projector 从 shared vterm damage/scroll-out/mode/cursor 生成 `HistoryEvent`，继续收缩 `historyANSIParser` 的 CSI/OSC、scroll region、reverse index、clear、alt-screen 等终端语义职责；不能把当前 shared batch 边界误判为最终完成。
