...
- FILE001-FILE004 与 CLOUD006-CLOUD008 已完成；Official Android 显式 development build 已通过公网 HTTP staging 在 5G 真机完成 direct、terminal 与恢复链路。生产上线前必须另行切换 HTTPS/TLS，不得复用本切片的明文 profile；真机 single Relay 仍受云安全组 `41003/udp` 入站阻断，放行后必须补验且不得以 lease/listener 冒充通过。
...
- CLOUD008 已完成：Official Android 增加互斥、显式的 `termxOfficialPublicHTTPStaging` debug build，默认 Official、Community 和原 loopback profile 边界不变。`24129PN74C` 真机在无 ADB reverse、5G 网络下完成公网 pairing、managed direct、List/Attach/Input/Output 和 8 秒后台恢复；Connection Info 观测为 `P2P direct`、`prflx / host`，RTT 约 51-64 ms。真机同时暴露并修复 `Use relay` 仅更新 UI 偏好、未把 `forceRelay` 下沉 native 的问题；修复后明确使用 `relay_only` 且失败不回退 direct。当前云厂商安全组未放行 `41003/udp`，外部探测无法到达 `ens18`，所以真机 single Relay 记录为外部阻塞，放行后必须补验。
