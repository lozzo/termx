...
| R351. SK TUI live render loop 接口锁定与重写 | 待开始 | `termx-tui-v3/services/`、`termx-tui-v3/state/`、`termx-tui-v3/app/` | 在旧链路删除后，先定接口锁范围：native screen source、live invalidation source、lifecycle source、render request state；再按接口重写 TUI render loop，维护 `wanted_revision + in_flight + dirty`，渲染完再拉 latest；screen、lifecycle、resize-control 消息分离，不再用 LiveSurface union event 和 runtime 旧队列补丁追帧 |
