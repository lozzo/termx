...
- 当前下一阶段任务队列继续：切片 69 Terminal Pool 数据源与 Picker 服务接线一期。后续如继续推进 Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
