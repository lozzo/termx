...
| 215E1-R139. SK history range copy backend fetch | 待开始 | `termx-core-v2/`、`internal/protocol/`、`termx-proto/`、`termx-tui-v3/services/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`workflow.md` | 补用户要求的 copy mode “记录 index，然后从后端取”：copy selection 端点保存 logical line/row/col boundary 或 logical index，大范围复制按 frozen token + logical range 从 core/file backend 拉 authoritative logical line 组装，不要求 TUI 常驻所有滚过内容；必须与 R138 的 older/newer virtual window 同一 stale guard 语义对齐 |
...
- `215E1-R139 history range copy backend fetch` 待开始：把 copy selection 改成 logical index/range 端点，大范围复制时从 frozen backend 拉 authoritative logical line，不要求 TUI 常驻所有滚过内容。
...
- `215E1-R140 repeated stress RSS 高水位回收` 待开始：按用户实测的同一 terminal 多次执行压力脚本后 RSS 高水位不回落问题，补连续运行 harness，并定位落盘 history 之外的临时 decode/reflow/snapshot/transport/allocator 常驻来源；该切片排在 range copy 后执行，不和 copy/history 虚拟滚动混在一起。
