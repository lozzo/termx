...
| 66. copy-history content renderer 一期 | 待开始 | `termx-tui-v3/render/`、`termx-tui-v3/state/`、`termx-tui-v3/app/`、`termx-tui-v3/services/`、`termx-cli/` 按需 | 在 terminal-live content renderer 一期完成后深化 copy-history 内容 renderer：只消费 core-v2 authoritative `HistoryWindow`，表达 selection、cursor、logical-line marker、clipped before/after、scrollbar 或 position token、copy/yank feedback 和宽字符裁切；resize 或 content cols 变化必须继续 invalid/rebind authoritative window，不得从 live surface、snapshot、grid viewport 或 local VTerm scrollback fallback；harness 覆盖 selection 跨 logical line、clipped span、older/latest 后位置保持、resize 后 no stale rows、copy feedback 和 no chrome leak；准入运行 `cd termx-tui-v3 && go test ./... -count=1`、`cd termx-cli && go test ./... -count=1`、`go run ./termx-cli/cmd/termx v3 smoke`、`go run ./termx-cli/cmd/termx v3 e2e-smoke`、`git diff --check` |

当前下一步：切片 66 待开始。terminal-live content renderer 一期已完成，当前 live 内容能在 pane content rect 内表达基础 ANSI/style、cursor、pending/empty/exited 和宽字符裁切；下一阶段建议深化 copy-history content renderer。Terminal Pool、Workbench Tree、floating 完整交互、remote 迁移和旧依赖拆分不作为下一刀，必须继续按任务队列显式新增。
...
- 当前下一阶段任务队列继续：切片 66 copy-history content renderer 一期。后续如继续推进 Terminal Pool、Workbench Tree、floating 深化、remote 迁移、彻底移除 `termx-cli` module 级旧依赖或拆分 legacy binary，必须继续在本文件新增或调整任务队列后再开始。
