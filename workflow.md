...
- 已收紧 core persisted logical line record metadata 恢复校验：persisted record 必须是 clean、归一后的 persisted/reclaimed origin、persisted id namespace、persisted residency、连续覆盖当前 index 且 generation 匹配；未迁移的 runtime id、live/resize origin、dirty 或未知 origin sidecar 会被视为损坏并回退。
