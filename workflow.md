...
| 215C1. SK terminal inventory delete 接线 | 待开始 | `termx-tui-v3/services/`、`termx-tui-v3/app/`、`termx-tui-v3/state/`、`termx-tui-v3/docs/`、`workflow.md` | core-v2 与 CLI 已有 `RemoveTerminal` / `rm`；本切片只补 tui-v3 service adapter、fake service、Terminal Pool remove message/reducer、Picker/Pool `Ctrl-X DELETE` 的真实 effect/result path 和状态清理；delete 仍是 terminal inventory remove，不得伪装成 kill，也不得绕过 reducer 直接改 service 状态 |
...
| 215F. SK multi-view integration and tmux harness | 待开始 | `termx-cli/`、`termx-tui-v3/`、`termx-core-v2/`、`internal/protocol/`、`Makefile` 按需 | 建立真实/黑盒证据：同一 terminal 连接到同 tab 多 pane 或 floating，输入只进 active view channel，owner resize 触达 PTY，follower 不覆盖 size，kill/remove terminal 更新所有 view，copy/history 只保留现有 no-fallback guard，不扩展 history 功能；运行相关模块测试、tmux/e2e smoke 和 `git diff --check` |
| 216. SK history MVP 重启计划 | 待开始 | `workflow.md`、`termx-core-v2/`、`termx-tui-v3/`、`internal/protocol/`、`termx-proto/`、相关架构文档 | 重新设计并排期真正的 history MVP：先定义 logical-line authoritative history、storage、pagination、resize projection、copy mode 绑定和黑盒 harness，再决定是否恢复 clipboard history overlay；不得在当前简化 demo history 上继续堆 paste/history UI |

当前下一步：执行切片 215C1，先把已有 core-v2 terminal inventory remove 能力接入 tui-v3；随后执行 215D floating overview/group，再做 215F multi-view integration 和 tmux harness。215E clipboard paste/history overlay 因依赖 history MVP 暂时标记为阻塞，不进入当前连续自动推进队列；后续以 216 history MVP 重启计划重新落档。继续保持 copy/history 不从 live surface、snapshot 或 local VTerm scrollback 推断历史，也不得把 styled payload parser 扩张成补丁式完整 VT emulator。
...
- 当前快捷键迁移已完成到 215C：overlay keyboard command router 已收敛到 `ActionSpec`、`ShellContentActionMsg` 和 reducer/effect/message；下一步先执行 215C1 terminal inventory delete 接线，再执行 215D floating overview/group 与 215F multi-view/tmux 验证。215E clipboard paste/history overlay 已后置到 history MVP 之后。
