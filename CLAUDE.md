...
- 需要提交代码时，提交信息保持中文
- 不要在任务进行中自动创建 commit；只有当用户明确要求提交时，才进行提交
- 对于后续步骤，默认自动继续执行，不需要人类逐步介入；只有涉及敏感操作（如破坏性 git 操作、删除重要数据、修改外部环境/凭据、网络侧危险动作）时，才中断并请求人工确认
- TUI V2 迁移允许从现有 `tui/` 复制代码到 `tuiv2/` 作为重组起点，但 **禁止保留任何代码层级的运行时/编译时依赖** 到旧 `tui` 实现；V2 必须能够在删除旧 TUI 代码后独立运行。
- 当前代码库仍处于开发阶段，默认**不要为了兼容性保留额外过渡层、双路径或历史包袱**。
- 如果旧实现、旧接口、旧状态结构已经妨碍新架构，应优先直接删除、替换或推进迁移，而不是继续叠加兼容补丁。
- 发生取舍时，以架构边界清晰、模型一致、实现可维护为最高优先级；向后兼容不是当前阶段的默认目标。
## 构建和测试
