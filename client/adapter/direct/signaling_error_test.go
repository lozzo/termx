...
func TestTCPSignalingClientPreservesOverloadedErrorCode(t *testing.T) {
	...
	go func() {
		...
		serverDone <- directsignal.WriteMessage(serverConnection, &remoteauthpb.DirectSignalingResponseV1{
			Payload: &remoteauthpb.DirectSignalingResponseV1_Error{Error: &remoteauthpb.DirectSignalingErrorV1{
	...
	defer cancel()
	_, err := (direct.TCPSignalingClient{Dialer: directSingleConnectionDialer{connection: clientConnection}}).Exchange(ctx, []string{"direct.test:1"}, &remoteauthpb.DirectSignalingRequestV1{RequestId: "overloaded-client-test"})
