...
- shell/pane 已按切片 88 完成二轮视觉重绘：默认 palette 使用紫色 accent 和深色 chrome/status 背景；toast/floating/overlay title 使用 `·` 分层。header/footer 后续在切片 123 再次对齐 `tuiv2` tab/status bar，切片 139 后 header 当前合同为 `workspace ─┬─ 1:tab × ─┬─ ＋ │ pane:id`，并在第二行用 `├…┴…┤` 表达 tab strip 连接；footer 当前合同为左侧 mode/action hints、右侧 `● active` 与 `ws:*` / `tabs:*` / `panes:*` / `float:*` / live hint metadata；切片 125 后 footer key chip 使用 `[Ctrl+P]` 类紧凑形态，旧 `[＋]`、`▎`、`LIVE`、`[Ctrl] ·` 和 `[Ctrl] • [P]` 不得作为新断言。
