...
- shell/pane 已按切片 88 完成二轮视觉重绘：默认 palette 使用紫色 accent 和深色 chrome/status 背景；toast/floating/overlay title 使用 `·` 分层。header/footer 后续在切片 123 再次对齐 `tuiv2` tab/status bar：header 当前合同为 `workspace │ 1:tab × │ ＋ │ pane:id`，footer 当前合同为左侧 mode/action hints、右侧 `● active` 与 `ws:*` / `tabs:*` / `panes:*` / `float:*` / live hint metadata；旧 `[＋]`、`▎`、`LIVE` 和 `[Ctrl] ·` 不得作为新断言。
- 切片 124 已按用户反馈回收 tiled pane 顶边未设计 token：默认 pane chrome 只保留标题、边框、active/inactive style 和内容裁切，不再提前画状态 Nerd Font、`⇄2`、`◆ owner`、`1/31` 或 action cluster；Nerd Font 字符体系必须后续独立定义语义、字形、fallback 和命中区后再接入。
