...
- shell/pane 已在切片 88 完成二轮视觉重绘；toast/floating/overlay title 使用 `·` 分层。header/footer 已在切片 123 再次对齐 `tuiv2` tab/status bar：top bar 使用 `workspace │ 1:tab × │ ＋ │ pane:id`，bottom bar 左侧放 mode/action hints，右侧放 `● active`、`ws:*`、`tabs:*`、`panes:*`、`float:*` 和 live hint metadata；旧 `[＋]`、`▎`、`LIVE`、`[Ctrl] ·` 不再是当前视觉合同。切片 124 后 tiled pane 顶边只保留标题、边框、active/inactive style 和内容裁切，不再提前画状态 Nerd Font、`⇄2`、`◆ owner`、`1/31` 或 action cluster；Nerd Font 字符体系后续必须独立定义语义、字形、fallback 和命中区。
