...
- shell/pane 已在切片 88 完成二轮视觉重绘；pane top chrome 使用 `· ↔2`、`· ◆ owner`、`· 1/31` 和 action cluster，toast/floating/overlay title 使用 `·` 分层。header/footer 已在切片 123 再次对齐 `tuiv2` tab/status bar：top bar 使用 `workspace │ 1:tab × │ ＋ │ pane:id`，bottom bar 左侧放 mode/action hints，右侧放 `● active`、`ws:*`、`tabs:*`、`panes:*`、`float:*` 和 live hint metadata；旧 `[＋]`、`▎`、`LIVE`、`[Ctrl] ·` 不再是当前视觉合同。
