...
- shell/pane 已在切片 88 完成二轮视觉重绘；toast/floating/overlay title 使用 `·` 分层。header/footer 已在切片 123 再次对齐 `tuiv2` tab/status bar，切片 139 后 top bar 使用 `workspace ─┬─ 1:tab × ─┬─ ＋ │ pane:id`，第二行用 `├…┴…┤` 表达 tab strip 和 body 分隔；bottom bar 左侧放 mode/action hints，右侧放 `● active`、`ws:*`、`tabs:*`、`panes:*`、`float:*` 和 live hint metadata；切片 125 后 bottom bar key chip 使用 `[Ctrl+P]` 类紧凑形态，旧 `[＋]`、`▎`、`LIVE`、`[Ctrl] ·`、`[Ctrl] • [P]` 不再是当前视觉合同。切片 125 后 tiled pane 顶边恢复真实接通的 split-down、split-right、close action，按钮文本与 hit region 同源，pane action 优先于 shared divider resize；仍不提前画状态 Nerd Font、`⇄2`、`◆ owner`、`1/31`、zoom 或 owner/follower token。
