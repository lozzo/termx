...
- 实际键盘路由：`termx-tui-v3/input/bindings.go`、`termx-tui-v3/input/types.go`。
- copy/history 输入：`termx-tui-v3/app/copymode.go`。
- overlay 专用输入：`termx-tui-v3/app/ui_input.go`。
- footer/action 展示：`termx-tui-v3/render/action_ids.go`、`termx-tui-v3/render/vm.go`。
- help 内容：`termx-tui-v3/render/product_content.go`。
- 旧配置入口：`termx-tui-v3/config/config.go`、`termx-tui-v3/state/config.go`。
