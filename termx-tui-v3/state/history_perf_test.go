...
func historyPerfStoreWithLines(firstLineID uint64, count int, cols int) HistoryStore {
	...
	rows, spans := historyPerfRowsForLines(source)
	return HistoryStore{
		ViewID:      "pane:pane-1",
		PaneID:      "pane-1",
		TerminalID:  "term-1",
...
func historyPerfRowsForLines(lines []HistoryLogicalLine) ([]HistoryRow, []HistoryLineSpan) {
	rows := make([]HistoryRow, len(lines))
	spans := make([]HistoryLineSpan, len(lines))
