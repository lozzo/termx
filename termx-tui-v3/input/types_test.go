...
func TestBindingCatalogIsUniqueAndContainsDocumentedAliases(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "resize s layout lock", mode: InteractionModeResize, event: InputEvent{Kind: EventKindKey, Key: KeyChar, Char: "s"}, kind: IntentWorkbenchCommand, command: "terminal layout lock"},
