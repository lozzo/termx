...
type AgentGatewayClient interface {
	Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentEvent, EdgeCommand], error)
}
...
func (c *agentGatewayClient) Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentEvent, EdgeCommand], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentGateway_ServiceDesc.Streams[0], AgentGateway_Connect_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AgentEvent, EdgeCommand]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentGateway_ConnectClient = grpc.BidiStreamingClient[AgentEvent, EdgeCommand]
...
type AgentGatewayServer interface {
	Connect(grpc.BidiStreamingServer[AgentEvent, EdgeCommand]) error
...
func (UnimplementedAgentGatewayServer) Connect(grpc.BidiStreamingServer[AgentEvent, EdgeCommand]) error {
...
func _AgentGateway_Connect_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentGatewayServer).Connect(&grpc.GenericServerStream[AgentEvent, EdgeCommand]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentGateway_ConnectServer = grpc.BidiStreamingServer[AgentEvent, EdgeCommand]
...
var AgentGateway_ServiceDesc = grpc.ServiceDesc{
	...
	Metadata: "cloud/v1/agent_gateway.proto",
}
