...
// ClientHandshake 是公开客户端在 termx protocol 前执行的 DeviceHello/CapabilityOpen 状态机。
...
// 成功后 transport 才能切换到 termx protocol；失败时调用方必须关闭当前 transport，不能回退到 v1 bearer/HMAC。
...
// 它不创建 termx protocol session；成功结果仍须先写入平台 secure credential store，之后普通连接重新执行 capability handshake。
...
// capability 模式可以进入 termx protocol；pairing 模式已经返回 grant 后必须关闭 transport，不能复用为业务 session。
type ServerHandshakeMode uint8

const (
	// ServerHandshakeModeCapability 表示 client-bound capability 已验证，可进入 scoped termx protocol。
