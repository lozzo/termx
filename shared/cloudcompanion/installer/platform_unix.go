...
func DefaultRootDir() string {
	if dataHome := os.Getenv("XDG_DATA_HOME"); filepath.IsAbs(dataHome) {
		return filepath.Join(dataHome, "muxvia", "cloud-companion")
	}
	home, err := os.UserHomeDir()
	if err == nil {
		return filepath.Join(home, ".local", "share", "muxvia", "cloud-companion")
	}
	return filepath.Join(os.TempDir(), fmt.Sprintf("muxvia-cloud-companion-%d", os.Getuid()))
}

// ExecutableName 返回当前平台 archive 中唯一允许的 Companion executable 名称。
func ExecutableName() string { return "muxvia-cloud" }
