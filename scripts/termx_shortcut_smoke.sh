...
usage() {
  cat <<'EOF'
Usage: scripts/termx_shortcut_smoke.sh [--bin PATH] [--root PATH] [--keep-root]
...
if [[ -z "$ROOT" ]]; then
  ROOT="$(mktemp -d "${TMPDIR:-/tmp}/termx-shortcut-smoke.XXXXXX")"
fi
...
SESSION="termx-shortcut-$$"
CSI_SESSION="termx-shortcut-csi-$$"
FAILED=1
DAEMON_PID=""
...
cleanup() {
  ...
  if [[ "$FAILED" == "0" && "$KEEP_ROOT" == "0" ]]; then
    ...
    printf '[termx-shortcut-smoke] artifacts: %s\n' "$ROOT" >&2
  fi
}
trap cleanup EXIT
...
mkdir -p "$CFG/termx" "$STATE"
XDG_CONFIG_HOME="$CFG" XDG_STATE_HOME="$STATE" TERMX_REMOTE_ENABLE=false \
...
if [[ -z "$TERMINAL_ID" ]]; then
  echo "termx new did not return a terminal id" >&2
  exit 1
fi

printf -v ATTACH 'env -u TERMX -u TERMX_TERMINAL_ID -u TERMX_TUI_DIAG -u TERMX_TUI_INPUT_TRACE XDG_CONFIG_HOME=%q XDG_STATE_HOME=%q TERMX_REMOTE_ENABLE=false TERM=xterm-256color %q --socket %q --log-file %q attach %q' \
...
FAILED=0
printf '[termx-shortcut-smoke] PASS root/sticky/overlay/copy/quit/CSI-u\n'
