...
usage() {
  cat <<'EOF'
Usage: scripts/anytty_shortcut_smoke.sh [--bin PATH] [--root PATH] [--keep-root]
...
if [[ -z "$ROOT" ]]; then
  ROOT="$(mktemp -d "${TMPDIR:-/tmp}/anytty-shortcut-smoke.XXXXXX")"
fi
...
SESSION="anytty-shortcut-$$"
CSI_SESSION="anytty-shortcut-csi-$$"
FAILED=1
DAEMON_PID=""
...
cleanup() {
  ...
  if [[ "$FAILED" == "0" && "$KEEP_ROOT" == "0" ]]; then
    ...
    printf '[anytty-shortcut-smoke] artifacts: %s\n' "$ROOT" >&2
  fi
}
trap cleanup EXIT
...
mkdir -p "$CFG/anytty" "$STATE"
XDG_CONFIG_HOME="$CFG" XDG_STATE_HOME="$STATE" ANYTTY_REMOTE_ENABLE=false \
...
if [[ -z "$TERMINAL_ID" ]]; then
  echo "anytty new did not return a terminal id" >&2
  exit 1
fi

printf -v ATTACH 'env -u ANYTTY -u ANYTTY_TERMINAL_ID -u ANYTTY_TUI_DIAG -u ANYTTY_TUI_INPUT_TRACE XDG_CONFIG_HOME=%q XDG_STATE_HOME=%q ANYTTY_REMOTE_ENABLE=false TERM=xterm-256color %q --socket %q --log-file %q attach %q' \
...
cat >"$CFG/anytty/tui-v3.yaml" <<'EOF'
version: 1
tui:
  shortcuts:
    global:
...
FAILED=0
printf '[anytty-shortcut-smoke] PASS root/sticky/overlay/copy/quit/CSI-u\n'
