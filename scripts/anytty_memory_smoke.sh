...
usage() {
  cat <<'EOF'
Usage: scripts/anytty_memory_smoke.sh [options]

Run a real-process memory smoke path:
  build/use anytty -> start core-v2 daemon -> create stress terminal
...
if [[ -z "$ROOT" ]]; then
  ROOT="$(mktemp -d "${TMPDIR:-/tmp}/anytty-memory-smoke.XXXXXX")"
else
  mkdir -p "$ROOT"
fi
ROOT="$(cd "$ROOT" && pwd)"
...
if [[ -n "$DAEMON_MEMORY_LIMIT_MB" ]]; then
  ...
  ANYTTY_DAEMON_MEMORY_LIMIT_MB="$DAEMON_MEMORY_LIMIT_MB" ANYTTY_DAEMON_HEAP_PROFILE_DIR="$DAEMON_HEAP_DIR" "$BIN" --socket "$SOCK" --log-file "$LOG" daemon >"$ROOT/daemon.stdout" 2>"$ROOT/daemon.stderr" &
else
  ANYTTY_DAEMON_HEAP_PROFILE_DIR="$DAEMON_HEAP_DIR" "$BIN" --socket "$SOCK" --log-file "$LOG" daemon >"$ROOT/daemon.stdout" 2>"$ROOT/daemon.stderr" &
fi
DAEMON_PID=$!
...
export ANYTTY_TUI_HEAP_PROFILE_DIR=$TUI_HEAP_DIR_QUOTED
