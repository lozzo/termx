...
$packageRoot = Join-Path $repoRoot ".artifacts\packages\anytty-$Version-windows-amd64"
...
Copy-Item -LiteralPath $binary -Destination (Join-Path $packageRoot 'anytty.exe')
Copy-Item -LiteralPath (Join-Path $repoRoot 'LICENSE') -Destination $packageRoot
Copy-Item -LiteralPath (Join-Path $repoRoot 'cmd\anytty\THIRD_PARTY_NOTICES.txt') -Destination (Join-Path $packageRoot 'THIRD_PARTY_NOTICES.txt')
