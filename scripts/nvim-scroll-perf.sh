...
go test ./tuiv2/app -run TestPerfNvimScrollReport -count=1 -v | tee "$OUT_DIR/test.log"

if [[ "${TERMX_PERF_PPROF:-0}" == "1" ]]; then
  echo "collecting cpu profile..."
  go test ./tuiv2/app -run TestPerfNvimScrollReport -count=1 -cpuprofile "$OUT_DIR/cpu.out"
fi
