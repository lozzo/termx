...
usage() {
  cat <<'EOF'
Usage: scripts/muxvia_tui_stress_memory.sh [options]

Run the real interactive stress path:
  build/use muxvia -> start core-v2 daemon -> create shell terminal
...
Options:
  ...
  --bin PATH            existing muxvia binary; default builds into ROOT/muxvia
  ...
  --baseline-time       run the same stress script outside muxvia first (default)
  --no-baseline-time    skip outside-muxvia baseline timing
  ...
  --history-disabled    start daemon with MUXVIA_HISTORY_DISABLE=1 and skip copy/history checks;
  ...
  --history-backpressure-mode MODE
                        set MUXVIA_HISTORY_BACKPRESSURE_MODE for daemon; low-latency or bounded
  --history-backpressure-buffer-mb N
                        set MUXVIA_HISTORY_BACKPRESSURE_BUFFER_MB for daemon
  --daemon-memory-limit-mb N
                        set MUXVIA_DAEMON_MEMORY_LIMIT_MB for daemon runtime GC pacing
  --daemon-request-reclaim-min-heap-mb N
                        set MUXVIA_DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB for request-boundary page reclaim
  --use-real-state      do not isolate XDG_STATE_HOME; useful for reproducing user default-state RSS
  --tui-memory-limit-mb N
                        set MUXVIA_TUI_MEMORY_LIMIT_MB for TUI runtime GC pacing
...
Artifacts:
  ...
  baseline-time.txt     outside-muxvia baseline time(1) output when enabled
  ...
  state/muxvia/history-v2/ default daemon file-backed history payloads
...
log() {
  printf '[muxvia-tui-stress-memory] %s\n' "$*"
}
...
run_tmux_stress_baseline() {
  local run_seed="$1"
  local baseline_session="muxvia-stress-baseline-$$"
...
write_profile_summary() {
  ...
  {
    echo "# muxvia TUI stress profile summary"
    echo
...
if [[ -z "$ROOT" ]]; then
  ROOT="$(mktemp -d "${TMPDIR:-/tmp}/muxvia-tui-stress-memory.XXXXXX")"
else
  mkdir -p "$ROOT"
fi
ROOT="$(cd "$ROOT" && pwd)"
...
DAEMON_DEFAULT_HISTORY_BACKEND_DIR="$DAEMON_DEFAULT_STATE_DIR/muxvia/history-v2"
DIAG_STAGE_FILE="$ROOT/diag-stage.txt"
SESSION="muxvia-tui-stress-$$"
TARGET="$SESSION:0.0"
...
if [[ ! -x "$BIN" ]]; then
  echo "muxvia binary is not executable: $BIN" >&2
  exit 1
fi
...
if [[ "$BASELINE_TIME" == "1" ]]; then
  log "running outside-muxvia stress baseline: $(stress_command_display "$SEED")"
...
log "starting daemon"
DAEMON_ENV_PREFIX=(MUXVIA_STRESS_HARNESS=1)
if [[ "$HISTORY_DISABLED" == "1" ]]; then
  # 中文说明：该模式只用于隔离 live SurfaceTrack 可见延迟，不改变默认历史准入。
  DAEMON_ENV_PREFIX+=(MUXVIA_HISTORY_DISABLE=1)
fi
if [[ -n "$HISTORY_BACKPRESSURE_MODE" ]]; then
  log "history backpressure mode: $HISTORY_BACKPRESSURE_MODE"
  DAEMON_ENV_PREFIX+=(MUXVIA_HISTORY_BACKPRESSURE_MODE="$HISTORY_BACKPRESSURE_MODE")
fi
if [[ -n "$HISTORY_BACKPRESSURE_BUFFER_MB" ]]; then
  log "history backpressure buffer: ${HISTORY_BACKPRESSURE_BUFFER_MB}MB"
  DAEMON_ENV_PREFIX+=(MUXVIA_HISTORY_BACKPRESSURE_BUFFER_MB="$HISTORY_BACKPRESSURE_BUFFER_MB")
fi
if [[ "$USE_REAL_STATE" == "0" ]]; then
  DAEMON_ENV_PREFIX+=(XDG_STATE_HOME="$DAEMON_DEFAULT_STATE_DIR")
fi
if [[ "$PERFTRACE" == "1" ]]; then
  DAEMON_ENV_PREFIX+=(MUXVIA_PERF_TRACE="$DAEMON_PERFTRACE" MUXVIA_PERF_TRACE_INTERVAL_MS=200 MUXVIA_PERF_TRACE_BUCKET_MS=100)
fi
if [[ "$USE_REAL_STATE" == "1" ]]; then
...
if [[ -n "$DAEMON_MEMORY_LIMIT_MB" ]]; then
  ...
  env "${DAEMON_ENV_PREFIX[@]}" MUXVIA_DAEMON_MEMORY_LIMIT_MB="$DAEMON_MEMORY_LIMIT_MB" MUXVIA_DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB="$DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB" MUXVIA_DAEMON_HEAP_PROFILE_DIR="$DAEMON_HEAP_DIR" MUXVIA_DAEMON_MEMSTATS_DIR="$DAEMON_MEMSTATS_DIR" MUXVIA_DIAG_STAGE_FILE="$DIAG_STAGE_FILE" "$BIN" --socket "$SOCK" --log-file "$LOG" daemon >"$ROOT/daemon.stdout" 2>"$ROOT/daemon.stderr" &
else
  if [[ -n "$DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB" ]]; then
    log "daemon request reclaim min heap: ${DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB}MB"
  fi
  env "${DAEMON_ENV_PREFIX[@]}" MUXVIA_DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB="$DAEMON_REQUEST_RECLAIM_MIN_HEAP_MB" MUXVIA_DAEMON_HEAP_PROFILE_DIR="$DAEMON_HEAP_DIR" MUXVIA_DAEMON_MEMSTATS_DIR="$DAEMON_MEMSTATS_DIR" MUXVIA_DIAG_STAGE_FILE="$DIAG_STAGE_FILE" "$BIN" --socket "$SOCK" --log-file "$LOG" daemon >"$ROOT/daemon.stdout" 2>"$ROOT/daemon.stderr" &
fi
DAEMON_PID=$!
...
export MUXVIA_ALLOW_NESTED=1
export MUXVIA_TUI_HEAP_PROFILE_MODE=$(shell_quote "$PROFILE_MODE")
export MUXVIA_TUI_HEAP_PROFILE_DIR=$(shell_quote "$TUI_HEAP_DIR")
export MUXVIA_TUI_MEMSTATS_DIR=$(shell_quote "$TUI_MEMSTATS_DIR")
export MUXVIA_DIAG_STAGE_FILE=$(shell_quote "$DIAG_STAGE_FILE")
EOF
if [[ "$PERFTRACE" == "1" ]]; then
  {
    printf 'export MUXVIA_PERF_TRACE=%s\n' "$(shell_quote "$TUI_PERFTRACE")"
    printf 'export MUXVIA_PERF_TRACE_INTERVAL_MS=200\n'
    printf 'export MUXVIA_PERF_TRACE_BUCKET_MS=100\n'
  } >>"$ATTACH_SCRIPT"
fi
if [[ "$USE_REAL_STATE" == "0" ]]; then
...
if [[ "$PROFILE_MODE" == "all" ]]; then
  cat >>"$ATTACH_SCRIPT" <<EOF
export MUXVIA_TUI_HEAP_PROFILE_EVERY_MB=8
export MUXVIA_TUI_DIAG=1
export MUXVIA_TUI_DIAG_INTERVAL_MS=200
EOF
fi
if [[ -n "$TUI_MEMORY_LIMIT_MB" ]]; then
  log "TUI memory limit: ${TUI_MEMORY_LIMIT_MB}MB"
  printf 'export MUXVIA_TUI_MEMORY_LIMIT_MB=%s\n' "$(shell_quote "$TUI_MEMORY_LIMIT_MB")" >>"$ATTACH_SCRIPT"
fi
cat >>"$ATTACH_SCRIPT" <<EOF
...
if [[ -s "$ROOT/baseline-time.txt" ]]; then
  log "outside-muxvia baseline time"
