...
export function RegisterPage() {
  ...
  useEffect(() => { document.title = '创建账号 · Muxvia Cloud' }, [])
  ...
  return <AuthLayout title="创建 Muxvia 账号" description="注册后即可添加 daemon，并使用托管 P2P 与 Relay。" alternate={<>已经有账号？<Link to="/login">直接登录</Link></>}>
    <form onSubmit={submit}>
      <Field label="显示名称"><Input autoComplete="name" autoFocus value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></Field>
      ...
      {mutation.error && <p className="form-error" role="alert">该邮箱可能已经注册，或注册信息不符合要求。</p>}
