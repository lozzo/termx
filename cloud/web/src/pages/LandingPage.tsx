...
export function LandingPage() {
  ...
  return <div className="landing-page">
    <header className="landing-header"><Link className="landing-brand" to="/"><img src={logo} alt="AnyTTY" /><strong>AnyTTY Cloud</strong></Link><nav aria-label="公开导航"><a href="#connect">如何连接</a><a href="#plans">套餐</a><a href="#security">安全</a><a href="https://github.com/anytty/anytty" aria-label="在 GitHub 查看 AnyTTY"><GitFork size={19} /></a><Link className="button button-quiet" to="/login">登录</Link><Link className="button button-primary" to="/register">免费开始</Link></nav></header>
    <main>
      <section className="landing-hero">
        <div className="hero-copy"><span className="eyebrow">ANYTTY CLOUD</span><h1>AnyTTY Cloud</h1><strong className="hero-statement">随时回到你的电脑。</strong><p>在手机、TUI 和桌面客户端中找到自己的设备并安全连接。Cloud 优先直连，网络受限时自动通过 Edge 中转。</p><div className="hero-actions"><Link className="button button-primary" to="/register">免费开始<ArrowRight size={18} /></Link><Link className="button" to="/login">登录账号</Link></div><dl><div><dt>你的数据</dt><dd>端到端加密</dd></div><div><dt>连接方式</dt><dd>P2P / Relay</dd></div><div><dt>无需订阅</dt><dd>Direct / SSH</dd></div></dl></div>
        <div className="hero-product" aria-label="AnyTTY Cloud 产品连接画面">
          <div className="route-stage"><span><Smartphone size={20} />AnyTTY App</span><i data-state="active" /><b>Cloud Edge · CN1</b><i /><span><MonitorSmartphone size={20} />我的 Mac</span></div>
      ...
      <section className="final-cta"><div><img src={logo} alt="" /><h2>从第一台设备开始使用 AnyTTY Cloud。</h2><p>注册账号后生成一次性安装命令，几分钟内完成接入。</p><Link className="button button-primary" to="/register">免费创建账号<ArrowRight size={18} /></Link></div></section>
    </main>
