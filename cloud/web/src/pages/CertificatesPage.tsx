...
export function CertificatesPage() {
  ...
  return <>
    ...
    {pending ? <Skeleton /> : failedQuery && !dialogOpen ? <ErrorState error={failedQuery.error} onRetry={() => void failedQuery.refetch()} /> : !failedQuery ? <div className="certificate-workspace">
    ...
    <Dialog title={editing ? `替换 ${editing.name}` : '上传证书'} open={dialogOpen} onClose={closeUpload} footer={<><Button tone="quiet" onClick={closeUpload}>取消</Button><Button tone="primary" type="submit" form="certificate-upload" disabled={upload.isPending}>{upload.isPending ? '正在校验' : editing ? '替换并自动更新' : '上传证书'}</Button></>}>
      <form id="certificate-upload" className="form-grid" onSubmit={submit}>
        ...
        {failedQuery && <ErrorState error={failedQuery.error} onRetry={() => void failedQuery.refetch()} />}
