...
export function AccountDetailPage() {
  ...
  if (query.error || !summary || !profile) return <ErrorState error={query.error} />
  return <>
    ...
    <section className="plain-section"><header><h2>订单记录</h2></header>{commerce.isPending ? <Skeleton rows={3} /> : commerce.error ? <ErrorState error={commerce.error} /> : !commerce.data?.orders.length ? <Empty>暂无订单</Empty> : <TableFrame><table><thead><tr><th>订单</th><th>套餐</th><th>状态</th><th>金额</th><th>创建时间</th></tr></thead><tbody>{commerce.data.orders.map((order) => <tr key={order.orderId}><td className="mono">{compactID(order.orderId)}</td><td>{order.planId} v{order.planVersion.toString()}</td><td>{orderState(order.status)}</td><td>{money(order.amount?.currency ?? 'CNY', order.amount?.minorUnits ?? 0n)}</td><td>{dateTime(order.createdAt)}</td></tr>)}</tbody></table></TableFrame>}</section>
    <Dialog title={profile.state === AccountState.ACTIVE ? '禁用账号' : '恢复账号'} open={stateOpen} onClose={() => setStateOpen(false)} footer={<><Button tone="quiet" onClick={() => setStateOpen(false)}>取消</Button><Button tone={profile.state === AccountState.ACTIVE ? 'danger' : 'primary'} onClick={() => setState.mutate()} disabled={!reason.trim() || setState.isPending}>确认</Button></>}><Notice tone="warning">账号状态变更会影响后续 Cloud 准入，不会伪造或保留旧在线拓扑。</Notice><Field label="操作原因"><Input autoFocus value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{setState.error && <p className="form-error">{setState.error.message}</p>}</Dialog>
    <Dialog title="调整运营角色" open={roleOpen} onClose={() => setRoleOpen(false)} footer={<><Button tone="quiet" onClick={() => setRoleOpen(false)}>取消</Button><Button tone="primary" onClick={() => setRole.mutate()} disabled={!reason.trim() || setRole.isPending}>保存</Button></>}><div className="form-grid"><Field label="角色"><select className="input" value={selectedRole} onChange={(event) => setSelectedRole(Number(event.target.value))}><option value={AccountRole.OPERATOR}>运营</option><option value={AccountRole.ADMIN}>管理员</option></select></Field><label className="check-field"><input type="checkbox" checked={enableRole} onChange={(event) => setEnableRole(event.target.checked)} />授予角色</label><Field label="操作原因"><Input value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{setRole.error && <p className="form-error">{setRole.error.message}</p>}</div></Dialog>
  </>
}
