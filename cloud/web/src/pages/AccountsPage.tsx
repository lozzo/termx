...
import { Check, Clipboard, KeyRound, Search, Shield, UserPlus, UserRoundCog } from 'lucide-react'
...
import { GetOperatorAccountResponseSchema, ListOperatorAccountsResponseSchema, ProvisionAccountRequestSchema, ProvisionAccountResponseSchema, ResetAccountSetupRequestSchema, ResetAccountSetupResponseSchema, SetAccountRoleRequestSchema, SetAccountRoleResponseSchema, SetAccountStateRequestSchema, SetAccountStateResponseSchema, type AccountSummary, type ProvisionAccountResponse, type ResetAccountSetupResponse } from '../generated/cloud/v1/operator_pb'
...
export function AccountsPage() {
  ...
  const [search, setSearch] = useState(params.get('query') ?? '')
	...
	const provision = useMutation({ mutationFn: () => protoSend('/api/operator/accounts', ProvisionAccountRequestSchema, create(ProvisionAccountRequestSchema, { email, displayName, reason }), ProvisionAccountResponseSchema), onSuccess: () => { void client.invalidateQueries({ queryKey: ['accounts'] }) } })
	function closeProvision() { setProvisionOpen(false); setEmail(''); setDisplayName(''); setReason(''); provision.reset() }
	function submit(event: FormEvent) { event.preventDefault(); pagination.reset(); setParams(search.trim() ? { query: search.trim() } : {}) }
	return <>
		<PageHeader title="用户与权限" meta="账号、角色、订阅、Entitlement 与当期用量" actions={<Button tone="primary" onClick={() => setProvisionOpen(true)}><UserPlus size={16} />创建账号</Button>} />
    <form className="list-toolbar" onSubmit={submit}><div className="search-input"><Search size={16} aria-hidden="true" /><label className="visually-hidden" htmlFor="account-search">搜索账号</label><input id="account-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="账号名称、邮箱或 ID" /></div><Button type="submit">查询</Button></form>
		...
		<Dialog title={provision.data ? '一次性凭据' : '创建账号'} open={provisionOpen} onClose={closeProvision} closable={!provision.isPending} footer={provision.data ? <Button tone="primary" onClick={closeProvision}>完成</Button> : <><Button tone="quiet" onClick={closeProvision} disabled={provision.isPending}>取消</Button><Button tone="primary" onClick={() => provision.mutate()} disabled={!email.trim() || !displayName.trim() || !reason.trim() || provision.isPending}>{provision.isPending ? '正在创建' : '创建账号'}</Button></>}>
			{provision.data ? <SetupCredentialResult value={provision.data} /> : <div className="form-grid"><Field label="邮箱"><Input type="email" autoComplete="off" autoFocus value={email} onChange={(event) => setEmail(event.target.value)} /></Field><Field label="显示名称"><Input value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></Field><Field label="创建原因"><Input value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{provision.error && <MutationError action="创建账号" error={provision.error} />}</div>}
		</Dialog>
	</>
}
...
export function AccountDetailPage() {
  ...
  const profile = summary?.account
	const setState = useMutation({ mutationFn: () => protoSend(`/api/operator/accounts/${accountId}/state`, SetAccountStateRequestSchema, create(SetAccountStateRequestSchema, { accountId, expectedRevision: profile?.revision, state: profile?.state === AccountState.DISABLED ? AccountState.ACTIVE : AccountState.DISABLED, reason }), SetAccountStateResponseSchema), onSuccess: () => { setStateOpen(false); setReason(''); void client.invalidateQueries({ queryKey: ['accounts'] }) } })
	...
	const reset = useMutation({ mutationFn: () => protoSend(`/api/operator/accounts/${accountId}/reset`, ResetAccountSetupRequestSchema, create(ResetAccountSetupRequestSchema, { accountId, reason }), ResetAccountSetupResponseSchema), onSuccess: () => { setReason(''); void client.invalidateQueries({ queryKey: ['accounts'] }) } })
	function closeReset() { setResetOpen(false); setReason(''); reset.reset() }
  if (query.isPending) return <Skeleton />
  if (query.error || !summary || !profile) return <ErrorState error={query.error} onRetry={() => void query.refetch()} />
  return <>
		<PageHeader title={profile.displayName} meta={`${profile.email || '未填写邮箱'} · ${profile.accountId}`} actions={<><Button onClick={() => setResetOpen(true)}><KeyRound size={16} />重置凭据</Button><Button onClick={() => setRoleOpen(true)}><Shield size={16} />调整角色</Button>{profile.state !== AccountState.PENDING && <Button tone={profile.state === AccountState.DISABLED ? 'primary' : 'danger'} onClick={() => setStateOpen(true)}><UserRoundCog size={16} />{profile.state === AccountState.DISABLED ? '恢复账号' : '禁用账号'}</Button>}</>} />
    ...
    <section className="plain-section"><header><h2>订单记录</h2></header>{commerce.isPending ? <Skeleton rows={3} /> : commerce.error ? <ErrorState error={commerce.error} onRetry={() => void commerce.refetch()} /> : !commerce.data?.orders.length ? <Empty>暂无订单</Empty> : <TableFrame><table><thead><tr><th>订单</th><th>套餐</th><th>状态</th><th>金额</th><th>创建时间</th></tr></thead><tbody>{commerce.data.orders.map((order) => <tr key={order.orderId}><td className="mono">{compactID(order.orderId)}</td><td>{order.planId} v{order.planVersion.toString()}</td><td>{orderState(order.status)}</td><td>{money(order.amount?.currency ?? 'CNY', order.amount?.minorUnits ?? 0n)}</td><td>{dateTime(order.createdAt)}</td></tr>)}</tbody></table></TableFrame>}</section>
		<Dialog title={profile.state === AccountState.DISABLED ? '恢复账号' : '禁用账号'} open={stateOpen} onClose={() => setStateOpen(false)} footer={<><Button tone="quiet" onClick={() => setStateOpen(false)}>取消</Button><Button tone={profile.state === AccountState.DISABLED ? 'primary' : 'danger'} onClick={() => setState.mutate()} disabled={!reason.trim() || setState.isPending}>确认</Button></>}><Notice tone="warning">账号状态变更会影响后续 Cloud 准入，不会伪造或保留旧在线拓扑。</Notice><Field label="操作原因"><Input autoFocus value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{setState.error && <MutationError action="变更账号状态" error={setState.error} />}</Dialog>
		<Dialog title="调整运营角色" open={roleOpen} onClose={() => setRoleOpen(false)} footer={<><Button tone="quiet" onClick={() => setRoleOpen(false)}>取消</Button><Button tone="primary" onClick={() => setRole.mutate()} disabled={!reason.trim() || setRole.isPending}>保存</Button></>}><div className="form-grid"><Field label="角色"><select className="input" value={selectedRole} onChange={(event) => setSelectedRole(Number(event.target.value))}><option value={AccountRole.OPERATOR}>运营</option><option value={AccountRole.ADMIN}>管理员</option></select></Field><label className="check-field"><input type="checkbox" checked={enableRole} onChange={(event) => setEnableRole(event.target.checked)} />授予角色</label><Field label="操作原因"><Input value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{setRole.error && <MutationError action="调整运营角色" error={setRole.error} />}</div></Dialog>
		<Dialog title={reset.data ? '一次性凭据' : '重置账号凭据'} open={resetOpen} onClose={closeReset} closable={!reset.isPending} footer={reset.data ? <Button tone="primary" onClick={closeReset}>完成</Button> : <><Button tone="quiet" onClick={closeReset} disabled={reset.isPending}>取消</Button><Button tone="danger" onClick={() => reset.mutate()} disabled={!reason.trim() || reset.isPending}>{reset.isPending ? '正在重置' : '重置凭据'}</Button></>}>
			{reset.data ? <SetupCredentialResult value={reset.data} /> : <><Notice tone="warning">重置后账号将变为待设置状态，旧密码和全部持久登录凭据立即失效；已签发的访问凭据最多 15 分钟后失效。</Notice><Field label="操作原因"><Input autoFocus value={reason} onChange={(event) => setReason(event.target.value)} /></Field>{reset.error && <MutationError action="重置凭据" error={reset.error} />}</>}
		</Dialog>
	</>
}

export function SetupCredentialResult({ value }: { value: ProvisionAccountResponse | ResetAccountSetupResponse }) {
	const [copied, setCopied] = useState(false)
	const [copyFailed, setCopyFailed] = useState(false)
	const setupLink = `${window.location.origin}/setup#${value.setupCredential}`
...
export function MutationError({ action, error }: { action: string; error: unknown }) {
	...
	const permission = error instanceof APIError && error.code === 'recent_auth_required'
		? '请先在账号安全页完成最近认证。'
		: error instanceof APIError && error.status === 403
			? '没有执行此操作的权限。'
			: '请检查输入或刷新后重试。'
	return <p className="form-error" role="alert">无法{action}。{permission}{correlationID ? ` 关联 ID：${correlationID}` : ''}</p>
}
