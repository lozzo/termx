...
export function SetupCredentialResult({ value }: { value: ProvisionAccountResponse | ResetAccountSetupResponse }) {
	...
	async function copyCredential() {
		setCopyFailed(false)
		try { await navigator.clipboard.writeText(value.setupCredential); setCopied(true) } catch { setCopyFailed(true) }
	}
	return <div className="credential-result"><Notice tone="warning"><strong>仅展示一次</strong><span>关闭窗口后无法再次查看；请通过可信方式交给账号本人。</span></Notice><div className="credential-value"><code>{value.setupCredential}</code><Button type="button" onClick={() => void copyCredential()}>{copied ? <Check size={16} /> : <Clipboard size={16} />}{copied ? '已复制' : '复制'}</Button></div><p>有效期至 {dateTime(value.expiresAt)}</p>{copyFailed && <p className="form-error" role="alert">无法访问剪贴板，请手动选择凭据。</p>}</div>
}

function MutationError({ action, error }: { action: string; error: unknown }) {
	const correlationID = error instanceof APIError ? error.correlationID : ''
	const permission = error instanceof APIError && error.status === 403 ? '请先在账号安全页完成最近认证。' : '请检查输入或刷新后重试。'
