...
export function UserOverviewPage() {
  ...
  return <>
    <section className="account-hero">
      <div className="account-hero-copy"><span className="account-kicker"><ShieldCheck size={15} />AnyTTY Cloud 已就绪</span><h1>你好，{current.account?.displayName ?? ''}</h1><p>{total === 0 ? '添加第一台设备后，就能在 AnyTTY App 中从外网安全连接。' : `${online} 台设备在线。打开 AnyTTY App，即可继续上次的终端工作。`}</p><div className="account-hero-actions"><Link className="button button-primary" to="/app/devices">{total === 0 ? <><Plus size={17} />添加第一台设备</> : <><MonitorSmartphone size={17} />管理设备</>}</Link><Link className="button" to="/app/subscription">查看套餐</Link></div></div>
      <div className={`account-route-state ${online > 0 ? 'is-online' : ''}`} aria-label={online > 0 ? 'Cloud 设备连接可用' : '当前没有在线设备'}><span><Smartphone size={21} />AnyTTY App</span><i><b /></i><span>{online > 0 ? <Wifi size={21} /> : <WifiOff size={21} />}{online > 0 ? '连接可用' : '等待设备'}</span><small>{currentPlan?.name ?? subscription?.planId ?? '尚未订阅'}</small></div>
    </section>
