...
export function EdgesPage() {
  ...
  return <>
    ...
    {query.isPending ? <Skeleton /> : query.error ? <ErrorState error={query.error} /> : !query.data?.edges.length ? <Empty>尚未创建 Edge</Empty> : <EdgeTable edges={query.data.edges} />}
    <Dialog title={install ? '安装命令' : '添加 Edge'} open={open} onClose={() => setOpen(false)} footer={install ? <Button tone="primary" onClick={() => setOpen(false)}>完成</Button> : <><Button tone="quiet" onClick={() => setOpen(false)}>取消</Button><Button tone="primary" type="submit" form="create-edge" disabled={createEdge.isPending}>生成安装命令</Button></>}>
      {install ? <><Notice tone="warning">命令中的注册凭据只显示一次，并在十分钟后失效。</Notice><div className="command-box"><code>{install}</code><Button onClick={() => navigator.clipboard.writeText(install)}><Copy size={16} />复制</Button></div></> : <form id="create-edge" className="form-grid" onSubmit={submit}>
        ...
        {createEdge.error && <p className="form-error">{createEdge.error.message}</p>}
      </form>}
    </Dialog>
  </>
}
...
export function EdgeDetailPage() {
  ...
  if (edges.error) return <ErrorState error={edges.error} />
