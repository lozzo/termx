...
export function DevicesPage() {
  ...
  const daemonCount = query.data?.daemonCount ?? 0
  const daemonLimit = query.data?.daemonLimit ?? 0
  const daemonFull = daemonLimit > 0 && daemonCount >= daemonLimit
  const capacityMeta = query.isPending ? '正在读取套餐设备额度' : daemonLimit > 0 ? `已注册 ${daemonCount} / ${daemonLimit}` : '设备额度暂不可用'
  ...
  return <>
    <PageHeader title="Daemon 管理" meta={`${capacityMeta}；Cloud 账号不会把 daemon 自动加入 AnyTTY App`} actions={<Button tone="primary" disabled={query.isPending || daemonFull} onClick={() => { setCommand(''); setDaemonName(''); setCopyState('idle'); setOpen(true) }}><Plus size={17} />注册 daemon</Button>} />
    <Notice>在这里注册目标 daemon。安装完成后，请在目标 daemon 或服务上生成配对二维码，并用无需账号的 AnyTTY App 扫描；配对端点只保存在 App 本机。</Notice>
    {daemonFull && <Notice tone="warning">已注册 daemon 达到当前套餐上限（{daemonCount} / {daemonLimit}）。请升级套餐或永久删除不再使用的 daemon 后再注册。</Notice>}
