...
export function DaemonsPage() {
  ...
  const enroll = useMutation({ mutationFn: () => protoSend('/api/operator/daemons', CreateDaemonEnrollmentRequestSchema, create(CreateDaemonEnrollmentRequestSchema, { accountId: accountID, accountName, daemonName }), CreateDaemonEnrollmentResponseSchema), onSuccess: (value) => setCommand(value.enrollCommand) })
  ...
  return <>
    ...
    <Dialog title={command ? '注册命令' : '注册 daemon'} open={enrollOpen} onClose={() => setEnrollOpen(false)} footer={command ? <Button tone="primary" onClick={() => setEnrollOpen(false)}>完成</Button> : <><Button tone="quiet" onClick={() => setEnrollOpen(false)}>取消</Button><Button tone="primary" onClick={() => enroll.mutate()} disabled={!accountID || !daemonName || enroll.isPending}>生成命令</Button></>}>
      {command ? <><Notice tone="warning">注册口令只可消费一次。</Notice><div className="command-box"><code>{command}</code><Button onClick={() => navigator.clipboard.writeText(command)}><Copy size={16} />复制</Button></div></> : <div className="form-grid"><Field label="账号 ID"><Input required value={accountID} onChange={(event) => setAccountID(event.target.value)} /></Field><Field label="账号名称"><Input value={accountName} onChange={(event) => setAccountName(event.target.value)} /></Field><Field label="daemon 名称"><Input required value={daemonName} onChange={(event) => setDaemonName(event.target.value)} /></Field>{enroll.error && <p className="form-error">{enroll.error.message}</p>}</div>}
    </Dialog>
