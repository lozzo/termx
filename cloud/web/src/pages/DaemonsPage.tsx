...
export function DaemonsPage() {
  ...
  return <>
    ...
    <Dialog title={command ? '注册命令' : '注册 daemon'} open={enrollOpen} onClose={() => setEnrollOpen(false)} closable={!enroll.isPending} footer={command ? <Button tone="primary" onClick={() => setEnrollOpen(false)}>完成</Button> : <><Button tone="quiet" onClick={() => setEnrollOpen(false)} disabled={enroll.isPending}>取消</Button><Button tone="primary" onClick={() => enroll.mutate()} disabled={!accountID || !daemonName || enroll.isPending}>生成命令</Button></>}>
      {command ? <><Notice tone="warning">注册口令只可消费一次。</Notice><div className="command-box"><code>{command}</code><Button autoFocus onClick={() => navigator.clipboard.writeText(command)}><Copy size={16} />复制</Button></div></> : <div className="form-grid"><Field label="账号 ID" hint="必须是已经注册且状态正常的账号"><Input required value={accountID} onChange={(event) => setAccountID(event.target.value)} /></Field><Field label="daemon 名称"><Input required value={daemonName} onChange={(event) => setDaemonName(event.target.value)} /></Field>{enroll.error && <p className="form-error" role="alert">无法生成 daemon 注册命令，请检查账号和名称后重试。{enroll.error instanceof APIError && enroll.error.correlationID ? ` 关联 ID：${enroll.error.correlationID}` : ''}</p>}</div>}
    </Dialog>
