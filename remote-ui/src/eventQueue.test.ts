...
describe('ConnectionEventQueue', () => {
  it('flushes transport/lifecycle events in sequence order and drops duplicates', () => {
    const queue = new ConnectionEventQueue({ maxSize: 8 })

    expect(queue.enqueue({ id: 'e2', sequence: 2, message: { type: 'transport.connecting', mode: 'local' } })).toBe(true)
    expect(queue.enqueue({ id: 'e1', sequence: 1, message: { type: 'user.connectMachine', machineId: 'machine-local' } })).toBe(true)
    expect(queue.enqueue({ id: 'e2', sequence: 2, message: { type: 'transport.connected', mode: 'local', connectionId: 'conn-ignored' } })).toBe(false)
  ...
  it('applies backpressure by keeping lifecycle/user-intent events and dropping stale transport chatter first', () => {
    ...
    queue.enqueue({ id: 'connecting', sequence: 2, message: { type: 'transport.connecting', mode: 'local' } })
    queue.enqueue({ id: 'noise-1', sequence: 3, message: { type: 'transport.disconnected', reason: 'brief' } })
  ...
  it('keeps messages transport-interface shaped without browser or native implementation leakage', () => {
