...
func TestPairCreateAndImportUsesTicketThenClientBoundCredential(t *testing.T) {
	...
	if bytes.Contains(registryPayload, created) || strings.Contains(string(registryPayload), "capability_grant") || strings.Contains(string(registryPayload), "muxvia-grant-v2") {
	...
	credential, err := remoteauth.NewCredentialStore(filepath.Join(stateHome, "muxvia", "remote-v2", "credentials")).Resolve(route.CredentialRef)
	if err != nil {
		t.Fatal(err)
	}
...
func assertPairImportLocksReleased(t *testing.T, stateHome string, registryPath string, endpointID endpointdomain.EndpointID, bundle *remoteauth.PairingBundle) {
	...
	credentialStore := remoteauth.NewCredentialStore(filepath.Join(stateHome, "muxvia", "remote-v2", "credentials"))
...
func configurePairCommandTest(t *testing.T) (string, string, string) {
	t.Helper()
	t.Setenv("MUXVIA_DIRECT_SIGNALING_LISTEN", "127.0.0.1:41120")
	t.Setenv("MUXVIA_DIRECT_ICE_TCP_LISTEN", "127.0.0.1:41121")
