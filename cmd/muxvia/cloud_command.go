...
func cloudCommand() *cobra.Command {
	command := &cobra.Command{Use: "cloud", Short: "Manage Muxvia Cloud enrollment", Args: cobra.NoArgs}
...
func cloudEnrollCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use: "enroll CODE", Short: "Enroll this daemon DeviceIdentity into Muxvia Cloud", Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("enroll daemon in Muxvia Cloud: %w", err)
			}
