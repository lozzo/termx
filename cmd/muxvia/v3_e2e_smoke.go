...
func runV3E2ESmoke(ctx context.Context) (v3E2ESmokeResult, error) {
	socketDir, err := os.MkdirTemp("", "muxvia-v3-smoke-*")
	if err != nil {
		return v3E2ESmokeResult{}, err
	}
	defer os.RemoveAll(socketDir)
	socketPath := filepath.Join(socketDir, fmt.Sprintf("muxvia-v2-smoke-%d.sock", time.Now().UnixNano()))
...
func validateV3E2EStyledChrome(frames []render.Frame) error {
	...
	frame := frames[len(frames)-1]
	required := []string{"  main", "▎ 1 main ", "󰐕", "v3-e2e-smoke", "◆ owner", "ws:main"}
