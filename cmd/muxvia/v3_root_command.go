...
const v3RootTerminalID = "muxvia-main"
...
func runV3RootCommand(cmd *cobra.Command, socket string, logFile string, configPath string) error {
	if !isInteractiveTerminal() {
		return fmt.Errorf("muxvia TUI requires an interactive terminal; use `muxvia --help` or subcommands like `new`, `ls`, `attach`, `kill`, `rm`, `daemon`")
	}
