...
func v3CloudCommand() *cobra.Command {
	command := &cobra.Command{Use: "cloud", Short: "Manage the optional Muxvia Cloud Companion"}
...
func v3CloudUserError(err error) error {
	...
	return cloudcompanion.NewError(
		...
		"Muxvia Cloud is optional and Cloud Companion is not bundled with muxvia by default. Run `muxvia cloud install` to enable cloud features; local and SSH features remain available.",
	)
}
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			flow, err := client.BeginLogin(cmd.Context(), &cloudpb.BeginLoginRequest{
				...
				ClientMetadata: &cloudpb.DeviceMetadata{
					...
					MuxviaVersion: muxviaBuildVersion,
				},
			})
			if err != nil {
				return err
