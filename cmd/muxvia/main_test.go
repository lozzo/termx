...
import (
	...
	tuiinput "github.com/muxvia/muxvia/tui/input"
	tuiservices "github.com/muxvia/muxvia/tui/port"
...
func TestRootCmdRoutesToTUIv3ByDefault(t *testing.T) {
	...
	socketPath := filepath.Join(t.TempDir(), "muxvia-v2.sock")
	logPath := filepath.Join(t.TempDir(), "muxvia.log")
	configHome := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", configHome)
	legacyDir := filepath.Join(configHome, "muxvia")
	...
	configPath := filepath.Join(configHome, "muxvia", "muxvia.yaml")
...
`)
	...
	cmd := newRootCmd()
	cmd.SetArgs([]string{"--log-file", filepath.Join(t.TempDir(), "muxvia.log")})
	cmd.SetIn(bytes.NewBuffer(nil))
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
...
`)
	...
	cmd := newRootCmd()
	cmd.SetArgs([]string{"--socket", flagSocket, "--log-file", filepath.Join(t.TempDir(), "muxvia.log")})
	cmd.SetIn(bytes.NewBuffer(nil))
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
...
func TestRootCmdBlocksNestedTUIByDefault(t *testing.T) {
	...
	err := cmd.Execute()
	if err == nil || !strings.Contains(err.Error(), "refusing to start muxvia TUI inside a muxvia remote terminal") {
...
func TestAttachCmdAllowsNestedTUIWhenOverrideIsSet(t *testing.T) {
	...
	t.Setenv("MUXVIA", "1")
	t.Setenv("MUXVIA_ALLOW_NESTED", "1")
...
func TestV3SmokeRunsTUIv3Smoke(t *testing.T) {
	...
	text := out.String()
	if !strings.Contains(text, "muxvia v3 smoke ok") ||
...
func TestV3SmokeCommandIncludesVisualReviewCases(t *testing.T) {
	...
	for _, want := range []string{
		"muxvia v3 smoke ok: tui=tui cases=12",
...
func TestDefaultDaemonUsesCoreV2Server(t *testing.T) {
	...
	cmd := newRootCmd()
	cmd.SetArgs([]string{"--socket", filepath.Join(t.TempDir(), "muxvia-v2.sock"), "--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "daemon"})
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
...
func TestDaemonCanDisableHistoryFromEnv(t *testing.T) {
	...
	t.Setenv("MUXVIA_HISTORY_DISABLE", "1")
...
func TestR446DaemonConfiguresHistoryBackpressureFromEnv(t *testing.T) {
	...
	t.Setenv("MUXVIA_HISTORY_BACKPRESSURE_MODE", "bounded")
	t.Setenv("MUXVIA_HISTORY_BACKPRESSURE_BUFFER_MB", "12")
...
func TestV3PingConnectsExistingCoreV2Daemon(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"--socket", socketPath, "--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "v3", "ping"})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(out.String(), "muxvia v3 daemon ok") || !strings.Contains(out.String(), socketPath) {
...
func TestV3PingAutoStartsCoreV2Daemon(t *testing.T) {
	...
	if !strings.Contains(out.String(), "muxvia v3 daemon ok") {
		t.Fatalf("unexpected v3 ping output:\n%s", out.String())
	}
}
...
func TestV3PingReturnsAutoStartError(t *testing.T) {
	...
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"--socket", filepath.Join(t.TempDir(), "muxvia-v2.sock"), "--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "v3", "ping"})
	cmd.SetOut(io.Discard)
	cmd.SetErr(io.Discard)
...
func TestStartCoreV2DaemonCommandUsesV3Daemon(t *testing.T) {
	...
	exe := filepath.Join(t.TempDir(), "muxvia")
	osExecutable = func() (string, error) {
		return exe, nil
	}
	got, err := buildStartCoreV2DaemonCommand("/tmp/muxvia-v2.sock", "/tmp/muxvia.log")
	if err != nil {
	...
	wantArgs := []string{exe, "--socket", "/tmp/muxvia-v2.sock", "--log-file", "/tmp/muxvia.log", "daemon"}
...
func TestStartCoreV2DaemonCommandCarriesHistoryDisableEnv(t *testing.T) {
	...
	if !containsEnv(got.Env, "MUXVIA_HISTORY_DISABLE=1") {
...
func TestStartCoreV2DaemonCommandCanCarryExplicitConfigPath(t *testing.T) {
	...
	exe := filepath.Join(t.TempDir(), "muxvia")
	configPath := filepath.Join(t.TempDir(), "muxvia.yaml")
	osExecutable = func() (string, error) {
		return exe, nil
	}
	got, err := buildStartCoreV2DaemonCommandWithConfig("/tmp/muxvia-v2.sock", "/tmp/muxvia.log", configPath)
	if err != nil {
		t.Fatalf("buildStartCoreV2DaemonCommandWithConfig returned error: %v", err)
	}
	wantArgs := []string{exe, "--socket", "/tmp/muxvia-v2.sock", "--log-file", "/tmp/muxvia.log", "--config", configPath, "daemon"}
	if !reflect.DeepEqual(got.Args, wantArgs) {
...
func TestDialOrStartV3ClientUsesConfigStarterWhenConfigPathIsExplicit(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "muxvia.yaml")
	socketPath := filepath.Join(t.TempDir(), "muxvia.sock")
	logPath := filepath.Join(t.TempDir(), "muxvia.log")
	startV3Daemon = func(path string, logFile string) error {
...
func TestV3LocalControlCommandsRemainAvailable(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs(append([]string{"--socket", socketPath, "--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "v3", "new", "--name", "v3-demo", "--"}, testShellSleepCommand()...))
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	if err := cmd.Execute(); err != nil {
...
func TestR448V3HistoryBacklogWritesDiagnostics(t *testing.T) {
	...
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"--socket", socketPath, "--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "v3", "history-backlog", "term-backlog", "--out", outPath})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	if err := cmd.Execute(); err != nil {
	...
	if !strings.Contains(out.String(), "muxvia v3 history backlog ok") || !strings.Contains(out.String(), outPath) {
...
func TestV3AttachRejectsNonInteractiveTerminal(t *testing.T) {
	...
	err := cmd.Execute()
	if err == nil || !strings.Contains(err.Error(), "muxvia terminal attach requires an interactive terminal") {
...
func TestV3RootEmptyRuntimeEnablesPerfTrace(t *testing.T) {
	...
	t.Setenv("MUXVIA_PERF_TRACE", tracePath)
	t.Setenv("MUXVIA_PERF_TRACE_INTERVAL_MS", "10000")
...
func TestV3E2ESmokeCommandRunsLocalCoreAndTUIPath(t *testing.T) {
	...
	text := out.String()
	for _, want := range []string{"muxvia v3 e2e smoke ok", "copy_cols=98", "zoom_checked=true"} {
		if !strings.Contains(text, want) {
...
func TestV3TmuxSmokeHarnessCapturesArtifact(t *testing.T) {
	...
	if !strings.Contains(result.Captured, "muxvia tmux harness ready") || !strings.Contains(result.Captured, "muxvia tmux input:hello-from-tmux") {
...
func TestV3TmuxSmokeCommandReportsArtifactPaths(t *testing.T) {
	...
	if !strings.Contains(text, "muxvia v3 tmux smoke ok") ||
		!strings.Contains(text, "session=muxvia-v3-smoke-") ||
...
func TestV3TmuxTerminalSmokeCreatesAttachesAndSendsInput(t *testing.T) {
	...
	muxviaBin := buildMuxviaBinaryForTest(t)
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	result, err := runV3TmuxTerminalSmoke(ctx, muxviaBin)
	if err != nil {
	...
	if !strings.Contains(result.Captured, "muxvia-pty-ready") || !strings.Contains(result.Captured, "muxvia-pty-echo:tmux-live-input") {
...
func TestV3TmuxTerminalSmokeCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-terminal-smoke", "--muxvia-bin", muxviaBin})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux terminal smoke ok") ||
		!strings.Contains(text, "terminal=tmux-e2e") ||
		!strings.Contains(text, "session=muxvia-v3-terminal-") ||
...
func TestV3TmuxResizeSmokePropagatesHostResizeToPTY(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	result, err := runV3TmuxResizeSmoke(ctx, muxviaBin)
	if err != nil {
	...
	if !strings.Contains(result.Captured, "muxvia-pty-size:size-before:") || !strings.Contains(result.Captured, "muxvia-pty-size:size-after:"+result.AfterSize) {
...
func TestV3TmuxResizeSmokeCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-resize-smoke", "--muxvia-bin", muxviaBin})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux resize smoke ok") ||
		!strings.Contains(text, "terminal=tmux-resize") ||
		!strings.Contains(text, "session=muxvia-v3-resize-") ||
...
func TestV3TmuxANSISmokeCapturesLiveSurfaceSGR(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	result, err := runV3TmuxANSISmoke(ctx, muxviaBin)
	if err != nil {
...
func TestV3TmuxANSISmokeCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-ansi-smoke", "--muxvia-bin", muxviaBin})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux ansi smoke ok") ||
		!strings.Contains(text, "terminal=tmux-ansi") ||
		!strings.Contains(text, "session=muxvia-v3-ansi-") ||
		!strings.Contains(text, "artifact_dir=") ||
		!strings.Contains(text, "ansi=") ||
		!strings.Contains(text, "plain=") ||
		!strings.Contains(text, "timeline=") {
...
func TestV3TmuxEmojiDotsSmokeReproducesOwnerFollowerGeometry(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 20*time.Second)
	defer cancel()
	result, err := runV3TmuxEmojiDotsSmoke(ctx, muxviaBin)
	if err != nil {
	...
	for _, marker := range []string{"owner", "follow", "muxvia-pty-size:size-before:", "muxvia-pty-size:size-after:", "muxvia-pty-echo:", "····"} {
		if !strings.Contains(result.Captured, marker) {
...
func TestV3TmuxEmojiDotsSmokeCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-emoji-dots-smoke", "--muxvia-bin", muxviaBin})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux emoji dots smoke ok") ||
		!strings.Contains(text, "terminal=tmux-emoji-dots") ||
		!strings.Contains(text, "session=muxvia-v3-emoji-dots-") ||
...
func TestV3TmuxVisualCompareCapturesTargetAndDiffArtifacts(t *testing.T) {
	...
	defer cancel()
	result, err := runV3TmuxVisualCompare(ctx, muxviaBin)
	if err != nil {
...
func TestV3TmuxVisualCompareCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-visual-compare", "--muxvia-bin", muxviaBin})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux visual compare ok") ||
		!strings.Contains(text, "session=muxvia-v3-visual-") ||
...
func TestV3TmuxStabilitySmokeRunsShortRound(t *testing.T) {
	...
	defer cancel()
	result, err := runV3TmuxStabilitySmoke(ctx, muxviaBin, 1)
	if err != nil {
...
func TestV3TmuxStabilitySmokeCommandReportsArtifacts(t *testing.T) {
	...
	var out bytes.Buffer
	cmd := newDevelopmentRootCmd()
	cmd.SetArgs([]string{"v3", "tmux-stability-smoke", "--muxvia-bin", muxviaBin, "--rounds", "1"})
	cmd.SetOut(&out)
	cmd.SetErr(io.Discard)
	...
	if !strings.Contains(text, "muxvia v3 tmux stability smoke ok") ||
...
func buildMuxviaBinaryForTest(t *testing.T) string {
	...
	if err != nil {
		t.Fatalf("build muxvia test binary: %v\n%s", err, output)
	}
	return path
}
