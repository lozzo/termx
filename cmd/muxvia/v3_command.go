...
func v3PingCommand(socket *string, logFile *string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "muxvia v3 daemon ok: socket=%s\n", socketPath)
			return nil
		},
	}
}
...
func v3SmokeCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "muxvia v3 smoke ok: tui=%s cases=%d\n", tuiv3.ModuleName, len(result.Cases))
...
func v3E2ESmokeCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 e2e smoke ok: terminal=%s frames=%d viewport=%dx%d session=%dx%d copy_cols=%d pane_commands=%d panes=%d active=%s zoom_checked=%v\n",
				result.TerminalID,
				result.Frames,
				result.ViewportCols,
				result.ViewportRows,
...
func v3TmuxSmokeCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux smoke ok: session=%s input=%s artifact_dir=%s ansi=%s plain=%s\n",
				result.Session,
				result.SentInput,
				result.ArtifactDir,
				result.ANSIPath,
...
func v3TmuxTerminalSmokeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxTerminalSmoke(cmd.Context(), muxviaBin)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux terminal smoke ok: terminal=%s session=%s input=%s artifact_dir=%s ansi=%s plain=%s daemon_log=%s socket=%s timeline=%s\n",
				result.TerminalID,
				result.Session,
				result.SentInput,
				result.ArtifactDir,
	...
	cmd.Flags().StringVar(&muxviaBin, "muxvia-bin", "", "muxvia binary path to run inside tmux")
	return cmd
}
...
func v3TmuxResizeSmokeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxResizeSmoke(cmd.Context(), muxviaBin)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux resize smoke ok: terminal=%s session=%s window=%s before=%s after=%s artifact_dir=%s ansi=%s plain=%s daemon_log=%s socket=%s timeline=%s\n",
				result.TerminalID,
				result.Session,
				result.WindowSize,
				result.BeforeSize,
...
func v3TmuxANSISmokeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxANSISmoke(cmd.Context(), muxviaBin)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux ansi smoke ok: terminal=%s session=%s artifact_dir=%s ansi=%s plain=%s daemon_log=%s socket=%s timeline=%s\n",
				result.TerminalID,
				result.Session,
				result.ArtifactDir,
				result.ANSIPath,
...
func v3TmuxEmojiDotsSmokeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxEmojiDotsSmoke(cmd.Context(), muxviaBin)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux emoji dots smoke ok: terminal=%s session=%s before=%s after=%s dots=%v artifact_dir=%s ansi=%s plain=%s daemon_log=%s socket=%s timeline=%s\n",
				result.TerminalID,
				result.Session,
				result.BeforeSize,
				result.AfterSize,
...
func v3TmuxVisualCompareCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxVisualCompare(cmd.Context(), muxviaBin)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux visual compare ok: session=%s mismatches=%d style_mismatches=%d stylemap_mismatches=%d artifact_dir=%s current_plain=%s current_ansi=%s target=%s diff=%s style=%s style_diff=%s current_stylemap=%s target_stylemap=%s stylemap_diff=%s\n",
				result.Session,
				result.Mismatches,
...
func v3TmuxStabilitySmokeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			result, err := runV3TmuxStabilitySmoke(cmd.Context(), muxviaBin, rounds)
			if err != nil {
				return err
			}
			fmt.Fprintf(
				cmd.OutOrStdout(),
				"muxvia v3 tmux stability smoke ok: rounds=%d artifacts=%d artifact_dir=%s timeline=%s\n",
				result.Rounds,
