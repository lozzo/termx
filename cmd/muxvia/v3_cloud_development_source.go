...
func v3BundledDevelopmentCompanionSource() (activation.InstallationSource, bool, error) {
	...
	if muxviaBuildVersion != v3DevelopmentBuildVersion || name == "" || filepath.Base(name) != name || name == "." || digest == "" || version == "" || channel != "development" {
	...
	if err != nil {
		return nil, false, companionDevelopmentTrustError("muxvia executable path cannot be resolved")
	}
	realExecutable, err := filepath.EvalSymlinks(executable)
	if err != nil {
		return nil, false, companionDevelopmentTrustError("muxvia executable path cannot be verified")
	}
...
func (source *v3BundledDevelopmentSource) Status() (installer.Installation, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return installer.Installation{}, cloudcompanion.NewError(cloudpb.CloudErrorCode_CLOUD_ERROR_CODE_COMPANION_MISSING, "development Cloud Companion is missing beside muxvia; rebuild with `make build-cloud-test`")
		}
	...
	if copyErr != nil || closeErr != nil || !strings.EqualFold(hex.EncodeToString(hash.Sum(nil)), source.installation.BinarySHA256) {
		return installer.Installation{}, companionDevelopmentTrustError("development Cloud Companion does not match the muxvia build")
	}
