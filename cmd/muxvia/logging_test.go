...
func TestResolveLogFilePathPrefersExplicitValue(t *testing.T) {
	t.Setenv("MUXVIA_LOG_FILE", filepath.Join(t.TempDir(), "ignored.log"))
	got := resolveLogFilePath("/tmp/muxvia-explicit.log")
	if got != "/tmp/muxvia-explicit.log" {
		t.Fatalf("expected explicit log path to win, got %q", got)
	}
}

func TestResolveLogFilePathUsesEnvironmentOverride(t *testing.T) {
	want := filepath.Join(t.TempDir(), "muxvia-env.log")
	t.Setenv("MUXVIA_LOG_FILE", want)
	if got := resolveLogFilePath(""); got != want {
		t.Fatalf("expected MUXVIA_LOG_FILE path %q, got %q", want, got)
	}
}

func TestResolveLogFilePathFallsBackToXDGStateHome(t *testing.T) {
	base := t.TempDir()
	t.Setenv("MUXVIA_LOG_FILE", "")
	t.Setenv("XDG_STATE_HOME", base)
	got := resolveLogFilePath("")
	want := filepath.Join(base, "muxvia", "muxvia.log")
	if got != want {
...
func TestResolveWorkspaceStatePathFallsBackToXDGStateHome(t *testing.T) {
	...
	want := filepath.Join(base, "muxvia", "workspace-state.json")
	if got != want {
...
func TestResolveGridStatePathPrefersEnvironmentOverride(t *testing.T) {
	...
	t.Setenv("MUXVIA_GRID_DIR", want)
	if got := resolveGridStatePath(); got != want {
		t.Fatalf("expected MUXVIA_GRID_DIR path %q, got %q", want, got)
	}
}

func TestResolveGridStatePathFallsBackToXDGStateHome(t *testing.T) {
	base := t.TempDir()
	t.Setenv("MUXVIA_GRID_DIR", "")
	t.Setenv("XDG_STATE_HOME", base)
	got := resolveGridStatePath()
	want := filepath.Join(base, "muxvia", "grid")
	if got != want {
...
func TestV3PathPolicy(t *testing.T) {
	...
	if got := resolveV3Socket(""); got != filepath.Join(runtimeDir, fmt.Sprintf("muxvia-v2-wire%d.sock", wire.Version)) {
	...
	if got := resolveV3LogFilePath(""); got != filepath.Join(stateHome, "muxvia", "muxvia.log") {
		t.Fatalf("expected v3 log path to reuse global log policy, got %q", got)
	}
	if got := v3ConfigPathPolicy(); got != filepath.Join(configHome, "muxvia", "tui-v3.yaml") {
...
func TestV3AttachDoesNotCreateTuiv2Config(t *testing.T) {
	...
	cmd.SetArgs([]string{"--log-file", filepath.Join(t.TempDir(), "muxvia.log"), "v3", "attach", "term-1"})
...
func TestV3AttachLoadsTUIV3Config(t *testing.T) {
	...
	configHome := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", configHome)
	configDir := filepath.Join(configHome, "muxvia")
...
func TestOpenLogFileLoggerCreatesFileAndWrites(t *testing.T) {
	path := filepath.Join(t.TempDir(), "logs", "muxvia.log")
