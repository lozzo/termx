...
func TestDaemonServiceLifecycleUsesExactRuntimeRecord(t *testing.T) {
	binary := buildMuxviaBinaryForTest(t)
	runtimeDir := t.TempDir()
	socketPath := filepath.Join(runtimeDir, "muxvia.sock")
	logPath := filepath.Join(runtimeDir, "muxvia.log")
	t.Cleanup(func() {
		stop := exec.Command(binary, "--socket", socketPath, "--log-file", logPath, "daemon", "stop")
		_, _ = stop.CombinedOutput()
	})

	stopped := executeMuxviaBinary(t, binary, "--socket", socketPath, "--log-file", logPath, "daemon", "status", "--json")
	if stopped.State != "stopped" || stopped.PID != 0 {
		t.Fatalf("initial daemon status = %#v", stopped)
	}
	started := executeMuxviaBinary(t, binary, "--socket", socketPath, "--log-file", logPath, "daemon", "start", "--json")
	...
	restarted := executeMuxviaBinary(t, binary, "--socket", socketPath, "--log-file", logPath, "daemon", "status", "--json")
	...
	stopped = executeMuxviaBinary(t, binary, "--socket", socketPath, "--log-file", logPath, "daemon", "stop", "--json")
