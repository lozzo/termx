...
func runV3TmuxSmoke(ctx context.Context) (v3TmuxSmokeResult, error) {
	...
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-smoke-*")
	if err != nil {
		return v3TmuxSmokeResult{}, err
	}
	session := fmt.Sprintf("muxvia-v3-smoke-%d", time.Now().UnixNano())
	target := session + ":0.0"
	cleanup := func() {
	...
	script := "printf 'muxvia tmux harness ready\\n'; " +
		"IFS= read -r line; " +
		"printf 'muxvia tmux input:%s\\n' \"$line\"; " +
		"sleep 30"
	if err := runTmuxCommand(ctx, "new-session", "-d", "-x", "100", "-y", "30", "-s", session, "/bin/sh", "-lc", script); err != nil {
		_ = os.RemoveAll(artifactDir)
		return v3TmuxSmokeResult{}, err
	}
	if err := waitForTmuxCapture(ctx, target, "muxvia tmux harness ready", 2*time.Second); err != nil {
		_ = os.RemoveAll(artifactDir)
		return v3TmuxSmokeResult{}, err
	}
	...
	if err := waitForTmuxCapture(ctx, target, "muxvia tmux input:"+sent, 2*time.Second); err != nil {
		_ = os.RemoveAll(artifactDir)
		return v3TmuxSmokeResult{}, err
	}
...
func runV3TmuxTerminalSmoke(ctx context.Context, muxviaBin string) (v3TmuxTerminalSmokeResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxTerminalSmokeResult{}, fmt.Errorf("tmux terminal smoke requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxTerminalSmokeResult{}, fmt.Errorf("tmux terminal smoke requires a muxvia binary path")
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-terminal-*")
	if err != nil {
	...
	session := fmt.Sprintf("muxvia-v3-terminal-%d", time.Now().UnixNano())
	target := session + ":0.0"
	...
	defer stopDaemonCtx()
	daemonCmd := exec.CommandContext(daemonCtx, muxviaBin, "--socket", socketPath, "--log-file", daemonLog, "daemon")
	...
	terminalScript := "printf 'muxvia-pty-ready\\n'; while IFS= read -r line; do printf 'muxvia-pty-echo:%s\\n' \"$line\"; done"
	script := strings.Join([]string{
		...
		"muxvia_bin=" + shellQuote(muxviaBin),
		...
		"printf 'muxvia tmux terminal harness ready\\n'",
		"id=\"$($muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 new --name tmux-e2e -- /bin/sh -c " + shellQuote(terminalScript) + ")\"",
		"printf '%s\\n' \"$id\" > \"$terminal_id_path\"",
		"printf 'muxvia terminal id:%s\\n' \"$id\"",
		"$muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 ls > \"$ls_before_path\"",
		"exec $muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 attach \"$id\"",
		"",
	}, "\n")
	...
	if err := waitForTmuxCapture(ctx, target, "muxvia-pty-ready", 5*time.Second); err != nil {
	...
	if err := waitForTmuxCapture(ctx, target, "muxvia-pty-echo:"+sent, 5*time.Second); err != nil {
		return v3TmuxTerminalSmokeResult{}, err
	}
	...
	_ = runMuxviaCommand(context.Background(), muxviaBin, "--socket", socketPath, "--log-file", daemonLog, "v3", "kill", terminalID)
	_ = runMuxviaCommand(context.Background(), muxviaBin, "--socket", socketPath, "--log-file", daemonLog, "v3", "rm", terminalID)
...
func runV3TmuxResizeSmoke(ctx context.Context, muxviaBin string) (v3TmuxResizeSmokeResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxResizeSmokeResult{}, fmt.Errorf("tmux resize smoke requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxResizeSmokeResult{}, fmt.Errorf("tmux resize smoke requires a muxvia binary path")
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-resize-*")
	if err != nil {
	...
	daemonLog := filepath.Join(artifactDir, "daemon.log")
	timelinePath := filepath.Join(artifactDir, "timeline.txt")
	session := fmt.Sprintf("muxvia-v3-resize-%d", time.Now().UnixNano())
	target := session + ":0.0"
	appendTimeline := func(format string, args ...any) {
		line := fmt.Sprintf(format, args...) + "\n"
		_ = appendFile(timelinePath, line)
	...
	terminalScript := "printf 'muxvia-pty-ready\\n'\n" +
		...
		"  case \"$line\" in\n" +
		"    size-*) set -- $(stty size); printf 'muxvia-pty-size:%s:%sx%s\\n' \"$line\" \"$2\" \"$1\" ;;\n" +
		"    *) printf 'muxvia-pty-echo:%s\\n' \"$line\" ;;\n" +
		"  esac\n" +
		"done\n"
	script := strings.Join([]string{
		"#!/bin/sh",
		...
		"log_file=" + shellQuote(daemonLog),
		"terminal_id_path=" + shellQuote(terminalIDPath),
		"printf 'muxvia tmux resize harness ready\\n'",
		"id=\"$($muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 new --name tmux-resize -- /bin/sh -c " + shellQuote(terminalScript) + ")\"",
		"printf '%s\\n' \"$id\" > \"$terminal_id_path\"",
		"printf 'muxvia resize terminal id:%s\\n' \"$id\"",
		"exec $muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 attach \"$id\"",
		"",
	}, "\n")
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
	...
	beforeMarker := "muxvia-pty-size:size-before:"
	...
	afterMarker := "muxvia-pty-size:size-after:"
...
func runV3TmuxANSISmoke(ctx context.Context, muxviaBin string) (v3TmuxANSISmokeResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxANSISmokeResult{}, fmt.Errorf("tmux ansi smoke requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxANSISmokeResult{}, fmt.Errorf("tmux ansi smoke requires a muxvia binary path")
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-ansi-*")
	if err != nil {
	...
	daemonLog := filepath.Join(artifactDir, "daemon.log")
	timelinePath := filepath.Join(artifactDir, "timeline.txt")
	session := fmt.Sprintf("muxvia-v3-ansi-%d", time.Now().UnixNano())
	target := session + ":0.0"
	appendTimeline := func(format string, args ...any) {
		line := fmt.Sprintf(format, args...) + "\n"
		_ = appendFile(timelinePath, line)
	...
	terminalScript := "printf 'muxvia-ansi-ready\\n'\n" +
		...
		"while IFS= read -r line; do printf 'muxvia-ansi-echo:%s\\n' \"$line\"; done\n"
	script := strings.Join([]string{
		"#!/bin/sh",
		"set -eu",
		"export TERM=xterm-256color",
		...
		"log_file=" + shellQuote(daemonLog),
		"terminal_id_path=" + shellQuote(terminalIDPath),
		"printf 'muxvia tmux ansi harness ready\\n'",
		"id=\"$($muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 new --name tmux-ansi -- /bin/sh -c " + shellQuote(terminalScript) + ")\"",
		"printf '%s\\n' \"$id\" > \"$terminal_id_path\"",
		"printf 'muxvia ansi terminal id:%s\\n' \"$id\"",
		"exec $muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 attach \"$id\"",
		"",
	}, "\n")
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
	...
	terminalID := strings.TrimSpace(string(terminalIDBytes))
	appendTimeline("terminal created id=%s", terminalID)
	for _, marker := range []string{"muxvia-ansi-ready", "ANSI16_RED", "ANSI256_ORANGE", "TRUECOLOR_MINT", "CR_REPLACED", "PRIMARY_AFTER_ALT"} {
...
func runV3TmuxEmojiDotsSmoke(ctx context.Context, muxviaBin string) (v3TmuxEmojiDotsSmokeResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxEmojiDotsSmokeResult{}, fmt.Errorf("tmux emoji dots smoke requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxEmojiDotsSmokeResult{}, fmt.Errorf("tmux emoji dots smoke requires a muxvia binary path")
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-emoji-dots-*")
	if err != nil {
	...
	daemonLog := filepath.Join(artifactDir, "daemon.log")
	timelinePath := filepath.Join(artifactDir, "timeline.txt")
	session := fmt.Sprintf("muxvia-v3-emoji-dots-%d", time.Now().UnixNano())
	target := session + ":0.0"
	appendTimeline := func(format string, args ...any) {
		line := fmt.Sprintf(format, args...) + "\n"
		_ = appendFile(timelinePath, line)
	...
	script := strings.Join([]string{
		...
		"log_file=" + shellQuote(daemonLog),
		"terminal_id_path=" + shellQuote(terminalIDPath),
		"printf 'muxvia tmux emoji dots harness ready\\n'",
		"id=\"$($muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 new --name tmux-emoji-dots -- /bin/sh -c " + shellQuote(terminalScript) + ")\"",
		"printf '%s\\n' \"$id\" > \"$terminal_id_path\"",
		"printf 'muxvia emoji terminal id:%s\\n' \"$id\"",
		"exec $muxvia_bin --socket \"$socket\" --log-file \"$log_file\" v3 attach \"$id\"",
		"",
	}, "\n")
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
	...
	if err := waitForTmuxCapture(ctx, target, "muxvia-pty-echo:", 5*time.Second); err != nil {
		return v3TmuxEmojiDotsSmokeResult{}, err
	}
	...
	if !strings.Contains(plain, "muxvia-pty-echo:") || !strings.Contains(plain, "follow") || !strings.Contains(plain, "owner") {
...
func runV3TmuxVisualCompare(ctx context.Context, muxviaBin string) (v3TmuxVisualCompareResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxVisualCompareResult{}, fmt.Errorf("tmux visual compare requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxVisualCompareResult{}, fmt.Errorf("tmux visual compare requires a muxvia binary path")
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-visual-*")
	if err != nil {
	...
	session := fmt.Sprintf("muxvia-v3-visual-%d", time.Now().UnixNano())
	target := session + ":0.0"
	...
	script := strings.Join([]string{
		...
		"$muxvia_bin v3 visual-snapshot --ansi",
		"sleep 30",
		"",
	}, "\n")
	if err := os.WriteFile(scriptPath, []byte(script), 0o700); err != nil {
...
func v3VisualTargetPlain() string {
	lines := []string{
		...
		"│muxvia git:core-tui-v3-migration  go v1.26.0                                      ····················│ visual review baseline       ·····│",
...
func runV3TmuxStabilitySmoke(ctx context.Context, muxviaBin string, rounds int) (v3TmuxStabilitySmokeResult, error) {
	if _, err := exec.LookPath("tmux"); err != nil {
		return v3TmuxStabilitySmokeResult{}, fmt.Errorf("tmux stability smoke requires tmux in PATH: %w", err)
	}
	if muxviaBin == "" {
		return v3TmuxStabilitySmokeResult{}, fmt.Errorf("tmux stability smoke requires a muxvia binary path")
	}
	if rounds <= 0 {
		rounds = 1
	}
	artifactDir, err := os.MkdirTemp("", "muxvia-v3-tmux-stability-*")
	if err != nil {
	...
	for round := 1; round <= rounds; round++ {
		...
		terminal, err := runV3TmuxTerminalSmoke(ctx, muxviaBin)
		if err != nil {
		...
		resize, err := runV3TmuxResizeSmoke(ctx, muxviaBin)
		if err != nil {
		...
		ansi, err := runV3TmuxANSISmoke(ctx, muxviaBin)
		if err != nil {
...
func runMuxviaCommand(ctx context.Context, muxviaBin string, args ...string) error {
	output, err := exec.CommandContext(ctx, muxviaBin, args...).CombinedOutput()
	if err != nil {
		return fmt.Errorf("muxvia %s: %w: %s", strings.Join(args, " "), err, strings.TrimSpace(string(output)))
	}
	return nil
}
