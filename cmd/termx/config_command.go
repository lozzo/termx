...
func newConfigCommand(configPath, socket, logFile *string) *cobra.Command {
	command := &cobra.Command{Use: "config", Short: "Inspect and modify the TermX client configuration"}
	command.AddCommand(
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				if errors.Is(err, os.ErrNotExist) {
					return &cliError{code: 3, message: fmt.Sprintf("config file %s does not exist; use `termx config set` to create it", path), cause: err}
				}
				return err
			}
			if jsonOutput {
...
func validateAndWriteConfig(path string, document *yaml.Node) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(path), ".termx-config-*")
	if err != nil {
		return err
	}
