...
- `[x]` 抽出第一版 shell-neutral client API 落点：`clientapi`，并让 `tuiv2/bridge` 退化为兼容 shim。
...
- `[x]` 抽出第一版 workbench doc codec 落点：`tuiv2/workbenchcodec`，并让 `tuiv2/sessionstate` 退化为兼容 shim。
