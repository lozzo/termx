...
PTY bytes / resize
  -> single SemanticTap
       owns vterm parser, cursor, modes, alt state, resize order, OSC/DA/DSR response
...
- truth source：同一个 `vterm` emulator 解释后的 `TerminalSemanticTransaction` 与同一 emulator 的 latest native screen。
