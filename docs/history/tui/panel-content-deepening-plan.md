...
- `core/history.Cell` 目前只有 `Text`。
- `core/history.VisualRow` 目前只有 `Text`。
- `core/protocol_service.go` 当前把 history row 输出为 `protocol.CompactRow{Text: row.Text}`。
...
- `go test ./tui/render` 覆盖裁切、占位、ANSI、宽字符。
- `go test ./tui/...` 覆盖 live projection、cursor、surface revision stale guard 和 live coalescing。
