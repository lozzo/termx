...
| `proto/remoteauthpb/remote_auth.proto` | E2E auth、CapabilityGrant、client access、endpoint bootstrap/share | remoteauth、endpoint contract、daemon/client auth | ownership 基本正确；Go alias/domain copy 仍需通过 transformer 收口 |
...
- `client/endpoint` registry、route planner 和 credential descriptor 的内部 domain model；发布/导入必须经 transformer 与 proto contract。
...
2. 建立 API Layer 和 core/proto transformer，不接 UI。
...
- 没有新增 schema、generated code、API Layer 或 transformer 实现。
