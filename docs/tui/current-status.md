...
- 本轮继续把 overlay dialog 的信息层次补出来：shell dialog 内部开始显式分成 `TITLE[...]` 和 `FOOTER[...]`，并补上 `terminal_manager -> Esc` 的 mask/dialog 清理与正文恢复场景
...
- 本轮继续把顶部状态条里的技术字段压缩成更接近导航语义的表达：`active:/role:/slot:` 与 `layer:/term:/mode:` 已替代之前更重的说明式文案，主界面首屏现在更像“导航条 + 工作台”而不是“说明条 + 工作台”
...
- 本轮继续把 modern 工作台正文往旧版 pane-first 布局收口：single pane 与 floating `window deck` 卡片头部现在统一改成“左标题 + 右 badge”单线标题栏，显式显示 `run/owner/top/z-order`，右侧侧栏标题也从 `Context & Keys` 收成 `Context`；对应 renderer 断言与默认 modern single-pane E2E 已同步补齐
- 本轮继续把默认 `78x24` 首屏从“上下堆叠说明 rail”切回 pane-first：single / split / floating / mixed 在默认宽度下不再优先走 stacked rail，而是改成“左侧 pane canvas + 右侧窄 sidebar”；同时把 mixed 标题显式收成 `MIXED`，并把窄侧栏里的 `ACTION` 优先级提到 `CMD/PEEK` 之前，避免默认高度下操作提示被预览文本挤掉
- 本轮继续把右侧窄 sidebar 往“真实控制面板”推进：split 与 mixed 现在都会额外渲染 `PANES` roster，显式列出 active pane、其余 tiled pane、以及 mixed 下的 floating pane；roster 会带 `> active`、`T/F`、`run/wait/exit` 与 owner/follower 缩写，默认 78 列首屏已经可以直接从侧栏读出当前 pane 拓扑，而不必只靠主画布猜测
...
170. 第一百六十六轮 TDD 已补上最小 floating mode 导航与 create 到 layout resolve 的工作台闭环
171. 第一百六十七轮 TDD 已补上 terminal manager 直达已连接 terminal pane 的 jump 主线
172. 第一百六十八轮 TDD 已补上 terminal manager 位置列表的精确 pane 跳转主线
173. 第一百六十九轮 TDD 已补上 floating pane 的最小 move 与 center 主线
174. 第一百七十轮 TDD 已补上 floating pane 的最小 resize 主线
175. 第一百七十一轮 TDD 已补上 floating pane 的最小 z-order / close 与 stack 投影主线
176. 第一百七十二轮 TDD 已补上 help overlay 与主界面 shortcut bar 主线
177. 第一百七十三轮 TDD 已补上主界面 workspace bar / tab strip / focus bar 主线
178. 第一百七十四轮 TDD 已补上主界面 workspace summary / pane bar 主线
179. 第一百七十五轮 TDD 已补上主界面 tiled outline 工作台概览主线
180. 第一百七十六轮 TDD 已补上主界面 tiled layout 摘要与 pane 运行预览主线
...
201. 第一百九十七轮 TDD 已补上现代 pane header/footer 条、floating top-window 摘要、overlay backdrop workbench 预览，并完成 shell-only / default-run / 全量回归闭环
...
- 默认 modern 的 top/context/footer chrome 也继续往产品导航语言收口：顶部 active summary 现在更偏 `control / windows`，context bar 更偏 `session / view / terminal`，footer 的 layer badge 和快捷键文案也从全大写技术标签压成了更接近产品导航的 `desk / dialog / Pane / Tab / Actions`
- modern overlay dialog 也已经开始脱离调试式 ASCII `#` 对话框，切到统一的 Unicode 盒模型
- 下一阶段可以继续沿着这一层推进：
  - 继续把 header/context/footer 里剩余仍然偏技术字段的 token 再压成更短、更像最终产品的导航语义
