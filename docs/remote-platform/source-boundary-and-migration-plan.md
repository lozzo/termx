# Muxvia Remote Platform 源码边界与迁移计划
...
### 3.1 Public namespace：未来复制到 `muxvia`
...
internal/protocol/          muxvia protocol implementation
...
```text
private/archive/muxvia-platform-legacy/muxvia-hub/
private/archive/muxvia-platform-legacy/web-control/
private/archive/muxvia-platform-legacy/muxvia-remote/
```

remote-ui 的历史 localweb 入口、旧 WebRTC 设计文档和对应测试保存在 `muxvia-remote/` archive 子目录；活动 `clients/ui/` 本身仍是公开 App/shared UI package。
...
| `private/archive/muxvia-platform-legacy/muxvia-hub/client/` | client 与 server module/内部 wire 耦合 | stream/signaling API 经验 | contract 已抽到 public namespace；旧 client 只留 archive |
| `private/archive/muxvia-platform-legacy/muxvia-hub/internal/hub/` | 非空 Bearer 即通过、长期 agent token、terminal inventory | TTL presence、offer/answer correlation、ICE/traffic 思路 | `private/cloud/hub/` 与 `relay/` 已按新模型重建；旧代码只留 archive |
| `private/archive/muxvia-platform-legacy/web-control/` | agent/server 限额、heartbeat kick、订阅直接控制在线状态 | 用户、订单、支付、管理 UI 和运维经验 | 只留 archive；活动服务已在 `private/cloud/` 重建 |
...
| `private/archive/muxvia-platform-legacy/muxvia-remote/` | 旧 remote/localweb 模型 | 历史行为和 UI 参考 | 只留 archive，不恢复依赖 |
