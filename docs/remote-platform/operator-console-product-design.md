...
- 保留 `/operator/*` 管理路由作为刷新、分享和历史恢复地址，但不建立第二套全屏应用壳。
- 登录后的账号项与获准运营模块共用唯一左侧栏，点击后只替换右侧内容。
...
- 不维护账号壳与运营壳两套品牌头、导航、语言、退出或 Workspace 请求。
...
- 登录完成后只进入一个 Web 控制台壳；左侧先展示概览、设备、套餐和账号，再展示后端 Workspace 投影允许的九个运营模块。
- 运营人员直接点击左侧“用户管理”“订单管理”等项目，右侧切换到对应列表、详情和表单；不要求手工输入 URL，也不进入独立全屏程序。
- `/operator` 本身不建设无排期的统计首页，默认重定向到 `/operator/users`。如果当前角色没有用户模块，则进入第一个获准模块。
- 普通账号看不到运营分组；直接访问 `/operator/*` 时仍由后端逐请求鉴权并回到账户内容。
...
- 页头展示模块标题、必要摘要、当前模块刷新和近期认证状态；语言与退出统一归属左侧栏。
- 正文最大化用于表格和详情，不使用营销型大标题、hero 或装饰卡片。

### 6.2 移动端布局

- 小于 768px 时左侧栏变成页头菜单按钮打开的全高抽屉。
- 抽屉包含账号项、获准运营模块、当前选中状态、语言和退出入口。
...
8. `OPSCONSOLE008`：删除账号/运营双壳，收敛为登录后唯一 `ConsolePage` 左栏与右侧内容 outlet。
...
- 新模块稳定后，删除账号页 hash 链接和单一跳转入口，把获准的九个模块直接放入统一左侧栏。
- 删除巨型 `OperatorPage` 的全量 `load()`、九模块同页渲染和重复局部状态。
- 删除 `OperatorPage` 与 `AccountPage` 各自持有的品牌头、侧栏、移动菜单、语言、退出和 Workspace 重复请求；这些职责只能由 `ConsolePage` 持有一次。
