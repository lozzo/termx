...
- 独立目录：`/opt/termx-file004`。
- 独立服务：`termx-file004-cloud.service`、`termx-file004-companion.service`、`termx-file004-daemon.service`。
- DeviceID：`device-v4-559N-ft-rcTVYBl1wKw`。
- TerminalID：`file004-public`。
- 服务器测试目录：`/opt/termx-file004/test-files`。
...
现有 `termx-hub`、`tgent-hub`、`termx-web-control` 和 3478/8447 监听保持运行；FILE004 服务不覆盖这些进程或端口。
...
- [x] 文件页浏览 `/opt/termx-file004/test-files`。
- [x] `preview.txt` 显示 `FILE004 public download verification` 与 `line-2`。
- [x] 下载 `download-source.bin` 到 `Download/termx/download-source.bin`；Android 与服务器 SHA-256 均为 `69708069862e7a31f3d449233f45c527a3156ee9dacf581e8cffe886cb7215e0`。
...
- [x] 下载 `cancel-download.bin` 后立即取消，`Download/termx/cancel-download.bin` 不存在，私有 `.part` 已删除。
- [x] 上传 `cancelupload` 后立即取消，服务器目标和 `.termx-upload-*` 均不存在。
...
ssh root@114.66.58.243 \
  'sha256sum /opt/termx-file004/test-files/download-source.bin /opt/termx-file004/test-files/<uploaded-file>'

adb logcat -s \
  TermxNativePlugin TermxConnStore TermxWebRTC TermxChannelMgr \
  TermxFileTransfer TermxHeartbeat TermxBridgeServer TermxBridgeRouter
```
