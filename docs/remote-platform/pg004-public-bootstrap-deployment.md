...
该环境仍不是正式商业生产：daemon enrollment 还是 bootstrap 单账号一次性 development code，支付仍为测试 provider，R2 备份恢复、账号长期续期和完整 Android file/lifecycle E2E 尚未通过。
...
1. daemon enrollment 是 bootstrap 单账号一次性 code，Controller 重启会重建该 development flow；不能开放给任意注册账号。
2. Android account access session 到期时曾退出登录，没有观察到成功的 `/v1/sessions/refresh`；daemon refresh contract 已通过，但 Android account 自动续期仍需单独修复和真实长时验证。
3. Edge 重启后 daemon Presence 收到的通用 `UNAUTHENTICATED` 不能区分 cold-start transient 与真实 revoke。当前 daemon 不会盲目重试所有鉴权错误，因此 Edge 重启后的自动 Presence 恢复仍未通过。
4. 模拟器发生 Wi-Fi -> cellular -> Wi-Fi 真实网络切换时，native generation 正确失效，旧 Relay session 没有复活；但 workspace 停在 `client session is unavailable`，没有自动创建新 session。该项不计为 lifecycle PASS。
5. Relay terminal attach/input 与远端文件浏览通过；上传、下载、取消和内容摘要校验尚未完成。
6. 当前部署 credential window 到 `2026-08-20T15:41:28Z`。到期前必须完成正式 key 配置/轮换或重新生成 staging 资产。
7. Let's Encrypt 证书到期日为 2026-10-19；当前已删除临时 Cloudflare credential，自动续期尚未配置。
8. R2 age 加密上传和独立恢复仍是 PG004 的未完成门禁。
9. 真实支付、邮件验证和密码找回未接入；bootstrap staging 不得作为商业生产发布。
