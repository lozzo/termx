...
supervisor manifest 记录 PID、listener、Hub/Relay identity、数据库引擎、usage outbox、配置和日志路径；PostgreSQL DSN 只存在于 0600 Controller 配置。`--fault-harness` 只为测试分配稳定 listener、控制透明 HubControl TCP proxy，并允许测试精确停止后重启单个子进程；默认 development 启动行为不变。
...
| assignment migration | Operator API 创建 `daemon-edge-a: hub-edge-a/epoch1 -> hub-edge-b/epoch2` | CommandOutbox 固化 source Hub、source generation、source epoch 与 target epoch；旧 Presence 被 `FenceAssignment` 关闭；PostgreSQL 在同一 receipt 事务中切换 assignment |
...
| Controller restart | 用相同 PostgreSQL 与稳定 listener 重启 Controller | Edge A generation 达到 2、Edge B 达到 4；`daemon-edge-a` 仍为 `hub-edge-b/epoch2`，initial config 不覆盖运行期 assignment truth |
...
| device revoke authority | Playwright `hub007.spec.ts` 与 planner/PostgreSQL command tests |
...
(cd private/cloud && go test -race ./control-plane/commandoutbox ./control-plane/postgres ./controller ./hub ./devcloud/cmd/muxvia-cloud-dev -count=1)
