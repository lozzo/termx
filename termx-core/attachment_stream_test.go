...
func TestAttachmentStreamPumpCoalescesPayloadDeltasWhileFrameInFlight(t *testing.T) {
	...
	pump := newAttachmentStreamPump(
		...
		func() StreamMessage {
			...
			defer latestMu.Unlock()
			latestSeq++
			return StreamMessage{Type: StreamScreenUpdate, Payload: []byte(fmt.Sprintf("latest-%d", latestSeq))}
		},
		func(channel uint16, typ uint8, payload []byte) error {
			if typ == wire.TypeScreenUpdate {
				select {
	...
	pump.screenReady()
	src <- StreamMessage{Type: StreamScreenUpdate, Payload: []byte("delta-1")}
	select {
	case <-sendStarted:
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for first send")
	...
	for i := 2; i <= 8; i++ {
		src <- StreamMessage{Type: StreamScreenUpdate, Payload: []byte(fmt.Sprintf("delta-%d", i))}
	}
	assertNoSentFrame(t, sent, 75*time.Millisecond)

	releaseSend <- struct{}{}
	first := waitSentFrame(t, sent)
	if first.Type != wire.TypeScreenUpdate || string(first.Payload) != "delta-1" {
		t.Fatalf("unexpected first frame: %#v", first)
	}
	assertNoSentFrame(t, sent, 75*time.Millisecond)
	...
	releaseSend <- struct{}{}
	second := waitSentFrame(t, sent)
	if second.Type != wire.TypeScreenUpdate || string(second.Payload) != "latest-1" {
