...
func (s *terminalGridStore) Viewport(beforeOffset int, limit int, cols int) (terminalGridViewport, error) {
	...
	for {
		...
		result.Rows, result.Timestamps, result.RowKinds, result.Wrapped, result.LogicalLineIDs, result.LogicalLineIDAuthoritative, result.LineTimestampStart, result.LineTimestampEnd, result.RowIDRanges = reflowTerminalGridRowsWithRowIDRanges(gridRows, cols, lineRecords, rowIDs)
...
func trimTerminalGridViewportToTail(result *terminalGridViewport, limit int) bool {
	...
	result.LineTimestampStart = trimTimeSliceTail(result.LineTimestampStart, limit)
	result.LineTimestampEnd = trimTimeSliceTail(result.LineTimestampEnd, limit)
...
func trimTerminalGridViewportPrefix(result *terminalGridViewport, start int) {
	...
	if start >= len(result.Rows) {
		...
		result.LineTimestampStart = nil
		result.LineTimestampEnd = nil
	...
	result.LineTimestampStart = cloneTimeSlice(result.LineTimestampStart[start:])
	result.LineTimestampEnd = cloneTimeSlice(result.LineTimestampEnd[start:])
...
func (s *terminalGridStore) recoveredLiveTailFromMetadata() (terminalPrimaryLiveTail, bool) {
	...
	var persistedRefs []terminalGridRowRef
	if terminalGridLineRecordMetasContainOrigin(metadata.LiveRecords, terminalLiveTailOriginReclaimed) {
		var ok bool
		persistedRefs, persistedRecords, ok = s.persistedStateForRecoveredLiveTail(baseRowID, generation)
		if !ok {
			return terminalPrimaryLiveTail{}, false
		}
	}
	migrations := terminalGridLineMigrationMap(metadata.Migrations)
	segments, ok := terminalLiveTailSegmentsFromMetadata(metadata.LiveRecords, rows, baseRowID, generation, rowCount, persistedRecords, persistedRefs, s.dir, migrations)
	if !ok {
		return terminalPrimaryLiveTail{}, false
	}
...
func (s *terminalGridStore) persistedStateForRecoveredLiveTail(baseRowID uint64, generation uint64) ([]terminalGridRowRef, []terminalGridLogicalLineRecord, bool) {
	if s == nil {
	...
	return refs, s.persistedLogicalLineRecordsForRetention(refs, baseRowID, generation), true
}
...
func terminalLiveTailSegmentsFromMetadata(records []terminalGridLineRecordMeta, rows []vterm.DamageOp, persistedBaseRowID uint64, persistedGeneration uint64, persistedRowCount int, persistedRecords []terminalGridLogicalLineRecord, persistedRefs []terminalGridRowRef, persistedDir string, migrations map[uint64]uint64) ([]terminalLiveTailSegment, bool) {
	...
	for i, record := range records {
		...
		if record.Origin == terminalLiveTailOriginReclaimed {
			...
			if !terminalLiveTailReclaimedPayloadMatchesPersistedRows(firstRowID, lastRowID, persistedBaseRowID, persistedRefs, persistedDir, segmentRows) {
				return nil, false
			}
...
func terminalLiveTailReclaimedPayloadMatchesPersistedRows(firstRowID uint64, lastRowID uint64, persistedBaseRowID uint64, persistedRefs []terminalGridRowRef, persistedDir string, rows []vterm.DamageOp) bool {
	if len(rows) == 0 || len(persistedRefs) == 0 || strings.TrimSpace(persistedDir) == "" {
		return false
	}
	if firstRowID < persistedBaseRowID || lastRowID < firstRowID {
		return false
	}
	startOffset := firstRowID - persistedBaseRowID
	endOffset := lastRowID - persistedBaseRowID
	if endOffset >= uint64(len(persistedRefs)) {
		return false
	}
	if int(endOffset-startOffset)+1 != len(rows) {
		return false
	}
	refs := persistedRefs[int(startOffset) : int(endOffset)+1]
	persistedRows, err := readTerminalGridRows(persistedDir, refs)
	if err != nil {
		return false
	}
	return terminalGridRowsMatchRecoveredDamageRows(persistedRows, rows)
}

func terminalGridRowsMatchRecoveredDamageRows(gridRows []terminalGridRow, damageRows []vterm.DamageOp) bool {
	...
	for i, gridRow := range gridRows {
		...
		if !gridRow.timestamp.Equal(damageRow.Timestamp) {
			return false
		}
		if gridRow.rowKind != damageRow.RowKind {
			return false
		}
...
func reflowTerminalGridRowsWithAuthority(rows []terminalGridRow, cols int, lineRecords []terminalGridLogicalLineRecord) ([][]vterm.Cell, []time.Time, []string, []bool, []uint64, []bool) {
	projected, timestamps, rowKinds, wrapped, lineIDs, authoritative, _, _, _ := reflowTerminalGridRowsWithRowIDRanges(rows, cols, lineRecords, nil)
	return projected, timestamps, rowKinds, wrapped, lineIDs, authoritative
}

func reflowTerminalGridRowsWithRowIDRanges(rows []terminalGridRow, cols int, lineRecords []terminalGridLogicalLineRecord, rowIDs []uint64) ([][]vterm.Cell, []time.Time, []string, []bool, []uint64, []bool, []time.Time, []time.Time, []terminalGridRowIDRange) {
	if len(rows) == 0 {
	...
	reflow := terminalGridReflowState{
		...
		lineTimestampStart:  make([]time.Time, 0, minInt(len(rows), 1024)),
		lineTimestampEnd:    make([]time.Time, 0, minInt(len(rows), 1024)),
	...
	for i, row := range rows {
		...
		lineTimestampStart := row.timestamp
		lineTimestampEnd := row.timestamp
		if hasRecord {
			if !lineRecords[recordIndex].timestampStart.IsZero() {
				lineTimestampStart = lineRecords[recordIndex].timestampStart
			}
			if !lineRecords[recordIndex].timestampEnd.IsZero() {
				lineTimestampEnd = lineRecords[recordIndex].timestampEnd
			}
		}
		rowKind := row.rowKind
		...
		meta := terminalGridReflowMeta{
			...
			timestampStart:             lineTimestampStart,
			timestampEnd:               lineTimestampEnd,
		}
		if trackRowIDRanges {
			meta.rowID = rowIDs[i]
	...
	return reflow.rows,
		...
		alignGridTimes(reflow.lineTimestampStart, len(reflow.rows)),
		alignGridTimes(reflow.lineTimestampEnd, len(reflow.rows)),
...
func (r *terminalGridReflowState) emitSegment(segment []terminalGridReflowCell, meta terminalGridReflowMeta, wrapped bool, suppressRowKind bool) {
	...
	r.lineTimestampStart = append(r.lineTimestampStart, terminalGridReflowSegmentTimestampStart(segment, meta))
	r.lineTimestampEnd = append(r.lineTimestampEnd, terminalGridReflowSegmentTimestampEnd(segment, meta))
	if r.trackRowIDRanges {
		r.rowIDRanges = append(r.rowIDRanges, terminalGridRowIDRangeForSegment(segment, meta))
	}
}

func terminalGridReflowSegmentTimestampStart(segment []terminalGridReflowCell, meta terminalGridReflowMeta) time.Time {
	if len(segment) == 0 {
		return meta.timestampStart
	}
	var out time.Time
	for _, cell := range segment {
		value := cell.meta.timestampStart
		if value.IsZero() {
			value = cell.meta.timestamp
		}
		if value.IsZero() {
			continue
		}
...
func terminalGridReflowSegmentTimestampEnd(segment []terminalGridReflowCell, meta terminalGridReflowMeta) time.Time {
	if len(segment) == 0 {
		return meta.timestampEnd
	}
	var out time.Time
	for _, cell := range segment {
		value := cell.meta.timestampEnd
		if value.IsZero() {
			value = cell.meta.timestamp
		}
		if value.IsZero() {
...
func terminalLiveTailRecordsValidForLineState(records []terminalLiveTailLogicalLineRecord, rows []vterm.DamageOp, baseRowID uint64, generation uint64, rowCount int, persistedRecords []terminalGridLogicalLineRecord, migrations map[uint64]uint64) bool {
	...
	for i, record := range records {
		...
		if record.origin != terminalLiveTailOriginReclaimed {
			...
			if rowCount <= 0 || record.firstRowID < baseRowID {
				return false
			}
			lastPersistedRowID := baseRowID + uint64(rowCount) - 1
			if record.lastRowID > lastPersistedRowID {
				return false
			}
