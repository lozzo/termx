...
当前项目已迁入 monorepo 子目录 `termx-core/`。
当前模块是 core runtime，不再直接承载 `termx` CLI 或 `tuiv2` 产品壳。
...
- `termx-core/` 提供 daemon / protocol / transport / Go API
- `tuiv2/` 提供 TUI 客户端库
- `termx-cli/` 提供 `termx` 命令行与 web bridge 壳
