...
func snapshotFromVTerm(source *localvterm.VTerm) *protocol.Snapshot {
	...
	for _, row := range scrollback {
		scrollbackRows = append(scrollbackRows, protocol.CompactRowFromCellsPreserveTrailingBlankCells(protocolCellsFromVTermRow(row), true))
	}
...
func repeatedProtocolOwnership(value string, count int) []string {
	if count <= 0 || value == "" {
		return nil
	}
...
func applyScreenUpdateSnapshot(current *protocol.Snapshot, terminalID string, update protocol.ScreenUpdate) *protocol.Snapshot {
	...
	if update.FullReplace {
		...
		snapshot.ScreenOwnership = repeatedProtocolOwnership(protocol.RowOwnershipScreen, len(snapshot.Screen.Cells))
		for _, row := range update.ScrollbackAppend {
