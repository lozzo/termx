# termx-core Docs

这里放的是 core runtime 文档，只覆盖 shell-neutral 能力。

## 核心规格

- [spec-overview.md](spec-overview.md)
- [spec-terminal.md](spec-terminal.md)
- [spec-protocol.md](spec-protocol.md)
- [spec-transport.md](spec-transport.md)
- [spec-snapshot.md](spec-snapshot.md)
- [spec-events.md](spec-events.md)
- [spec-go-api.md](spec-go-api.md)
- [spec-pty-manager.md](spec-pty-manager.md)
- [spec-vterm.md](spec-vterm.md)

## Implementation Notes

- [screen-update-opcode-prototype.md](screen-update-opcode-prototype.md)
- [transport-slow-consumer-strategy.md](transport-slow-consumer-strategy.md)
- [x-vt-local-fork.md](x-vt-local-fork.md)

## Out Of Scope

TUI 专属设计、主题、交互和渲染文档已迁到：

- [../../tuiv2/docs/README.md](../../tuiv2/docs/README.md)
