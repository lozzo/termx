...
func TestTerminalSemanticProjectorConsumesC1CSISGRRawWithoutFallback(t *testing.T) {
	resetTerminalSemanticIngestTestHooks()
	var stats terminalSemanticProjectorStats
	terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
		stats.SemanticProjectors += next.SemanticProjectors
	...
	if !damageOpsContainTextSpan(damage.SemanticOps, "ERR") || !damageOpsContainTextSpan(damage.SemanticOps, " plain") {
		t.Fatalf("C1 CSI SGR should expose styled/plain text through vterm semantic ops, damage=%#v", damage)
	}
	...
	if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.WriteSpanOps == 0 {
		t.Fatalf("C1 CSI SGR raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
	}
	window, err := pipeline.LatestWindow(32, 4)
	if err != nil {
		t.Fatalf("latest: %v", err)
	}
	if len(window.Rows) != 1 || window.Rows[0].Text != "ERR plain" {
		t.Fatalf("C1 CSI SGR semantic text should preserve one logical row, got %#v damage=%#v", window.Rows, damage)
	}
	cells := window.Rows[0].Cells
	if len(cells) != 2 {
		t.Fatalf("expected styled/plain cells from C1 CSI SGR vterm semantic text, got %#v rows=%#v", cells, window.Rows)
	}
	if cells[0].Text != "ERR" || cells[0].Style.FG != "ansi:1" || !cells[0].Style.Bold {
		t.Fatalf("expected C1 CSI SGR style metadata from vterm semantic text, got %#v rows=%#v", cells[0], window.Rows)
	}
	if cells[1].Text != " plain" || cells[1].Style != (history.CellStyle{}) {
		t.Fatalf("expected following text to reset C1 CSI SGR style, got %#v rows=%#v", cells[1], window.Rows)
	}
}
...
func TestTerminalSemanticProjectorConsumesC1CSIPrivateModesRawWithoutFallback(t *testing.T) {
	resetTerminalSemanticIngestTestHooks()
	var stats terminalSemanticProjectorStats
	terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
		stats.SemanticProjectors += next.SemanticProjectors
	...
	for _, mode := range []int{2026, 1004} {
		if firstDamageMode(damage, mode).Code != vterm.ScreenOpModes {
			t.Fatalf("C1 CSI private mode %d should be exposed as vterm semantic mode, damage=%#v", mode, damage)
		}
	}
	pipeline := newTerminalHistoryPipeline(24, 3)
	batch := terminalSemanticBatch{
	...
	if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.ModeOps < 4 || stats.WriteSpanOps == 0 {
		t.Fatalf("C1 CSI private modes raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
	}
	window, err := pipeline.LatestWindow(24, 4)
	if err != nil {
		t.Fatalf("latest: %v", err)
	...
	if window.TotalLines != 0 || !historyWindowContainsText(window, "frame") {
		t.Fatalf("C1 CSI private modes should expose mutable primary frame without committed depth, total=%d rows=%#v damage=%#v", window.TotalLines, window.Rows, damage)
	}
}
...
func TestTerminalSemanticProjectorConsumesESCNELRawWithoutFallback(t *testing.T) {
	resetTerminalSemanticIngestTestHooks()
	var stats terminalSemanticProjectorStats
	terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
		stats.SemanticProjectors += next.SemanticProjectors
	...
	if firstDamageControl(damage, "cr").Control != "cr" || firstDamageControl(damage, "lf").Control != "lf" {
		t.Fatalf("ESC NEL raw should expose ordered CR/LF semantic controls, damage=%#v", damage)
	}
	pipeline := newTerminalHistoryPipeline(16, 3)
	batch := terminalSemanticBatch{
		Raw:             raw,
	...
	if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.ControlOps < 2 || stats.WriteSpanOps == 0 {
		t.Fatalf("ESC NEL raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
	}
	window, err := pipeline.LatestWindow(16, 4)
	if err != nil {
		t.Fatalf("latest: %v", err)
	...
	if len(window.Rows) != 2 || window.Rows[0].Text != "abc" || window.Rows[1].Text != "Z" {
		t.Fatalf("ESC NEL semantic projector should apply vterm CR/LF controls, got %#v damage=%#v", window.Rows, damage)
	}
}
...
func TestTerminalSemanticProjectorConsumesC1CSIMarginOriginRawWithoutFallback(t *testing.T) {
	c1 := string([]byte{0x9b})

	t.Run("scroll-region-origin-ri", func(t *testing.T) {
		resetTerminalSemanticIngestTestHooks()
		var stats terminalSemanticProjectorStats
		terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
			stats.SemanticProjectors += next.SemanticProjectors
		...
		raw := c1 + "2;5r" + c1 + "?6h" + c1 + "1;1HX" + c1 + "2;1H\x8dregion"
		term := vterm.New(16, 6, 100, nil)
		_, err, damage := term.WriteWithDamage([]byte(raw))
		if err != nil {
			t.Fatalf("vterm write: %v", err)
		}
		if firstDamageMode(damage, 6).Code != vterm.ScreenOpModes {
			t.Fatalf("C1 origin mode should expose vterm semantic mode, damage=%#v", damage)
		}
		region := firstDamageControl(damage, "decstbm")
		ri := firstDamageControl(damage, "ri")
		if region.Mode != 2 || region.Bottom != 5 || ri.Control != "ri" {
			t.Fatalf("C1 margin/origin batch should expose decstbm and ri controls, region=%#v ri=%#v damage=%#v", region, ri, damage)
		}
		pipeline := newTerminalHistoryPipeline(16, 6)
		if err := pipeline.IngestSemanticBatch(terminalSemanticBatch{
			Raw:             raw,
		...
		if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.ModeOps == 0 || stats.ControlOps < 4 || stats.WriteSpanOps == 0 {
			t.Fatalf("C1 scroll-region/origin raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
		}
		window, err := pipeline.LatestWindow(16, 8)
		if err != nil {
			t.Fatalf("latest: %v", err)
		}
		if !historyWindowContainsText(window, "region") {
			t.Fatalf("C1 margin/origin text should remain visible through vterm semantic projector, rows=%#v damage=%#v", window.Rows, damage)
		}
	})

	t.Run("left-right-margin", func(t *testing.T) {
		resetTerminalSemanticIngestTestHooks()
		var stats terminalSemanticProjectorStats
		terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
			stats.SemanticProjectors += next.SemanticProjectors
		...
		if firstDamageMode(damage, 69).Code != vterm.ScreenOpModes {
			t.Fatalf("C1 left/right margin mode should be exposed as vterm mode op, damage=%#v", damage)
		}
		region := firstDamageControl(damage, "decslrm")
		if region.Mode != 3 || region.Bottom != 6 {
			t.Fatalf("C1 DECSLRM should expose vterm-owned horizontal margins, got %#v damage=%#v", region, damage)
		}
		cbt := firstDamageControl(damage, "cbt")
		if cbt.Col != 2 {
			t.Fatalf("C1 CBT should expose final left-margin column from vterm, got %#v damage=%#v", cbt, damage)
		}
		...
		if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.ModeOps < 2 || stats.ControlOps < 4 || stats.WriteSpanOps == 0 {
			t.Fatalf("C1 left/right margin raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
		}
		...
		if !historyWindowContainsText(window, "AX") {
			t.Fatalf("C1 left/right margin text should follow vterm-resolved CBT coordinates, rows=%#v damage=%#v", window.Rows, damage)
		}
	})
}
...
func TestTerminalSemanticProjectorConsumesC1CSILinefeedNewlineModeRawWithoutFallback(t *testing.T) {
	resetTerminalSemanticIngestTestHooks()
	var stats terminalSemanticProjectorStats
	terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
		stats.SemanticProjectors += next.SemanticProjectors
	...
	if firstDamageMode(damage, 20).Mode != 20 {
		t.Fatalf("C1 CSI linefeed-newline raw should expose ANSI mode 20 semantic op, damage=%#v", damage)
	}
	if firstDamageControl(damage, "cr").Control != "cr" {
		t.Fatalf("C1 CSI linefeed-newline mode should let vterm expose LF followed by CR, damage=%#v", damage)
	}
	pipeline := newTerminalHistoryPipeline(12, 4)
	if err := pipeline.IngestSemanticBatch(terminalSemanticBatch{
		Raw:             raw,
	...
	if stats.SemanticProjectors == 0 || stats.RawFallbacks != 0 || stats.ModeOps < 2 || stats.ControlOps == 0 || stats.WriteSpanOps == 0 {
		t.Fatalf("C1 CSI linefeed-newline raw should use vterm semantic projector without parser fallback, stats=%#v damage=%#v", stats, damage)
	}
	window, err := pipeline.LatestWindow(12, 4)
	if err != nil {
		t.Fatalf("latest: %v", err)
	...
	if len(window.Rows) != 2 || window.Rows[0].Text != "abc" || window.Rows[1].Text != "Z" {
		t.Fatalf("C1 CSI linefeed-newline semantic projector should write following text at line start, got %#v damage=%#v", window.Rows, damage)
	}
}
...
func TestTerminalSemanticProjectorKeepsSharedVTermAltExitFrameAsLatestOnly(t *testing.T) {
	resetTerminalSemanticIngestTestHooks()
	var stats terminalSemanticProjectorStats
	terminalSemanticProjectorHook = func(next terminalSemanticProjectorStats) {
		stats.ModeOps += next.ModeOps
	...
	if stats.AltExitFrames == 0 {
		t.Fatalf("shared vterm alt final frame should be exposed as transient latest frame, stats=%#v", stats)
	}
	...
	for _, row := range window.Rows {
		if strings.Contains(row.Text, "alt-final") && row.Committed {
			t.Fatalf("alt final frame must stay mutable, rows=%#v", window.Rows)
		}
	}
}
