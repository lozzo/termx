...
func TestTerminalResizeDoesNotLetSynchronizedFrameDeleteCommittedHistory(t *testing.T) {
	...
	for _, forbidden := range []string{"frame-one", "input-one", "frame-two", "input-two"} {
		if strings.Contains(olderText, forbidden) {
			t.Fatalf("older after resize/repaint must not page current or replaced frame %q, text=%q rows=%#v", forbidden, olderText, older.Rows)
		}
	}
}
