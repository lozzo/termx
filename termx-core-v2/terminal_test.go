...
func TestTerminalIngestOutputStyledEraseInLinePreservesBackgroundBlankCells(t *testing.T) {
	server := NewServer(WithProcessFactory(newRecordingProcessFactory()))
	if _, err := server.RegisterTerminal(TerminalRecord{
		ID:      "term-1",
		Command: []string{"shell"},
	...
	if err := server.IngestOutput(context.Background(), "term-1", "\x1b[44mBG\x1b[K\x1b[0m"); err != nil {
		t.Fatalf("ingest output: %v", err)
	}
	...
	if len(liveSnapshot.Screen.Cells) == 0 || len(liveSnapshot.Screen.Cells[0]) < 8 {
		t.Fatalf("expected live screen row with 8 cells, got %#v", liveSnapshot.Screen.Cells)
	}
	if got := liveSnapshot.Screen.Cells[0][7].Style.BG; got != "ansi:4" {
	...
	for i, cell := range cells {
		if cell.Width != 1 || cell.Style.BG != "ansi:4" {
			t.Fatalf("expected cell %d to keep bg from BCE erase, got %#v", i, cell)
		}
	}
}

func TestTerminalIngestOutputDefaultEraseInLineDoesNotPadToScreenWidth(t *testing.T) {
	server := NewServer(WithProcessFactory(newRecordingProcessFactory()))
	if _, err := server.RegisterTerminal(TerminalRecord{
		ID:      "term-1",
		Command: []string{"shell"},
	...
	if len(window.Rows) != 1 || window.Rows[0].Text != "he   " {
		t.Fatalf("default erase should only preserve existing logical footprint, got %#v", window.Rows)
	}
	if got := len(window.Rows[0].Cells); got != 5 {
		t.Fatalf("default erase should not synthesize full terminal-width history cells, got %d cells %#v", got, window.Rows[0].Cells)
	}
}
...
func TestTerminalIngestOutputAltScreenDoesNotWritePrimaryHistory(t *testing.T) {
	server := NewServer(WithProcessFactory(newRecordingProcessFactory()))
	if _, err := server.RegisterTerminal(TerminalRecord{
		ID:      "term-1",
		Command: []string{"shell"},
