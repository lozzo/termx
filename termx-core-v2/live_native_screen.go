...
// truth 来源是 terminal 当前 PTY/live surface size，不来自 TUI pane 或 renderer frame。
...
// 它由 Terminal 在同一 live 锁内读取 SurfaceTrack 与 LiveRevision 后生成；不携带 scrollback、history token 或 TUI view 状态。
