...
func TestR301HistoryCleanupGuard(t *testing.T) {
	...
	if strings.Contains(terminalSource, "historyQueue.Enqueue(text)") ||
		strings.Contains(terminalSource, "historyANSIParser") ||
		strings.Contains(terminalSource, "terminalHistoryPipeline") {
	...
	if !strings.Contains(workerSource, "terminalHistoryIngestQueue") {
		t.Fatal("history semantic worker guard must cover the ingest worker")
	}
}
