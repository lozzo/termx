...
flowchart TD
    ...
    VTERM["termx-vterm<br/>terminal parser + screen machine"]
...
| --- | --- | --- | --- |
| PTY / resize -> vterm | `TerminalSemanticSource.ApplyPTYWrite`、`TerminalSemanticSource.Resize` | `termx-vterm` adapter | R302 |
| vterm -> core history | `TerminalSemanticTransaction`，包含 ordered `Ops`、`PrimaryScrollOut`、`PrimaryFrame`、`AltFrame`、alt enter/exit、sync begin/end、full replace 标记 | `termx-vterm` 产出，`termx-core-v2` 消费 | R302 |
| semantic tx -> session decision | `ScreenAppClassifier.Classify(tx, state) ScreenAppDecision` | `termx-core-v2` | R303 |
| semantic tx + decision -> history mutation | `HistoryProjector.Apply(tx, decision)` | `termx-core-v2` | R303 |
| terminal lifecycle -> force close | `HistoryProjector.ForceClose(reason)`；process exit、terminal kill、detach cleanup 不能伪造成 vterm transaction | `termx-core-v2` | R303-R304 |
| projector -> authoritative store | `HistoryMutation`，只能表达 ordinary commit、frontier mutate、publish primary frame、archive primary frame、publish alt frame、close session、final frame commit | `termx-core-v2` | R303 |
| store 内部 payload/index/journal | `LogicalLineStore`、`CommittedHistoryIndex`、`ScreenFrameJournal`，三者不能互相复制成第二份 truth | `termx-core-v2` | R303-R307 |
| store -> file backend | `StorageBackend`，只负责 append/update/recover/compact residency，不决定 mutability | `termx-core-v2` | R309 |
| store -> history consumer | `HistoryWindowRequest`、`HistoryWindow`、`FreezeHistoryRequest`、`HistoryCopyRequest`、`HistoryToken` | `termx-core-v2` | R309-R310 |
| protocol -> TUI/App | `history.window`、`history.copy`、`history.release`，必须保留 row kind、segment cursor/token、generation | `internal/protocol` / `termx-proto` / `termx-tui-v3` | R310 |
| live display projection | `LiveSurfaceTrack` 或等价 view model，只能服务当前显示和短缓存 | `termx-core-v2` / `termx-tui-v3` | 按需 |
...
- `LogicalLineStore` 是唯一 payload truth。
...
`termx-vterm/vterm/vterm.go` 的 `WriteDamage` 已包含关键字段：
...
`termx-core-v2/live/types.go` 的 `SurfaceTrack.WriteWithResult` 和
...
`termx-core-v2/terminal_semantic_ingest.go` 已有：
...
`termx-core-v2/history/types.go` 已有：

- `RowKindScreenFrame`
- `RowKindArchivedScreenFrame`
- `RowKindAltScreenFrame`

`termx-core-v2/history/track.go`、`window.go`、`snapshot.go` 当前已经有
...
`termx-core-v2/protocol_service.go` 已投影 `RowKinds` / line span kind。
`termx-tui-v3/services/protocol_adapter.go` 和 `termx-tui-v3/state/history.go` 已认识
