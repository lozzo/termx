...
func TestR309MemoryStorageRecoverKeepsDomainStructuresSeparate(t *testing.T) {
	...
	err := backend.Apply(StorageTransaction{
		...
		Timeline: []HistoryRecord{{
			...
			LineIDs: []LogicalLineID{1},
		}},
		Mutable: []LogicalLineID{2},
		Frames: []FrameRecord{{
			SessionID: 1,
			FrameID:   2,
	...
	if state.Generation != 7 || len(state.Lines) != 2 || len(state.Timeline) != 1 || len(state.Mutable) != 1 || len(state.Frames) != 1 {
...
func TestR309MemoryStoragePersistedLineCanBeUpdated(t *testing.T) {
	...
	if err := backend.Apply(StorageTransaction{
		...
		Timeline:   []HistoryRecord{{ID: 1, Kind: HistoryRecordOrdinaryLine, LineIDs: []LogicalLineID{1}}},
	}); err != nil {
...
func TestR309MemoryStorageCompactKeepsReferencedPayloadOnly(t *testing.T) {
	...
	if err := backend.Apply(StorageTransaction{
		Generation: 1,
		Lines: []LogicalLine{
			{ID: 1, Cells: []Cell{{Text: "sealed", Width: 6}}},
			{ID: 2, Cells: []Cell{{Text: "mutable", Width: 7}}},
