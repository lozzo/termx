...
func TestOverlayViewShowsLabels(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{screen: projection.Screen{Overlay: projection.Overlay{Kind: "help"}}, want: "workbench / pool shortcuts"},
		{screen: projection.Screen{Overlay: projection.Overlay{Kind: "prompt", Title: "rename pane"}}, want: "rename pane"},
	} {
		...
		if !strings.Contains(view, "┌") {
			t.Fatalf("expected wireframe box, got %q", view)
		}
	}
}
