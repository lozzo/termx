...
func TestLiveAttachResultForBackgroundViewDoesNotReplaceActiveProjection(t *testing.T) {
	...
	next, _ := reducer(root, LiveAttachResultMsg{EndpointID: "west", TerminalID: "remote", ViewID: state.TerminalPaneViewID("pane-remote"), Result: services.TerminalAttachResult{
		EndpointID:    "west",
		TerminalID:    "remote",
		Channel:       9,
		Cols:          100,
...
func TestLiveAttachResultAcceptsPrefilledSessionBeforeFirstBinding(t *testing.T) {
	...
	if !root.Session.Attached || root.Session.Channel != 7 || root.Session.ViewID != "cmd/termx-v3-main" {
		t.Fatalf("prefilled CLI session should accept first attach result, session=%#v", root.Session)
	}
	if binding, ok := root.TerminalViews.PaneBinding(state.DefaultPaneID); !ok || binding.ViewID != "cmd/termx-v3-main" {
