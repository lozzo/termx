...
本文档定义 `tui` 的 render 架构方向。

本文档回答：

- 为什么 `tui` 需要一个内部 render framework。
...
本文档必须以 `tui/docs/ui-interaction-spec.md` 为产品基准。

## 2. 结论

`tui` 应把 render 做成内部 composition framework，而不是 terminal 专用 renderer。
...
- 根据 `tui/docs/visual-alignment-audit.md` 与 `docs/history/tui/default-tui-visual-review.md`，当前默认 TUI 仍未达到用户截图要求的完整 `tuiv2` 视觉等级；后续必须继续按视觉返工切片推进。
...
render framework 是 `tui` 内部架构，不是独立产品。
...
- Terminal Picker 已有 styled overlay 渲染路径，并已接入 query/filter/selection/attach/create 交互。
...
- Terminal Picker query、过滤、selected row、上下键移动、Enter attach/focus/create、row click attach/focus/create、compact overlay 和 no terminal input leak 已落地；`+ new terminal` 是第一行默认选中项，create row 带短说明，terminal row 使用短 `○/● id title state @source` 信息密度，Terminal Pool 式 detail/preview 管理块不得回流到 picker。
...
- Terminal Picker 真实交互深化已进入当前阶段：query、过滤、selected row、上下键移动、Enter focus/create、picker row click、第一行默认选中的 `+ new terminal` create row、compact search/list 内容、短 terminal id/state/source 行信息和 no terminal input leak 已通过 reducer-owned state 与 content renderer 路径表达。
