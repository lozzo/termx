...
`tui` 当前已经形成可用的自有 runtime、state、input、services、render framework、styled chrome 和视觉验收 harness。后续主要风险不再是“功能不可见”，而是代码聚合点继续膨胀，导致小视觉或交互修改必须同时理解 state、projection、layout、chrome、hit region 和 app command adapter。
...
下一阶段不要把 `tui` 做成通用 UI 框架，也不要引入 widget tree、DSL、插件系统或 Bubble Tea contract。
...
中期再考虑拆出 `tui/viewmodel` 或 `tui/projection` package。只有当下面条件满足时才移动 package：
...
注意：该切片默认只能处理 `workflow.md` 当前允许范围内的文件。远程产品目录是保留资产，不得为了维护性重构顺手删除或接成 TUI/core fallback；若必须重新设计 App/Web/control 面，必须先更新 workflow 范围表并单独说明原因。该切片必须单独提交，不和抽象代码变更混在一起。
