...
本计划定义 `tui` 后续快捷键系统的唯一目标形态：所有可配置键盘入口、sticky 场景按钮、footer/help/overlay 提示都从同一份 `tui.shortcuts` 配置解析出的 shortcut catalog 生成，并通过同一种 action invocation 进入 app action registry。未修饰 Esc 是 catalog 之外的全局返回导航，由 reducer-owned state 层级决定。
