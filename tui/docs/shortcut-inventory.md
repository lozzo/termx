...
KS012 重新盘点后的机器可读基线位于 `shortcut-contract-debt.json`。当前默认 catalog 有 203 个 scene+key 条目，其中 166 个进入 root/sticky 输入路由；现有 shortcut registry 有 146 个 canonical spec，render 第二 action catalog 有 123 个 projection。
