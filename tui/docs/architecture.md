...
- CONN003 的目标是让 `client/runtime` 成为每个 Endpoint 唯一跨端 session owner，统一 planner、route race、`ReadyPeerSession`、`SessionGeneration`、winner/loser cleanup 和 lifecycle mailbox；TUI 只通过 `tui/port` 与 `tui/adapter/clientruntime` 消费 projection。算法细节不在本文复制。
- 当前 interface/DTO 按 history、terminal、live、path、endpoint event、clipboard、storage 位于 `tui/port`；client runtime/protocol/system 映射位于 `tui/adapter/*`，测试 fake 位于 `tui/testkit`。TUI state/port 不 import Cloud、protocol 或 transport concrete type，生产 port 不执行 IO，也不持有测试状态。
...
copy mode 进入路径只保留 pending -> frozen 两个显式状态，避免把“快速显示”伪装成
authoritative history：

```text
CopyModeEnteringPending
  只记录 latest 请求飞行和输入拦截
...
copy/history 入口只能请求统一的 `history.window`。如果 core 需要快速返回，优化必须封装在该 API
