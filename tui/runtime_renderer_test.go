...
func TestRuntimeRendererCanHideDebugSections(t *testing.T) {
	...
	if !strings.Contains(stripped, "api-dev") || !strings.Contains(stripped, "● run  owner") || !strings.Contains(stripped, "┌─ api-dev") || !strings.Contains(stripped, "$ pwd") || !strings.Contains(stripped, "/tmp") || !strings.Contains(stripped, "term-1 running owner") {
...
func TestModernScreenShellCompactsChromeAtNarrowWidth(t *testing.T) {
	...
	if !strings.Contains(stripped, "<p> PANE") || !strings.Contains(stripped, "<o> FLT") {
...
func TestModernScreenShellUsesWiderCompactOverlayWithoutShadowAtNarrowWidth(t *testing.T) {
	...
	if !strings.Contains(view, "Help") ||
		!strings.Contains(view, "RETURN TO") ||
		!strings.Contains(view, "WORKBENCH") ||
		!strings.Contains(view, "QUICK KEYS") ||
		!strings.Contains(view, "SHARED MODEL") ||
		!strings.Contains(view, "ACTION BAR") ||
		!strings.Contains(view, "return to tiled:ws-1/tab-1/pane-1") ||
		!strings.Contains(view, "Esc close") {
		t.Fatalf("expected narrow overlay to keep productized help dialog, got:\n%s", view)
	}
	if !strings.Contains(view, "workbench paused  •  api-dev  •  owner") || !strings.Contains(view, "active pane api-dev  •  owner  •  connected") {
...
func TestRuntimeRendererShellOnlyOverlayKeepsPaneContext(t *testing.T) {
	...
	view := renderer.Render(state, nil)
	stripped := stripANSIForTest(view)
	if !strings.Contains(stripped, "termx") || !strings.Contains(stripped, "[main]") || !strings.Contains(stripped, "pane api-dev") || !strings.Contains(stripped, "mode picker") {
	...
	if !strings.Contains(stripped, "Help") ||
		...
		!strings.Contains(stripped, "return to tiled:ws-1/tab-1/pane-1") {
...
func TestRuntimeRendererShellOnlyFooterHighlightsErrorNotice(t *testing.T) {
	debugVisible := false
	state := connectedRunAppState()
	view := (runtimeRenderer{DebugVisible: &debugVisible}).Render(state, []btui.Notice{{
		Level: btui.NoticeLevelError,
		Text:  "runtime effect boom",
	}})
	stripped := stripANSIForTest(view)

	if !strings.Contains(stripped, "! error runtime effect") || !strings.Contains(stripped, "term-1") || !strings.Contains(stripped, "● run  owner") || !strings.Contains(stripped, "<p> PANE") {
...
func TestRuntimeRendererShellOnlyShowsContextualActionsForConnectedPane(t *testing.T) {
	...
	view := renderer.Render(state, nil)
	stripped := stripANSIForTest(view)
	if !strings.Contains(stripped, "<p> PANE") || !strings.Contains(stripped, "<g> GLOB") {
...
func TestRuntimeRendererShellOnlyShowsStatusAndActionsForDisconnectedStates(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			statusLine: "no terminal connected",
			actionLine: "n new | a connect | m manager",
		},
		{
			name:       "waiting",
			state:      buildSinglePaneAppState("main", "shell", types.PaneSlotWaiting),
			statusLine: "waiting for connect",
			actionLine: "n new | a connect",
		},
		{
			name: "exited",
			...
			statusLine: "process exited",
			actionLine: "r restart | a connect",
		},
	}
...
func TestRuntimeRendererShellOnlyRendersFloatingWorkbenchAsWindowDeck(t *testing.T) {
	...
	if strings.Contains(stripped, "FLOATING") || strings.Contains(stripped, "CONTEXT") || strings.Contains(stripped, "WINDOW DECK") {
		t.Fatalf("expected shell-only floating renderer to remove floating side rails from default path, got:\n%s", view)
	}
	if !containsRenderedLineWithAll(stripped, "main / shell / floating / api-dev", "top build-log", "float 2") {
		t.Fatalf("expected shell-only floating renderer context bar to expose active route and top window summary, got:\n%s", view)
	}
	if !strings.Contains(stripped, "◫ float 2") || !strings.Contains(stripped, "active api-dev") || !strings.Contains(stripped, "[top] build-log") {
		t.Fatalf("expected shell-only floating renderer to expose minimal floating status strip, got:\n%s", view)
	}
	...
	if !strings.Contains(stripped, "┌─ api-dev") || !strings.Contains(stripped, "┌─ build-log") {
		t.Fatalf("expected shell-only floating renderer to use legacy-like single-line window frames, got:\n%s", view)
	}
	if !regexp.MustCompile(`\x1b\[[0-9;]*m┌`).MatchString(view) ||
		!regexp.MustCompile(`\x1b\[[0-9;]*m●`).MatchString(view) {
...
func TestModernScreenShellMarksOffscreenFloatingPaneForRecall(t *testing.T) {
	...
	if strings.Contains(stripped, "CONTEXT") || strings.Contains(stripped, "WINDOW DECK") || strings.Contains(stripped, "Context & Keys") {
		t.Fatalf("expected modern floating shell to keep feedback inside top chrome and floating strip, got:\n%s", view)
	}
	if !strings.Contains(stripped, "top") || !strings.Contains(stripped, "●") || !strings.Contains(stripped, "own") || !strings.Contains(stripped, "c center") {
		t.Fatalf("expected modern floating shell to expose pane header badges and floating strip hints, got:\n%s", view)
	}
}
...
func TestRuntimeRendererShellOnlyRendersFloatingModeOperationHints(t *testing.T) {
	...
	view := (runtimeRenderer{DebugVisible: &debugVisible}).Render(state, nil)
	stripped := stripANSIForTest(view)
	if !strings.Contains(stripped, "<h/l>") || !strings.Contains(stripped, "<j/k>") || !strings.Contains(stripped, "<HJKL>") || !strings.Contains(stripped, "<c>") {
...
func TestRuntimeRendererShellOnlyRendersDetachedFloatingStripForMixedWorkbench(t *testing.T) {
	...
	view := renderer.Render(state, nil)
	stripped := stripANSIForTest(view)
	if strings.Contains(stripped, "MIXED") || strings.Contains(stripped, "PANES") || strings.Contains(stripped, "CONTEXT") || strings.Contains(stripped, "WINDOW DECK") {
		t.Fatalf("expected shell-only mixed workbench to remove mixed side rails from default path, got:\n%s", view)
	}
	if !strings.Contains(stripped, "◫ detached") || !strings.Contains(stripped, "float 1") || !strings.Contains(stripped, "unconnected pane") {
		t.Fatalf("expected shell-only mixed workbench to keep detached floating strip above the workbench canvas, got:\n%s", view)
	}
...
func TestRuntimeRendererShellOnlyRendersStructuredTerminalManagerOverlay(t *testing.T) {
	...
	if !strings.Contains(stripped, "return to tiled:ws-1/tab-1/pane-1") || !strings.Contains(stripped, "● api-dev") {
		t.Fatalf("expected shell-only terminal manager overlay to render selected manager row, got:\n%s", view)
	}
	if !strings.Contains(stripped, "running  •  visible") || !strings.Contains(stripped, "owner pane:pane-1") || !strings.Contains(stripped, "cmd npm run dev") || !strings.Contains(stripped, "path main/shell/pane:pane-1") {
		t.Fatalf("expected shell-only terminal manager overlay to render terminal detail summary, got:\n%s", view)
	}
}
...
func TestModernScreenShellFloatingOverlayBackdropKeepsFloatingStatusStrip(t *testing.T) {
	...
	if !containsNormalizedRuntimeLineWithAll(view, "◫ float 2") {
...
func TestModernScreenShellMixedOverlayBackdropKeepsDetachedFloatingStrip(t *testing.T) {
	...
	if !containsNormalizedRuntimeLineWithAll(view, "◫ detached", "float 1") {
...
func TestRuntimeRendererShellOnlyShowsUnavailableScreenBlockWhenSnapshotMissing(t *testing.T) {
	...
	view := (runtimeRenderer{DebugVisible: &debugVisible}).Render(state, nil)
	stripped := stripANSIForTest(view)
	if !strings.Contains(stripped, "api-dev") || !strings.Contains(stripped, "● run  owner") || !strings.Contains(stripped, "<screen unavailable>") || !strings.Contains(stripped, "term-1 running owner") {
...
func TestRuntimeRendererShellOnlyRendersStructuredPromptOverlay(t *testing.T) {
	...
	if !strings.Contains(stripped, "return to tiled:ws-1/tab-1/pane-1") || !strings.Contains(stripped, "2 fields  •  editing Name") || !strings.Contains(stripped, "Tags: env=dev") || !strings.Contains(stripped, "terminal term-1") || !strings.Contains(stripped, "value api-dev") || !strings.Contains(stripped, "Enter submit") {
		t.Fatalf("expected shell-only prompt overlay to keep field summary and active detail visible, got:\n%s", view)
	}
}
...
func TestRuntimeRendererShellOnlyRendersStructuredWorkspacePickerTarget(t *testing.T) {
	...
	if !strings.Contains(stripped, "Workspace Picker") ||
		...
		!strings.Contains(stripped, "workspace ops  (ws-2)") ||
		!strings.Contains(stripped, "workspace ws-2") ||
		!strings.Contains(stripped, "pane unconnected pane") {
		t.Fatalf("expected shell-only workspace picker overlay to render selected target detail, got:\n%s", view)
	}
	if !strings.Contains(stripped, "return to tiled:ws-1/tab-1/pane-1") {
...
func TestRuntimeRendererRendersWireframeWorkspacePickerTree(t *testing.T) {
	...
	if !strings.Contains(view, "  [tab] logs") || !strings.Contains(view, "    [pane] unconnected pane") || !strings.Contains(view, "> [workspace] ops") || !strings.Contains(view, "|>> ws ops") {
