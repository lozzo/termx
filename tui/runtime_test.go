...
func TestE2ERunScenarioDefaultShellOnlyEmptyPaneShowsStatusAndActions(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if !strings.Contains(stripped, "no terminal connected") || !strings.Contains(stripped, "n new | a connect | m manager") {
...
func TestE2ERunScenarioDefaultShellOnlyExitedPaneShowsStatusAndActions(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			stripped := stripANSIRuntimeView(view)
			if !strings.Contains(stripped, "process exited") || !strings.Contains(stripped, "history retained") || !strings.Contains(stripped, "r restart | a connect") {
...
func TestE2ERunScenarioDefaultModernSplitWorkbenchUsesFullWidthPaneCanvas(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			stripped := stripANSIRuntimeView(view)
			if !strings.Contains(stripped, "api-dev") || !strings.Contains(stripped, "build-log") || !strings.Contains(stripped, "term-2 running owner") || !strings.Contains(stripped, "┌─ api-dev") || !strings.Contains(stripped, "┌─ build-log") {
...
func TestE2ERunScenarioDefaultModernSingleWorkbenchKeepsTerminalSurfacePrimary(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			stripped := stripANSIRuntimeView(view)
			if !strings.Contains(stripped, "● run") || !strings.Contains(stripped, "running") || !strings.Contains(stripped, "▣ tiled") || !strings.Contains(stripped, "┌─ api-dev") {
...
func TestE2ERunScenarioDefaultModernFloatingWorkbenchUsesFullWidthCompositedCanvas(t *testing.T) {
	client := &stubRunClient{}
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if !strings.Contains(stripped, "┌─ api-dev") || !strings.Contains(stripped, "┌─ build-log") {
				t.Fatalf("expected default modern floating view to use legacy-like single-line window frames, got:\n%s", view)
			}
			if !regexp.MustCompile(`\x1b\[[0-9;]*m┌`).MatchString(view) ||
				!regexp.MustCompile(`\x1b\[[0-9;]*m●`).MatchString(view) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if strings.Contains(stripped, "MIXED") || strings.Contains(stripped, "PANES") || strings.Contains(stripped, "CONTEXT") || strings.Contains(stripped, "WINDOW DECK") {
				t.Fatalf("expected default modern mixed workbench to drop mixed side rails from the main path, got:\n%s", view)
			}
			if !strings.Contains(stripped, "◫ detached") || !strings.Contains(stripped, "float 1") || !strings.Contains(stripped, "unconnected pane") {
				t.Fatalf("expected default modern mixed workbench to keep detached floating strip above the full workbench canvas, got:\n%s", view)
			}
			if !strings.Contains(stripped, "api-dev") || !strings.Contains(stripped, "waiting pane") || !strings.Contains(stripped, "deploy-log") || !strings.Contains(stripped, "waiting for connect") || !strings.Contains(stripped, "process exited") || !strings.Contains(stripped, "no terminal con") || !strings.Contains(stripped, "history retained") || !strings.Contains(stripped, "[top] unconnected pane") {
...
func TestE2ERunScenarioDefaultModernHelpOverlayRendersStructuredModal(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if !strings.Contains(stripped, "workbench paused  •  api-dev  •  owner") || !strings.Contains(stripped, "active pane api-dev  •  owner  •  connected") || !strings.Contains(stripped, "overlay help  •  focus overlay") {
...
func TestE2ERunScenarioDefaultModernFloatingCenterRecallClearsOffscreenBadge(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if strings.Contains(after, "offscr") || !strings.Contains(after, "api-dev") || !strings.Contains(after, "remote") || !strings.Contains(after, "float 1") {
...
func TestE2ERunScenarioDefaultModernFloatingZOrderUpdatesTopSummaryAndKeepsOverlappedBodies(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			current := model
			before := stripANSIRuntimeView(current.View())
			if !strings.Contains(before, "top build-log") || !strings.Contains(before, "api ready") || !strings.Contains(before, "build-log") || !strings.Contains(before, "◫ float 2") {
				t.Fatalf("expected default modern floating shell to expose initial top-window summary and both window bodies on full canvas path, got:\n%s", current.View())
			}
			for _, key := range []tea.KeyMsg{
				{Type: tea.KeyCtrlO},
			...
			after := stripANSIRuntimeView(current.View())
			if !strings.Contains(after, "top api-dev") || !strings.Contains(after, "api ready") || !strings.Contains(after, "api-dev") || !strings.Contains(after, "build") || !strings.Contains(after, "◫ float 2") {
				t.Fatalf("expected default modern floating z-order flow to update top summary and preserve overlapped window bodies on full canvas path, got:\n%s", current.View())
			}
			return nil
		},
	}
...
func TestE2ERunScenarioDefaultModernOverlayCloseLeavesCleanWorkbench(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if !strings.Contains(opened, "Help") || !strings.Contains(opened, "┌─ Help") || !strings.Contains(opened, "┌─ RETURN TO") || !strings.Contains(opened, "WORKBENCH") || !strings.Contains(opened, "ACTION BAR") || !strings.Contains(opened, "┌─ ACTION BAR") || !strings.Contains(opened, "overlay help") || !strings.Contains(opened, "workbench paused  •  api-dev  •  owner") || !strings.Contains(opened, "active pane api-dev  •  owner  •  connected") {
			...
			if strings.Contains(closed, "overlay help") || strings.Contains(closed, "RETURN TO") || strings.Contains(closed, "ACTION BAR") || strings.Contains(closed, "workbench paused  •  api-dev  •  owner") || !strings.Contains(closed, "api-dev") || !strings.Contains(closed, "● run  owner") || !strings.Contains(closed, "<p> PANE") {
...
func TestE2ERunScenarioDefaultModernMixedHelpOverlayPreservesDetachedStripAndClosesCleanly(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			closed := stripANSIRuntimeView(current.View())
			if strings.Contains(closed, "overlay help") || !strings.Contains(closed, "◫ detached") || !strings.Contains(closed, "waiting pane") || !strings.Contains(closed, "deploy-log") || !strings.Contains(closed, "unconnected pane") {
...
func TestE2ERunScenarioDefaultModernWorkspacePickerOverlayRendersStructuredModal(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			stripped := stripANSIRuntimeView(view)
			if !strings.Contains(stripped, "Workspace Picker") || !strings.Contains(stripped, "┌─ Workspace Picker") || !strings.Contains(stripped, "WORKSPACE TREE") || !strings.Contains(stripped, "JUMP TARGET") || !strings.Contains(stripped, "ACTION BAR") || !strings.Contains(stripped, "┌─ ACTION BAR") || !strings.Contains(stripped, "overlay workspace_picker") {
...
func TestE2ERunScenarioDefaultModernPromptOverlayRendersStructuredModal(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			stripped := stripANSIRuntimeView(view)
			if !strings.Contains(stripped, "Prompt") || !strings.Contains(stripped, "┌─ Prompt") || !strings.Contains(stripped, "EDIT FIELDS") || !strings.Contains(stripped, "EDIT CONTEXT") || !strings.Contains(stripped, "ACTION BAR") || !strings.Contains(stripped, "┌─ ACTION BAR") || !strings.Contains(stripped, "overlay prompt") {
