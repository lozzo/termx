...
func TestE2ERunScenarioCtrlWOpensWorkspacePickerInView(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if view := current.View(); !strings.Contains(view, "workspace_picker_rows:") || !strings.Contains(view, "workspace_picker_selected: ws-1/tab-1/pane-1") || !strings.Contains(view, "workspace_picker_selected_kind: pane") || !strings.Contains(view, "[workspace] ops") {
...
func TestE2ERunScenarioTerminalManagerEditOpensPromptInView(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if view := current.View(); !strings.Contains(view, "prompt_title: edit terminal metadata") || !strings.Contains(view, "prompt_terminal: term-1") || !strings.Contains(view, "prompt_active_field: name") || !strings.Contains(view, "prompt_fields:") || !strings.Contains(view, "Name: api-dev") {
...
func TestE2ERunScenarioCtrlFOpensTerminalPickerInView(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if view := current.View(); !strings.Contains(view, "terminal_picker_rows:") || !strings.Contains(view, "terminal_picker_query: ops") || !strings.Contains(view, "terminal_picker_selected: term-3") || !strings.Contains(view, "terminal_picker_selected_label: ops-watch") || !strings.Contains(view, "terminal_picker_selected_state: running") || !strings.Contains(view, "ops-watch") {
...
func TestE2ERunScenarioLayoutResolveMoveUpdatesView(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if view := current.View(); !strings.Contains(view, "layout_resolve_pane: pane-1") || !strings.Contains(view, "layout_resolve_selected: create_new") || !strings.Contains(view, "> [create_new] create new") {
