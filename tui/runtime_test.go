...
func TestE2ERunScenarioDefaultModernFloatingWorkbenchRendersWindowDeck(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if strings.Contains(stripped, "Window deck") || strings.Contains(stripped, "Workbench shell") || strings.Contains(view, "wireframe_view:") {
				t.Fatalf("expected default modern floating view to use pane canvas layout without dashboard/debug sections, got:\n%s", view)
			}
			return nil
		},
	}
...
func TestE2ERunScenarioDefaultModernHelpOverlayRendersStructuredModal(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			...
			if !strings.Contains(stripped, "api-dev  ● run  owner") || !strings.Contains(stripped, "FOOTER[esc close]") || !strings.Contains(stripped, "░") {
