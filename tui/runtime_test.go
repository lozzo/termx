...
func TestE2ERunScenarioFloatingLayerShowsOutline(t *testing.T) {
	...
	runner := &stubProgramRunner{
		run: func(model *btui.Model) error {
			view := model.View()
			if !strings.Contains(view, "STATE[floating focus=floating mode=none overlay=none]") || !strings.Contains(view, "BODY[floating t=0 f=2]") || !strings.Contains(view, "FLOAT SHELL[2]") || !strings.Contains(view, "FLOAT CANVAS[78x12 windows=2]") || !strings.Contains(view, "STACK[windows] total=2") || !strings.Contains(view, "FOCUS[float-1] api-dev") || !strings.Contains(view, "WINDOW LIST[2]") || !strings.Contains(view, "> [float-1] api-dev | owner | 10,8 30x12 | running | api ready") || !strings.Contains(view, "  [float-2] build-log | owner | 45,14 28x10 | running | build ok") || strings.Contains(view, "WINDOW CARD[float-1]") || strings.Contains(view, "WINDOW CARD[float-2]") || !strings.Contains(view, "api-dev [owner] [floating]") || !strings.Contains(view, "api ready") || !strings.Contains(view, "build ok") || !strings.Contains(view, "term-1 running owner") || !strings.Contains(view, "term-2 running owner") || !strings.Contains(view, "tab_path_bar: path=main/shell/floating:float-1 | target=api-dev") || !strings.Contains(view, "tab_layer_bar: tiled_root=<none> | floating_top=float-2 | floating_total=2") || !strings.Contains(view, "floating_outline_bar: active=float-1 | total=2 | top=float-2") || !strings.Contains(view, "floating_outline:") || !strings.Contains(view, "> [floating] api-dev | role=owner | rect=10,8 30x12 | state=running | preview=api ready") || !strings.Contains(view, "  [floating] build-log | role=owner | rect=45,14 28x10 | state=running | preview=build ok") {
