...
func run(args []string, stdout io.Writer, now func() time.Time) error {
	flags := flag.NewFlagSet("muxvia-cloud-release", flag.ContinueOnError)
	...
	flags.StringVar(&binaryPath, "binary", "", "built muxvia-cloud executable")
	...
	baseName := fmt.Sprintf("muxvia-cloud_%s_%s_%s", strings.TrimPrefix(version, "v"), targetOS, targetArch)
