...
func TestReleaseToolProducesInstallerVerifiableArtifactWithoutKeyLeak(t *testing.T) {
	...
	err = run([]string{
		...
		"--download-url", "https://releases.termx.dev/cloud-companion/artifacts/v1.2.3.tar.gz", "--out", outputDir,
	}, &output, func() time.Time { return now })
	if err != nil {
		t.Fatal(err)
	}
	manifestPath := filepath.Join(outputDir, "termx-cloud_1.2.3_"+runtime.GOOS+"_"+runtime.GOARCH+".json")
	archivePath := filepath.Join(outputDir, "termx-cloud_1.2.3_"+runtime.GOOS+"_"+runtime.GOARCH+".tar.gz")
