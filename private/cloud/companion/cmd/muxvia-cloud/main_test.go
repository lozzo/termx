...
func TestSmokeServerCompletesPublicHelloStatusAndShutdown(t *testing.T) {
	runtimeDir, err := os.MkdirTemp("/tmp", "muxvia-cloud-test-")
	if err != nil {
		t.Fatal(err)
	}
	...
	hello, err := client.Hello(deadline, &cloudpb.CompanionHelloRequest{
		ProtocolMin: cloudcompanion.ProtocolVersionMin, ProtocolMax: cloudcompanion.ProtocolVersionMax, MuxviaVersion: "test", CallerRole: cloudpb.CallerRole_CALLER_ROLE_CLI,
...
func TestLicensesCommandPrintsEmbeddedThirdPartyNotices(t *testing.T) {
	...
	if !bytes.Contains(stdout.Bytes(), []byte("Muxvia Cloud Companion Third-Party Notices")) ||
