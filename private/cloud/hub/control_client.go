...
func NewControlClient(config ControlClientConfig) (*ControlClient, error) {
	...
	if err != nil || parsed.Scheme != "http" && parsed.Scheme != "https" || parsed.Host == "" || parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" || parsed.Path != "" || config.Metadata == nil || config.Metadata.GetHubId() == "" || config.Metadata.GetEdgeDeploymentId() == "" || len(config.PrivateKey) != ed25519.PrivateKeySize || config.Projection == nil {
...
func (client *ControlClient) runConnection(ctx context.Context) error {
	challenge, err := client.challenge(ctx)
	if err != nil {
		return err
	}
	...
	request, _ := http.NewRequestWithContext(ctx, http.MethodPost, client.controllerURL+hubControlOpenPath, bytes.NewReader(payload))
	...
	var generation, controllerSequence, hubSequence uint64
	for {
		...
		client.setAttached(generation, controllerSequence)
		hubSequence++
		if err := client.reportReconciliation(ctx, generation, hubSequence); err != nil {
			return err
		}
	}
}
...
func (client *ControlClient) reportReconciliation(ctx context.Context, generation, sequence uint64) error {
	snapshot := client.projection.Snapshot()
	now := client.clock.Now().UTC()
	envelope := &cloudpb.HubRuntimeEnvelope{HubId: client.metadata.GetHubId(), EdgeDeploymentId: client.metadata.GetEdgeDeploymentId(), ControlGeneration: generation, SenderRole: cloudpb.ControlSenderRole_CONTROL_SENDER_ROLE_HUB, SenderSequence: sequence, IssuedAtUnixMillis: now.UnixMilli(), ExpiresAtUnixMillis: now.Add(time.Minute).UnixMilli(), Payload: &cloudpb.HubRuntimeEnvelope_Reconciliation{Reconciliation: &cloudpb.ReconciliationDigest{HubId: client.metadata.GetHubId(), ProjectionRevision: snapshot.Revision, ProjectionDigest: snapshot.Digest, ObservedAtUnixMillis: now.UnixMilli()}}}
	response := &cloudpb.ReportHubRuntimeResponse{}
	if err := client.unary(ctx, hubControlReportPath, &cloudpb.ReportHubRuntimeRequest{Envelopes: []*cloudpb.HubRuntimeEnvelope{envelope}}, response); err != nil {
		return err
	}
