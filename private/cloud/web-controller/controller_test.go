...
func TestControllerIsThinAuditedManagementFacade(t *testing.T) {
	...
	controller, err := webcontroller.New(devices, auditLog, usageReader{value: usage.SessionUsage{BytesUp: 10, BytesDown: 20}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(devices.devices) != 1 || len(auditLog.events) != 1 {
		t.Fatalf("domain writes devices=%d audit=%d", len(devices.devices), len(auditLog.events))
	}
