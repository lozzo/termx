...
Muxvia 只构建一个面向用户的 Android App。该目录是同一 App 的 first-party Cloud module，作为标准 source set 编入 APK；默认构建没有开发 Cloud origin，账号与 managed Route 均 fail closed，但 Direct/SSH 不受影响。
...
```bash
./gradlew -PmuxviaPublicHTTPStaging=true testDebugUnitTest assembleDebug
```
