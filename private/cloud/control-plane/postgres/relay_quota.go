...
func (store *Store) Reserve(ctx context.Context, request relayquota.ReserveRequest, now time.Time) (*cloudpb.RelayLeaseReservation, *cloudpb.RelayQuotaPeriod, bool, error) {
	...
	if err := queryRowContext(ctx, tx, `SELECT COUNT(*),COALESCE(SUM(reserved_bytes-used_bytes),0) FROM relay_lease_reservations WHERE account_id=? AND period_start_unix_millis=? AND state=?`, request.AccountID, request.PeriodStart.UnixMilli(), cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE).Scan(&activeCount, &reservedBytes); err != nil {
		return nil, nil, false, err
	}
	var usedBytes uint64
	if err := queryRowContext(ctx, tx, `SELECT used_bytes FROM relay_quota_periods WHERE account_id=? AND period_start_unix_millis=?`, request.AccountID, request.PeriodStart.UnixMilli()).Scan(&usedBytes); err != nil {
		return nil, nil, false, err
	}
	...
	if _, err := execContext(ctx, tx, `INSERT INTO relay_lease_reservations(lease_id,account_id,managed_session_id,client_device_id,target_device_id,region,period_start_unix_millis,period_end_unix_millis,reserved_bytes,used_bytes,state,expires_at_unix_millis,release_after_unix_millis,revision,projection) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
	...
	if _, err := execContext(ctx, tx, `UPDATE relay_quota_periods SET revision=revision+1 WHERE account_id=? AND period_start_unix_millis=?`, request.AccountID, request.PeriodStart.UnixMilli()); err != nil {
		return nil, nil, false, err
	}
	period, err := relayQuotaPeriod(ctx, tx, request.AccountID, request.PeriodStart, request.PeriodEnd, request.PeriodLimitBytes)
	if err != nil {
...
func (store *Store) Release(ctx context.Context, accountID, leaseID string, now time.Time) (*cloudpb.RelayLeaseReservation, *cloudpb.RelayQuotaPeriod, error) {
	...
	if reservation.GetState() == cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE {
		...
		result, updateErr := execContext(ctx, tx, `UPDATE relay_lease_reservations SET state=?,revision=?,projection=? WHERE lease_id=? AND state=?`, reservation.GetState(), reservation.GetRevision(), body, leaseID, cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE)
		if updateErr != nil {
		...
		if _, err := execContext(ctx, tx, `UPDATE relay_quota_periods SET revision=revision+1 WHERE account_id=? AND period_start_unix_millis=?`, accountID, reservation.GetPeriodStartUnixMillis()); err != nil {
			return nil, nil, err
		}
	}
...
func (store *Store) Snapshot(ctx context.Context, accountID string, periodStart, periodEnd, now time.Time) (*cloudpb.GetAccountRelayQuotaResponse, error) {
	...
	rows, err := queryContext(ctx, tx, `SELECT projection FROM relay_lease_reservations WHERE account_id=? AND period_start_unix_millis=? AND state=? ORDER BY lease_id`, accountID, periodStart.UnixMilli(), cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (store *Store) SnapshotForPeriod(ctx context.Context, accountID string, periodStart, periodEnd time.Time, limit uint64, now time.Time) (*cloudpb.GetAccountRelayQuotaResponse, error) {
	...
	defer tx.Rollback()
	if _, err := execContext(ctx, tx, `INSERT INTO relay_quota_periods(account_id,period_start_unix_millis,period_end_unix_millis,limit_bytes,used_bytes,revision) VALUES(?,?,?,?,0,1) ON CONFLICT(account_id,period_start_unix_millis) DO NOTHING`, accountID, periodStart.UnixMilli(), periodEnd.UnixMilli(), limit); err != nil {
		return nil, err
	}
	if err := expireRelayReservations(ctx, tx, now.UTC()); err != nil {
		return nil, err
...
func ensureRelayPeriod(ctx context.Context, tx *sql.Tx, request relayquota.ReserveRequest) error {
	result, err := execContext(ctx, tx, `INSERT INTO relay_quota_periods(account_id,period_start_unix_millis,period_end_unix_millis,limit_bytes,used_bytes,revision) VALUES(?,?,?,?,0,1) ON CONFLICT(account_id,period_start_unix_millis) DO NOTHING`, request.AccountID, request.PeriodStart.UnixMilli(), request.PeriodEnd.UnixMilli(), request.PeriodLimitBytes)
	if err != nil {
		return err
	}
	if changed, _ := result.RowsAffected(); changed == 1 {
	...
	var periodEnd int64
	var limit uint64
	if err := queryRowContext(ctx, tx, `SELECT period_end_unix_millis,limit_bytes FROM relay_quota_periods WHERE account_id=? AND period_start_unix_millis=? FOR UPDATE`, request.AccountID, request.PeriodStart.UnixMilli()).Scan(&periodEnd, &limit); err != nil {
		return err
	}
...
func expireRelayReservations(ctx context.Context, tx *sql.Tx, now time.Time) error {
	rows, err := queryContext(ctx, tx, `SELECT lease_id,account_id,period_start_unix_millis,projection FROM relay_lease_reservations WHERE state=? AND release_after_unix_millis<=?`, cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE, now.UnixMilli())
	if err != nil {
		return err
	}
	type expired struct {
	...
	for _, value := range values {
		...
		if _, err := execContext(ctx, tx, `UPDATE relay_lease_reservations SET state=?,revision=?,projection=? WHERE lease_id=? AND state=?`, reservation.GetState(), reservation.GetRevision(), body, value.leaseID, cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE); err != nil {
			return err
		}
		if _, err := execContext(ctx, tx, `UPDATE relay_quota_periods SET revision=revision+1 WHERE account_id=? AND period_start_unix_millis=?`, value.accountID, value.periodStart); err != nil {
			return err
		}
	}
	return nil
...
func relayReservation(ctx context.Context, tx *sql.Tx, leaseID string) (*cloudpb.RelayLeaseReservation, error) {
	var body []byte
	if err := queryRowContext(ctx, tx, `SELECT projection FROM relay_lease_reservations WHERE lease_id=? FOR UPDATE`, leaseID).Scan(&body); err != nil {
...
func relayQuotaPeriod(ctx context.Context, tx *sql.Tx, accountID string, periodStart, periodEnd time.Time, expectedLimit uint64) (*cloudpb.RelayQuotaPeriod, error) {
	...
	if err := queryRowContext(ctx, tx, `SELECT period_end_unix_millis,limit_bytes,used_bytes,revision FROM relay_quota_periods WHERE account_id=? AND period_start_unix_millis=? FOR UPDATE`, accountID, periodStart.UnixMilli()).Scan(&storedEnd, &limit, &used, &revision); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, relayquota.ErrReservationNotFound
		}
		return nil, err
	...
	var reserved uint64
	var active uint32
	if err := queryRowContext(ctx, tx, `SELECT COALESCE(SUM(reserved_bytes-used_bytes),0),COUNT(*) FROM relay_lease_reservations WHERE account_id=? AND period_start_unix_millis=? AND state=?`, accountID, periodStart.UnixMilli(), cloudpb.RelayReservationState_RELAY_RESERVATION_STATE_ACTIVE).Scan(&reserved, &active); err != nil {
		return nil, err
	}
...
func (store *Store) RelayReservation(ctx context.Context, leaseID string) (*cloudpb.RelayLeaseReservation, error) {
	var body []byte
	if err := queryRowContext(ctx, store.db, `SELECT projection FROM relay_lease_reservations WHERE lease_id=?`, leaseID).Scan(&body); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, relayquota.ErrReservationNotFound
		}
		return nil, err
...
func (store *Store) RelayReservationsForSession(ctx context.Context, managedSessionID string) ([]*cloudpb.RelayLeaseReservation, error) {
	rows, err := queryContext(ctx, store.db, `SELECT projection FROM relay_lease_reservations WHERE managed_session_id=? ORDER BY lease_id`, managedSessionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
