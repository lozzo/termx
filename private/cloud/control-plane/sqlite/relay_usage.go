...
func (store *Store) ApplyRelayUsage(ctx context.Context, record *cloudpb.RelayUsageRecord, claims servicecredential.RelayLeaseClaims, event usage.Event, digest [sha256.Size]byte, now time.Time) (*cloudpb.RelayUsageAck, *cloudpb.RelayQuotaPeriod, *cloudpb.RelayUsageAggregate, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT COALESCE(MAX(sequence),0) FROM relay_usage_events WHERE relay_id=? AND lease_id=?`, event.RelayID, event.LeaseID).Scan(&lastSequence)
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO relay_usage_events(relay_id,lease_id,sequence,event_id,digest,record,created_at_unix_millis) VALUES(?,?,?,?,?,?,?)`, event.RelayID, event.LeaseID, event.Sequence, event.EventID, digest[:], recordBody, now.UTC().UnixMilli()); err != nil {
		return nil, nil, nil, usage.ErrJournalConflict
	}
	...
	if _, err := tx.ExecContext(ctx, `UPDATE relay_lease_reservations SET used_bytes=?,state=?,revision=?,projection=? WHERE lease_id=?`, reservation.GetUsedBytes(), reservation.GetState(), reservation.GetRevision(), reservationBody, reservation.GetLeaseId()); err != nil {
		return nil, nil, nil, err
	}
	period.UsedBytes += delta
	period.Revision++
	if _, err := tx.ExecContext(ctx, `UPDATE relay_quota_periods SET used_bytes=?,revision=? WHERE account_id=? AND period_start_unix_millis=?`, period.GetUsedBytes(), period.GetRevision(), period.GetAccountId(), period.GetPeriodStartUnixMillis()); err != nil {
		return nil, nil, nil, err
	}
...
func relayUsageReplay(ctx context.Context, tx *sql.Tx, event usage.Event, digest [sha256.Size]byte) (bool, error) {
	var stored []byte
	err := tx.QueryRowContext(ctx, `SELECT digest FROM relay_usage_events WHERE relay_id=? AND lease_id=? AND sequence=?`, event.RelayID, event.LeaseID, event.Sequence).Scan(&stored)
...
func relayUsageProjection(ctx context.Context, tx *sql.Tx, reservation *cloudpb.RelayLeaseReservation, managedSessionID, routeID string) (*cloudpb.RelayQuotaPeriod, *cloudpb.RelayUsageAggregate, error) {
	...
	var body []byte
	if err := tx.QueryRowContext(ctx, `SELECT projection FROM relay_usage_aggregates WHERE account_id=? AND managed_session_id=? AND route_id=? AND period_start_unix_millis=?`, reservation.GetAccountId(), managedSessionID, routeID, reservation.GetPeriodStartUnixMillis()).Scan(&body); err == nil {
