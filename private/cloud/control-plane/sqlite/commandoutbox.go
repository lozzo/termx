...
func (store *Store) CreateCommand(ctx context.Context, record commandoutbox.Record) (commandoutbox.Record, bool, error) {
	...
	defer tx.Rollback()
	result, err := tx.ExecContext(ctx, `INSERT INTO management_commands(command_id,account_id,idempotency_key,command_kind,delivery_state,execution_state,expires_at,updated_at,version,projection) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(account_id,idempotency_key) DO NOTHING`, record.Projection.GetCommandId(), record.Projection.GetAccountId(), record.IdempotencyKey, record.Projection.GetCommandKind(), record.Projection.GetDeliveryState(), record.Projection.GetExecutionState(), record.Projection.GetExpiresAtUnixMillis(), record.Projection.GetUpdatedAtUnixMillis(), record.Version, body)
	if err != nil {
		return commandoutbox.Record{}, false, commandoutbox.ErrCommandConflict
	}
	inserted, _ := result.RowsAffected()
	if inserted == 0 {
		stored, err := scanCommandRecord(tx.QueryRowContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE account_id=? AND idempotency_key=?`, record.Projection.GetAccountId(), record.IdempotencyKey))
		return stored, false, err
	}
	for _, child := range record.Projection.GetChildren() {
		if _, err := tx.ExecContext(ctx, `INSERT INTO management_command_children(child_command_id,parent_command_id,target_hub_id) VALUES(?,?,?)`, child.GetChildCommandId(), record.Projection.GetCommandId(), child.GetTargetHubId()); err != nil {
			return commandoutbox.Record{}, false, commandoutbox.ErrCommandConflict
		}
	}
...
func (store *Store) CreateDeviceRevokeCommand(ctx context.Context, record commandoutbox.Record, deviceID string, expectedAuthEpoch uint64) (commandoutbox.Record, bool, error) {
	...
	defer tx.Rollback()
	stored, loadErr := scanCommandRecord(tx.QueryRowContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE account_id=? AND idempotency_key=?`, record.Projection.GetAccountId(), record.IdempotencyKey))
	if loadErr == nil {
	...
	updated, err := tx.ExecContext(ctx, `UPDATE cloud_device_ownership SET auth_epoch=auth_epoch+1,revoked=1,updated_at=? WHERE device_id=? AND account_id=? AND auth_epoch=? AND revoked=0`, updatedAt, deviceID, record.Projection.GetAccountId(), expectedAuthEpoch)
	if err != nil {
		return commandoutbox.Record{}, false, err
	}
...
func (store *Store) Command(ctx context.Context, accountID, commandID string) (commandoutbox.Record, error) {
	return scanCommandRecord(store.db.QueryRowContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE account_id=? AND command_id=?`, accountID, commandID))
}
...
func (store *Store) CommandByIdempotency(ctx context.Context, accountID, idempotencyKey string) (commandoutbox.Record, error) {
	return scanCommandRecord(store.db.QueryRowContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE account_id=? AND idempotency_key=?`, accountID, idempotencyKey))
}
...
func (store *Store) CommandByChild(ctx context.Context, childID string) (commandoutbox.Record, error) {
	return scanCommandRecord(store.db.QueryRowContext(ctx, `SELECT c.projection,c.idempotency_key,c.version FROM management_commands c JOIN management_command_children child ON child.parent_command_id=c.command_id WHERE child.child_command_id=?`, childID))
}
...
func (store *Store) OpenCommands(ctx context.Context, limit int) ([]commandoutbox.Record, error) {
	...
	rows, err := store.db.QueryContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE execution_state=? ORDER BY updated_at,command_id LIMIT ?`, cloudpb.CommandExecutionState_COMMAND_EXECUTION_STATE_PENDING, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (store *Store) ListCommands(ctx context.Context, accountID string, kind cloudpb.ManagementCommandKind, execution cloudpb.CommandExecutionState, limit int) ([]commandoutbox.Record, error) {
	...
	rows, err := store.db.QueryContext(ctx, `SELECT projection,idempotency_key,version FROM management_commands WHERE account_id=? AND (?=0 OR command_kind=?) AND (?=0 OR execution_state=?) ORDER BY updated_at DESC,command_id DESC LIMIT ?`, accountID, kind, kind, execution, execution, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (store *Store) ApplyCommandResult(ctx context.Context, next commandoutbox.Record, expectedVersion uint64, childID, resultKind string, digest [sha256.Size]byte, resultBody []byte) (commandoutbox.Record, bool, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT result_kind,digest FROM management_command_results WHERE child_command_id=? AND result_kind=?`, childID, resultKind).Scan(&storedKind, &storedDigest)
	if err == nil {
		if storedKind != resultKind || !bytes.Equal(storedDigest, digest[:]) {
			return commandoutbox.Record{}, false, commandoutbox.ErrCommandConflict
		}
		stored, loadErr := scanCommandRecord(tx.QueryRowContext(ctx, `SELECT c.projection,c.idempotency_key,c.version FROM management_commands c JOIN management_command_children child ON child.parent_command_id=c.command_id WHERE child.child_command_id=?`, childID))
...
func (store *Store) ReplaceCommand(ctx context.Context, next commandoutbox.Record, expectedVersion uint64) (commandoutbox.Record, error) {
	...
	result, err := store.db.ExecContext(ctx, `UPDATE management_commands SET delivery_state=?,execution_state=?,expires_at=?,updated_at=?,version=?,projection=? WHERE command_id=? AND version=?`, next.Projection.GetDeliveryState(), next.Projection.GetExecutionState(), next.Projection.GetExpiresAtUnixMillis(), next.Projection.GetUpdatedAtUnixMillis(), next.Version, body, next.Projection.GetCommandId(), expectedVersion)
	if err != nil {
		return commandoutbox.Record{}, err
	}
