...
- `docs/remote-platform/multi-hub-control-topology-spec.md` 是 Hub registry/assignment、纯内存同步、daemon runtime topology、CommandOutbox 和远程管理的唯一稳定架构基准。
...
- Control Plane 只拥有账号、设备目录、entitlement、managed admission/signaling metadata、CommandOutbox、topology projection、服务准入票据、Relay 租约和 usage 结算；authenticated ManagedPeerSession 真值属于 daemon Go runtime。
...
- 正式 Hub 不落盘 policy、Presence、signaling、topology 或 command dedupe；Hub 重启后必须从 Control Plane full sync。Control Plane command 不得绕过 assignment/presence/session incarnation fencing。
- 新导出声明和关键消息链路写清晰中文注释。
