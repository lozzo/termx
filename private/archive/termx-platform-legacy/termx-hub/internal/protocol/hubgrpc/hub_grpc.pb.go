...
const (
	AgentHub_Connect_FullMethodName = "/termxhub.v1.AgentHub/Connect"
)
...
func (c *agentHubClient) Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentToHub, HubToAgent], error) {
...
type AgentHub_ConnectClient = grpc.BidiStreamingClient[AgentToHub, HubToAgent]
...
func (UnimplementedAgentHubServer) Connect(grpc.BidiStreamingServer[AgentToHub, HubToAgent]) error {
...
func _AgentHub_Connect_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentHubServer).Connect(&grpc.GenericServerStream[AgentToHub, HubToAgent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentHub_ConnectServer = grpc.BidiStreamingServer[AgentToHub, HubToAgent]
...
var AgentHub_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "termxhub.v1.AgentHub",
