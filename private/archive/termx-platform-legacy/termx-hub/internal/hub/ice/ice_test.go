...
import (
	...
	hubturn "github.com/lozzow/termx/termx-hub/internal/hub/turn"
)
...
func TestEmbeddedTURNCredentialsCarryMachineIDForTrafficAttribution(t *testing.T) {
	ctx := context.Background()
	clock := fixedClock(time.Date(2026, 5, 5, 7, 30, 0, 0, time.UTC))
	turnServer, err := hubturn.NewServer(hubturn.Config{
		ListenAddr: "127.0.0.1:0",
		PublicIP:   "127.0.0.1",
		Secret:     "embedded-secret",
	...
	if !strings.HasPrefix(cfg.ICEServers[1].Username, "machine_attr:") {
		t.Fatalf("embedded turn username should be machine-attributed, got %q", cfg.ICEServers[1].Username)
	}
	if key, ok := turnServer.AuthHandler()(cfg.ICEServers[1].Username, "termx", nil); !ok || len(key) == 0 {
		t.Fatal("machine-attributed embedded turn credential did not authenticate")
	}
}
