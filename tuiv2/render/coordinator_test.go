...
func TestRenderBodyAmbiguousEmojiVariationSelectorKeepsEmojiInAllHostModesAwayFromBoundary(t *testing.T) {
	wb := workbench.NewWorkbench()
	wb.AddWorkspace("main", &workbench.WorkspaceState{
		Name:      "main",
		ActiveTab: 0,
	...
	if !strings.Contains(body, "♻️"+xansi.ECH(1)) {
		t.Fatalf("expected renderBody to keep the raw emoji away from the right boundary, got %q", body)
	}
	if !strings.Contains(body, xansi.CHA(6)+":") {
		t.Fatalf("expected renderBody to re-anchor after the preserved ambiguous emoji, got %q", body)
	}
}

func TestRenderBodyAmbiguousEmojiVariationSelectorKeepsEmojiWhenRightBorderStillOneGutterAway(t *testing.T) {
	wb := workbench.NewWorkbench()
	wb.AddWorkspace("main", &workbench.WorkspaceState{
		Name:      "main",
		ActiveTab: 0,
	...
	row := make([]protocol.Cell, contentWidth)
	row[0] = protocol.Cell{Content: "A", Width: 1}
	row[1] = protocol.Cell{Content: "❄️", Width: 2}
	row[2] = protocol.Cell{Content: "", Width: 0}
	for i := 3; i < len(row); i++ {
		row[i] = protocol.Cell{Content: " ", Width: 1}
	}
	state := WithTermSize(AdaptVisibleStateWithSize(wb, runtime.New(nil), bodyWidth, bodyHeight), bodyWidth, bodyHeight+2)
	state.Runtime = &VisibleRuntimeStateProxy{
		...
		Terminals: []runtime.VisibleTerminal{{
			...
			Snapshot: &protocol.Snapshot{
				TerminalID: "term-1",
				Size:       protocol.Size{Cols: uint16(contentWidth), Rows: 1},
				Screen:     protocol.ScreenData{Cells: [][]protocol.Cell{row}},
				Cursor:     protocol.CursorState{Visible: false},
				Modes:      protocol.TerminalModes{AutoWrap: true},
			},
	...
	body := renderBody(state, bodyWidth, bodyHeight)
	if !strings.Contains(body, "❄️"+xansi.ECH(1)) {
		t.Fatalf("expected renderBody to keep the emoji while the reserved gutter still separates it from the border, got %q", body)
	}
}
