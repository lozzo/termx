...
func TestLoadSnapshotIntoVTermRoundTripsExplicitOwnershipProjection(t *testing.T) {
	snapshot := &protocol.Snapshot{
		TerminalID: "term-1",
		Size:       protocol.Size{Cols: 8, Rows: 2},
		Scrollback: protocol.CompactRowsFromCells([][]protocol.Cell{
			snapshotTestRow("persist"),
			snapshotTestRow("tail"),
		}),
		...
		ScreenOwnership: []string{protocol.RowOwnershipScreen},
		Cursor:          protocol.CursorState{Visible: true},
		Modes:           protocol.TerminalModes{AutoWrap: true},
	}
	vt := localvterm.New(8, 2, 100, nil)

	loadSnapshotIntoVTerm(vt, snapshot)
	got := snapshotFromVTerm("term-1", vt)

	if !reflect.DeepEqual(got.ScrollbackOwnership, snapshot.ScrollbackOwnership) {
	...
	if got.HistoryGeneration != 7 || got.ScrollbackFirstRowID != 0 || got.ScrollbackLastRowID != 0 {
		t.Fatalf("expected history metadata to follow explicit projection ownership, got generation=%d rows=%d..%d", got.HistoryGeneration, got.ScrollbackFirstRowID, got.ScrollbackLastRowID)
	}
}

func TestPreserveSnapshotHistoryMetadataRequiresExplicitProjectionOwnership(t *testing.T) {
	previous := &protocol.Snapshot{
		TerminalID:             "term-1",
		Size:                   protocol.Size{Cols: 8, Rows: 2},
		Scrollback:             protocol.CompactRowsFromCells([][]protocol.Cell{snapshotTestRow("persist")}),
		ScrollbackLoadedRows:   1,
		HistoryGeneration:      7,
		ScrollbackFirstRowID:   0,
		ScrollbackLastRowID:    0,
	...
	next := &protocol.Snapshot{
		TerminalID: "term-1",
		Size:       previous.Size,
		Scrollback: protocol.CompactRowsFromCells([][]protocol.Cell{snapshotTestRow("same-row")}),
	}
	...
	if next.HistoryGeneration != 0 || next.ScrollbackLoadedRows != 0 {
		t.Fatalf("expected missing projection ownership not to inherit metadata, got generation=%d loaded=%d", next.HistoryGeneration, next.ScrollbackLoadedRows)
	}
}
