...
状态：R437 后本文为 R318-R418 mutation-backed logical renderer 的设计背景和 legacy harness 说明，不再是默认 production history ingest 基准。当前默认链路已经由 `history/linehist` 持有 cold file-backed logical lines，并从 vterm hot screen 投影 `HistoryWindow`；本文中的 `HistoryLogicalRenderer -> HistoryMutationBatch -> HistoryStore logical lines` 只能按历史背景理解，不能作为 journal 失败 fallback，也不能重新接回默认 daemon。
...
不能扩散成 history raw PTY parser fallback；history 仍必须消费 vterm semantic pass，
