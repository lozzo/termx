...
状态：历史背景。R318 后，history 语义设计基准曾切换到 `core/docs/history-logical-renderer-design.md`；R430 后曾切到 screen-backed 设计；R431 后用户确认不再修补 screen-backed 路径，当前 production 基准已切到 `history/linehist` 的 file-backed logical-line + vterm hot-screen projection。
