...
```text
PTY bytes / resize
  -> history SemanticTap / vterm TerminalSemanticTransaction
...
- `tui` 只消费 core-v2 返回的 authoritative history 数据；普通模式消费按需投影的 history window，copy mode 消费冻结的 logical-line snapshot。
...
- core-v2 内部不直接依赖现有 `proto` 或 `internal/protocol` 的 wire 结构。
...
第一阶段范围是旧 logical-line-first 重构记录，R430 后不再作为当前执行计划。它当时只做：
