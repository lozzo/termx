...
- 当前主线默认依赖 `termx-core-v2`、`termx-tui-v3`、`termx-shared` 与必要协议包；不得恢复 `termx-remote` runtime、命令或 module replace。
- 不得恢复旧 `termx-core`、`tuiv2`、legacy daemon 或 remote fallback adapter。
- 不要把新的 shell-neutral 能力继续塞回 CLI。

## Frozen Remote CLI Rules

`termx-cli` 的 frozen remote 入口已从默认主线清理。后续只能按根 `workflow.md` 的明确切片重新设计新控制面：
