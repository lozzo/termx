...
- Edge：面向 daemon 和客户端的公网入口、内存 Presence、WebRTC 信令、TURN Relay 及使用量 outbox。
...
7. Relay 租约由 Edge 在 daemon binding 的签名委托上限内本地创建和续期；单次会话不向 Controller 同步申请。
8. Controller 与 Edge 的长连接只承担状态投影、策略与证书控制、主动关闭命令和用量结算。
...
6. Controller 签发 `DaemonBindingClaims`，绑定 daemon、账号、设备公钥、owning Edge、locator 摘要、revision、有效期和可选 Relay 委托。
...
Relay 与连接存活是不同状态。heartbeat 只证明传输活着，租约决定 TURN allocation 是否仍可转发。

Edge 从当前认证 daemon claims 中取得签名委托，并把每个租约收缩到以下最小值：

- binding 有效期。
- 固定五分钟租约窗口。
- 最大字节数、速率和并发 allocation。
- 当前 account、Edge、daemon、client 和 session。

续租保持同一 lease ID，不触发 Controller RPC 或 ICE restart。session 主动关闭时，Edge 立即撤销租约并关闭对应 TURN allocation；不会等待自然过期。

使用量在 allocation 关闭或窗口结束时冻结为幂等事件，先写 Edge durable outbox。Controller 不可达时继续积压；恢复后按原 event ID 重试并由 Controller 事务去重确认。

## 10. 真值与持久化

Controller PostgreSQL 保存账号、订阅、daemon identity、Edge 配置、证书、用量和运营审计。实时 Presence 只存在于 Controller Directory 内存，由 Edge snapshot/delta 重建。

Edge 内存保存当前 AgentGateway、ClientGateway、Relay allocation、认证 claims、correlation 和有界 mailbox。Edge 磁盘只保存运行配置、证书、可验证配置缓存和 usage outbox。
...
EdgeControl 使用 mTLS 双向流。协议版本 3 的 payload 仅包含：
...
单次用户连接和 Relay 租约不进入 EdgeControl。Controller 断开不能中断已有 P2P/DataChannel，也不能阻止仍在有效签名委托内的 Edge 本地 Relay 会话。
...
| 故障 | 当前行为 |
| --- | --- |
| Controller 停止，Edge 仍运行 | daemon 可重连同一 Edge；已授权客户端和 pairing 可直连；Relay 在 binding 委托内继续 |
...
| usage 上报失败 | 事件保留在 outbox，恢复后幂等补报 |
...
- Controller 停止后的 cached P2P、pairing 和新 Relay session 集成测试。
