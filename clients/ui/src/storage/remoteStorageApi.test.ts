...
describe('createRemoteStorageApi generated Proto API', () => {
  it('routes CRUD through typed storage commands', async () => {
    const key = create(StorageKeySchema, { appId: 'muxvia.paths', scope: StorageScope.PUBLIC, key: 'bookmarks/project' })
    ...
    expect(storageText(await api.put({ appId: 'muxvia.paths', key: 'bookmarks/project', value: 'hello', expectedVersion: 2 }))).toBe('hello')
    expect(storageText(await api.get({ appId: 'muxvia.paths', key: 'bookmarks/project' }))).toBe('hello')
    await expect(api.list({ appId: 'muxvia.paths', prefix: 'bookmarks/' })).resolves.toHaveLength(1)
    await expect(api.delete({ appId: 'muxvia.paths', key: 'bookmarks/project' })).resolves.toEqual({ deleted: true, version: 4 })
