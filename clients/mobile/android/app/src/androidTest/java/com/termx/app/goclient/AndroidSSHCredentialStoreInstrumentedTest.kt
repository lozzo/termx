...
class AndroidSSHCredentialStoreInstrumentedTest {
    ...
    fun exportsExplicitlyRequestedPublicKeyForE2EFixture() {
        val ref = InstrumentationRegistry.getArguments().getString("termxSshCredentialRef")?.trim().orEmpty()
    ...
    fun createsReusesSignsAndDeletesNonExportableSigner() {
        ...
        val digest = MessageDigest.getInstance("SHA-256").digest("termx-ssh-keystore".toByteArray())
