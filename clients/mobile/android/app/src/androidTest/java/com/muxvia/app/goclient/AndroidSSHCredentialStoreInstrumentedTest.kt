...
class AndroidSSHCredentialStoreInstrumentedTest {
    ...
    fun exportsExplicitlyRequestedPublicKeyForE2EFixture() {
        val ref = InstrumentationRegistry.getArguments().getString("muxviaSshCredentialRef")?.trim().orEmpty()
    ...
    fun createsReusesSignsAndDeletesNonExportableSigner() {
        ...
        val digest = MessageDigest.getInstance("SHA-256").digest("muxvia-ssh-keystore".toByteArray())
