...
#include "muxvia_client.h"

extern muxvia_status_v1 muxvia_android_spike_set_runtime_dir(const char *runtime_dir);
extern muxvia_status_v1 muxvia_android_spike_engine_create(muxvia_handle_t *out_engine_handle);

static int throw_status(JNIEnv *env, muxvia_status_v1 status) {
  if (status == MUXVIA_STATUS_OK) {
    return 0;
  }
  jclass exception = (*env)->FindClass(env, "java/lang/IllegalStateException");
  if (exception != NULL) {
    char message[64];
    snprintf(message, sizeof(message), "muxvia native status %d", (int)status);
...
Java_com_muxvia_app_goclient_GoClientNative_createSpike(JNIEnv *env, jobject self, jstring runtime_dir) {
  ...
  muxvia_status_v1 status = muxvia_android_spike_set_runtime_dir(path);
  (*env)->ReleaseStringUTFChars(env, runtime_dir, path);
  if (throw_status(env, status) != 0) {
    return 0;
  }
  muxvia_handle_t engine = 0;
  if (throw_status(env, muxvia_android_spike_engine_create(&engine)) != 0) {
    return 0;
  }
  return (jlong)engine;
}
...
Java_com_muxvia_app_goclient_GoClientNative_openSession(JNIEnv *env, jobject self, jlong engine, jbyteArray payload) {
  ...
  muxvia_handle_t operation = 0;
  muxvia_status_v1 status = muxvia_engine_open_session(
      (muxvia_handle_t)engine, (const uint8_t *)bytes, (size_t)length, &operation);
...
Java_com_muxvia_app_goclient_GoClientNative_execute(JNIEnv *env, jobject self, jlong engine, jlong session, jbyteArray payload) {
  ...
  muxvia_handle_t operation = 0;
  muxvia_status_v1 status = muxvia_engine_execute(
      (muxvia_handle_t)engine, (muxvia_handle_t)session,
...
Java_com_muxvia_app_goclient_GoClientNative_openResourceStream(JNIEnv *env, jobject self, jlong engine, jlong session, jbyteArray payload) {
  ...
  muxvia_handle_t stream = 0;
  muxvia_status_v1 status = muxvia_engine_open_resource_stream(
      (muxvia_handle_t)engine, (muxvia_handle_t)session,
...
Java_com_muxvia_app_goclient_GoClientNative_sendResourceStreamFrame(JNIEnv *env, jobject self, jlong engine, jlong stream, jbyteArray payload) {
  ...
  muxvia_status_v1 status = muxvia_engine_send_resource_stream_frame(
      (muxvia_handle_t)engine, (muxvia_handle_t)stream,
...
Java_com_muxvia_app_goclient_GoClientNative_engineCommand(JNIEnv *env, jobject self, jlong engine, jbyteArray payload) {
  ...
  muxvia_handle_t operation = 0;
  muxvia_status_v1 status = muxvia_engine_command(
      (muxvia_handle_t)engine, (const uint8_t *)bytes, (size_t)length, &operation);
  (*env)->ReleaseByteArrayElements(env, payload, bytes, JNI_ABORT);
  return throw_status(env, status) == 0 ? (jlong)operation : 0;
}
...
Java_com_muxvia_app_goclient_GoClientNative_nextEvent(JNIEnv *env, jobject self, jlong engine, jint timeout_millis) {
  ...
  muxvia_status_v1 free_status = muxvia_buffer_free(event.buffer_handle);
...
Java_com_muxvia_app_goclient_GoClientNative_nextPlatformRequest(JNIEnv *env, jobject self, jlong engine, jint timeout_millis) {
  ...
  muxvia_status_v1 free_status = muxvia_buffer_free(request.buffer_handle);
  if (result != NULL && throw_status(env, free_status) != 0) {
    return NULL;
  }
  return result;
...
Java_com_muxvia_app_goclient_GoClientNative_completePlatformRequest(JNIEnv *env, jobject self, jlong engine, jbyteArray payload) {
  ...
  muxvia_status_v1 status = muxvia_platform_complete(
      (muxvia_handle_t)engine, (const uint8_t *)bytes, (size_t)length);
  (*env)->ReleaseByteArrayElements(env, payload, bytes, JNI_ABORT);
  throw_status(env, status);
}
